...
def _mb_solve_mvo(dt, candidates, composite_scores, Pxs_df, sectors_s,
    ...
    try:
        valid, Sigma_emp, Sigma_lw, Sigma_factor, Sigma_pca, Sigma_ens, _f_diag = \
...
def get_last_rebal_diagnostics(results, Pxs_df,
    ...
    ic_weights = pd.Series(weights_by_date[past_wbd[-1]], name='IC weight') \
                 if past_wbd else pd.Series(dtype=float)
    ...
    scores_df.loc[scores_df.index.isin(active_u), 'filter_stage'] = \
        f'active ({len(active_u)} stocks)'
    ...
    scores_df.loc[scores_df.index.isin(prefilt_stocks), 'filter_stage'] = \
        f'prefilt ({n_prefilt} stocks)'
    ...
    scores_df.loc[scores_df.index.isin(alpha_pool), 'filter_stage'] = \
        f'alpha pool (top {n_alpha})'
    ...
    scores_df.loc[scores_df.index.isin(mvo_pool), 'filter_stage'] = \
        f'MVO pool (top {n_mvo})'

    # Stocks in both pools get the more specific label
    in_both = set(alpha_pool) & set(mvo_pool)
    scores_df.loc[scores_df.index.isin(in_both), 'filter_stage'] = \
        f'alpha+MVO pool'
...
def _state_to_json(st: dict) -> str:
    ...
    snap['last_rebal'] = st['last_rebal'].isoformat() \
                         if isinstance(st.get('last_rebal'), pd.Timestamp) else None
...
def run_backtest(
    ...
    if rebuild_cov or mode == 'rebuild':
        _mb_clear_x_cache(model_version)
        print(f"  X snapshot cache cleared "
              f"({'rebuild_cov=True' if rebuild_cov else 'mode=rebuild'})")
    ...
    N_STRAT = 10
    S_BASE, S_ALPHA, S_MVO, S_HYB, S_SMART, S_DYN, S_HEDGE, S_DD, S_EXCL, \
        S_MVO_HEDGE = range(10)
    ...
    _cached_nav, _cache_last_dt, _cached_states = \
        _cache_load(ph, STRATEGY_LABELS) if mode == 'incremental' \
        else (None, None, None)
    ...
    for day_idx, dt in enumerate(trading_days):
        ...
        if last_calc_dt is not None and last_calc_dt in composite_by_date:
            ...
            for s_idx in [S_DYN, S_HEDGE, S_DD, S_EXCL, S_MVO_HEDGE]:
                st   = state[s_idx]
                reg  = st['regime']
                ...
                if s_idx == S_EXCL:
                    ...
                    # Pure MVO weights — no regime switching
                    cands_s = cands_dyn
                    w_hyp   = _cached_dyn.get('mvo', pd.Series(dtype=float))
                else:
                    cands_s = cands_dyn
                    w_hyp = _regime_weights(dt, reg, {
