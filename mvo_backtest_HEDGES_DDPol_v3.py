...
DD_LEVELS = [
    (0.20, 0.20),  # at -20% DD: cut 20% of remaining exposure
    (0.30, 0.25),  # at -30% DD: cut 25% of remaining
    (0.40, 0.30),  # at -40% DD: cut 30% of remaining
    (0.50, 0.50),  # at -50% DD: cut 50% of remaining
    (0.60, 0.50),  # at -60% DD: cut 50% of remaining
]
...
DD_ANNUAL_RESET_PCT     = 0.25   # max YTD DD allowed for calendar-year re-entry
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    if nav_dyn_hedged is not None and not nav_dyn_hedged.empty:
        ...
        for i, dt in enumerate(nav_dyn_hedged.index[1:], 1):
            ...
            # ── Check next DD level ────────────────────────────────────────
            if next_level_idx < len(DD_LEVELS):
