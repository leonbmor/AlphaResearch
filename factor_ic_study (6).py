...
def run_composite_ic_study(ic_results, weights_df, regime_s, Pxs_df, sectors_s,
    ...
    # Re-normalise weights excluding dropped factors
    w_active = weights_df[active_factors].copy()
    row_sums = w_active.sum(axis=1)
    for r in w_active.index:
        if row_sums[r] > 0:
            w_active.loc[r] = w_active.loc[r] / row_sums[r]
        else:
            w_active.loc[r] = 1.0 / len(active_factors)
    w_active = w_active.round(4)
