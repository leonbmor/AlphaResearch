...
Runs **5 portfolios in parallel** and compares performance:
1. **Baseline** — quality factor only
...
Rebalancing frequency in days [default=30]:
Max stocks per sector (or Enter to skip):
Min market cap floor ($M, or Enter to skip):
Apply vol filter? (y/n) [default=n]:
Pre-filter fraction by composite score 0<x<=1 (or Enter for none):
Concentration factor for Pure Alpha >=1.0 (or Enter for equal weight):
Min cov matrices for stock selection (0=alpha-only, default=2):
```

### MVO Solver (`_mb_solve_mvo`)

Exact replica of `mvo_diagnostics` logic:
1. Build 4 cov matrices (Empirical, Ledoit-Wolf, Factor, PCA) + ensemble
2. Run `_mb_max_alpha_portfolio` (unconstrained solve + post-solve floor+cap) on each
3. Eligibility: stocks appearing in >= `min_cov_matrices` of the 4 individual matrices
...
### Hybrid Portfolio
Simple average of Alpha and MVO weight vectors:
- Union of all tickers, `fillna(0)` for missing
- Average, renorm to 100%
- No floor enforcement (by design — stocks absent from MVO were excluded for a reason)

### Smart Hybrid (`SH_DD_ALPHA=0.05`, `SH_DD_HYBRID=0.15`)
Drawdown computed from running hybrid NAV tracked incrementally during the main loop:
- `dd >= -5%`: full Alpha portfolio
- `-15% <= dd < -5%`: blind Hybrid portfolio
- `dd < -15%`: full MVO portfolio

Global constants at top of file — easy to tune:
```python
SH_DD_ALPHA  = 0.05   # below this drawdown: full alpha
SH_DD_HYBRID = 0.15   # below this drawdown: hybrid; above: full MVO
```

### Latest Results (IC=0.03, risk_aversion=10, N=25, rebal=15d)
```
Strategy                    CAGR    Vol   Sharpe    MDD
Baseline (quality)         27.3%  28.0%    0.97   -41.8%
Pure Alpha (conc=2.0x)     83.6%  42.6%    1.96   -45.1%
MVO (IC=0.03, max=8%)      58.8%  33.8%    1.74   -38.7%
Hybrid (Alpha+MVO avg)     71.1%  37.8%    1.88   -40.7%
Smart Hybrid               80.1%  39.8%    2.01   -40.1%  ← best Sharpe
```

### Per-Rebalance Display
Each rebalance date shows the **Smart Hybrid portfolio** with:
- Regime label and current drawdown: `regime=α (dd=-1.2%)`
- Turnover vs previous rebalance: `TO=84%`
- Period contribution stats: `port=+4.6%  avg=+8.2%  med=+5.1%  best=+41%  worst=-18%  std=12%`
- Per-stock: Weight%, AnnAlpha%, AnnVol%, Z-score, period return, Sector, Source (matrix tags or `alpha`)

### Post-Run Displays
1. **Live P&L table** — smart hybrid portfolio from last rebalance to today, with drifted weights and top 3/3 contributors per day
2. **Current live portfolio** — rebalance weights vs today's drifted weights
3. **Hypothetical smart hybrid rebalance** — what portfolio would look like if today were a rebalance date, showing current regime
4. **Portfolio return statistics** — per holding period: mean, median, std, min, max, % positive, quintiles, avg/median turnover, regime prevalence overall and by year
5. **Individual stock return statistics** — pooled across all periods: same stats + quintiles
6. **Two histograms** — portfolio returns (green) and individual stock returns (blue)

### X Snapshot Caching
- Table: `mvo_x_snapshots` in PostgreSQL, keyed by `(date, model_version)`
- `force_rebuild_cache=False` (default): incremental, only builds missing dates
- `force_rebuild_cache=True`: clears cache, rebuilds all snapshots
- Bounded to `MB_START_DATE - 90 days` onwards (~91 month-end snapshots)

### Return Dict Keys
```python
results = {
    'nav_baseline', 'nav_alpha', 'nav_mvo', 'nav_hybrid', 'nav_smart',
...
### Next Session
1. **Transaction costs** — model turnover cost (bps per unit of turnover) and deduct from period returns
2. **Smart hybrid threshold robustness** — grid search over `SH_DD_ALPHA` and `SH_DD_HYBRID` to confirm 5%/15% is a robust region not overfitted to sample
3. **Macro hedges / drawdown control** — regime-conditioned exposure scaling using existing `regime_s` signal
4. **Capacity / liquidity sensitivity** — run backtest across grid of `mktcap_floor` and volume filters, plot Sharpe/CAGR degradation curve
