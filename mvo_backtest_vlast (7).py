...
# turnover trigger.
SECTOR_CAP    = 1.0      # max total weight per sector      (1.0 = off)
SUBSECTOR_CAP = 1.0      # max total weight per sub-sector  (1.0 = off)
...
# After a parole is REVOKED, suppress the FAST-TRACK (CUSUM-latch) parole path for
# this many trading days. Rationale: a revoke leaves the 30d CUSUM down-latch still
# ticking, and fast-track re-grant only needs ONE window with p_low>THRESH -- a low
# bar that flip-flops parole day-to-day. During the cooldown, only the SLOW 5-day
# consensus (a reliable regime read) may re-grant; a fresh fast-track parole must
# then wait for genuinely new evidence. 5d ~ enough for p_low across windows to
# settle to the prevailing regime. Set 0 to disable the cooldown.
...
def _apply_group_caps(w, sectors_s, subsec_map_s=None,
    ...
    maps = []
    if sector_cap < 1.0 and sectors_s is not None:
        maps.append(('sector', sectors_s, sector_cap))
    if subsector_cap < 1.0 and subsec_map_s is not None:
        maps.append(('subsector', subsec_map_s, subsector_cap))
    if not maps:
        if residual:
            ...
            for gname, sub in w.groupby(grp):
                ...
                if tot > cap + 1e-12:
                    ...
                    for t in sub.index:
                        w[t] = w[t] * scale
                    freed += tot - cap
                    log.append({'level': level, 'name': str(gname),
                                'pre': tot, 'post': cap})
...
def run_backtest(
    ...
    def _cap_groups(w_in, s_idx=None, dt_=None):
        ...
        if (sector_cap >= 1.0 and subsector_cap >= 1.0) or w_in is None or len(w_in) == 0:
            return w_in
                        ...
                        if _cl:
                            ...
                            for _d in _clog:
                                ...
                                if _k in _seen:
                                    continue
                                _seen.add(_k)
                                _parts.append(f"{_d['name']}[{_d['level'][:3]}] "
