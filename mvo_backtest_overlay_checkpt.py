...
# FULL BACKTESTING: OVERLAY PORTFOLIO - REVISED
...
if (sector_key == 'ETF'):
    sSectors_df = Sectors_df[Sectors_df.columns[Sectors_df.columns.tolist().index(sector_key): ]].dropna(how = 'all')
    nSectors_df = pd.DataFrame(sSectors_df[sector_key])
    nSectors_df['Sector Key'] = sSectors_df.index.map(lambda x: sSectors_df.loc[x, s_l[0]] + ',' + sSectors_df.loc[x, s_l[1]] + ',' + 
                                                      sSectors_df.loc[x, s_l[2]] + ',' + sSectors_df.loc[x, s_l[3]] + ',' + sSectors_df.loc[x, s_l[4]])
    for c_stock in nSectors_df[sSectors_df[sector_key].isna()].index: 
        if (nSectors_df[nSectors_df['Sector Key'] == nSectors_df.loc[c_stock, 'Sector Key']][sector_key].value_counts().shape[0] != 0):
            nSectors_df.loc[c_stock, sector_key] = nSectors_df[nSectors_df['Sector Key'] == nSectors_df.loc[c_stock, 'Sector Key']][sector_key].value_counts().sort_values().index[-1]        
    sectors_s = nSectors_df[sector_key]
    fSectors_df = Sectors_df.loc[sectors_s[sectors_s.isna()].index]
    for c_stock in fSectors_df.index:
        sectors_s.loc[c_stock] = Sectors_df[Sectors_df[sector_choice] == fSectors_df.loc[fSectors_df.index[0], sector_choice]][sector_key].value_counts().sort_values().index[-1]
else:
    sSectors_df = Sectors_df.drop(['VA Ticker'], axis = 1).dropna(how = 'all')
    VAmissing_df = sSectors_df[sSectors_df[sector_key].isna()]
    for c_miss in VAmissing_df.index:    
        try:
            sSectors_df.loc[c_miss, sector_key] = sSectors_df[sSectors_df['AV Industry'] == VAmissing_df.loc[c_miss, 'AV Industry']][sector_key].value_counts().sort_values().index[-1]
        except:
            try:
                sSectors_df.loc[c_miss, sector_key] = sSectors_df[sSectors_df['AV Sector'] == VAmissing_df.loc[c_miss, 'AV Sector']][sector_key].value_counts().sort_values().index[-1]
            except:
                if (c_miss.split(' ')[1] != 'US'):
                    sSectors_df = sSectors_df.drop(c_miss)
                else:
                    print(c_miss + ' PROBLEM!!!!')  
    sectors_s = sSectors_df[sector_key]    
sectors_s = sectors_s[sectors_s.index.map(lambda x: x.split(' ')[1]) == 'US']    
sectors_s.index = sectors_s.index.map(lambda x: x.split(' ')[0])

Pxs_df = Pxs_df.T[~Pxs_df.T.index.duplicated(keep='first')].T
sectors_s = sectors_s[~sectors_s.index.duplicated(keep='first')]
Pxs_df = Pxs_df.replace({0: np.nan}).fillna(method = 'ffill')


# MACRO FEATURES

MOVE = (Pxs_df['USGG2YR'].ewm(halflife = 42).std() + Pxs_df['USGG5YR'].ewm(halflife = 42).std() + Pxs_df['USGG10YR'].ewm(halflife = 42).std()) / 3
Pxs_df['MOVE'] = (MOVE - MOVE.rolling(42).mean()).values
Pxs_df['USGG2YR'] = Pxs_df['USGG2YR'] - Pxs_df['USGG2YR'].shift(1)
Pxs_df['US10YREAL'] = Pxs_df['USGG10YR'] - Pxs_df['T10YIE']
Pxs_df['US10YREAL'] = Pxs_df['US10YREAL'] - Pxs_df['US10YREAL'].shift(1)
Pxs_df['10Y RATE'] = Pxs_df['USGG10YR'] * 100
Pxs_df['USGG10YR'] = Pxs_df['USGG10YR'] - Pxs_df['USGG10YR'].shift(1)
Pxs_df['US10Y2Y_SPREAD_CHG'] = Pxs_df['USGG10YR'] - Pxs_df['USGG2YR']
Pxs_df['BE5Y5YFWD'] = Pxs_df['T5YIFR'] - Pxs_df['T5YIFR'].shift(1)
Pxs_df['Crude'] = Pxs_df['Crude'].pct_change()
Pxs_df['XAUUSD'] = Pxs_df['XAUUSD'].pct_change()
...
DD_LEVELS = [
    (0.375, 2/5),  # -17.5%: cut 40% of remaining → 60% exposed
    (0.400, 3/7),  # -30.0%: cut 43% of remaining → 34% exposed
    (0.450, 1/2),  # -35.0%: cut 50% of remaining → 17% exposed
    (0.500, 2/3),  # -40.0%: cut 67% of remaining →  6% exposed
    (0.550, 1/1),  # -45.0%: cut 100%              →  0% exposed
]
...
MOM_FILTER    = 2     # alpha/hybrid candidate pool = MOM_FILTER x top_n by momentum
...
def _compute_beta(port_ret_s, inst_ret_s, window):
    common = port_ret_s.dropna().index.intersection(inst_ret_s.dropna().index)
    common = common[-window:] if len(common) >= window else common
    if len(common) < window // 2: return np.nan
    p, h = port_ret_s.reindex(common), inst_ret_s.reindex(common)
    var_h = h.var()
    return p.cov(h) / var_h if var_h > 0 else np.nan

def _compute_corr(port_ret_s, inst_ret_s, window):
    common = port_ret_s.dropna().index.intersection(inst_ret_s.dropna().index)
    common = common[-window:] if len(common) >= window else common
    if len(common) < window // 2: return np.nan
    return port_ret_s.reindex(common).corr(inst_ret_s.reindex(common))

def _get_effectiveness(signal_df, dt, mav_win):
    if 'effectiveness' not in signal_df.columns: return np.nan
    past = signal_df['effectiveness'].loc[:dt].dropna()
    if past.empty: return np.nan
    return past.iloc[-mav_win:].mean()

def _select_hedge_instruments(dt, hedges_l, results, port_ret_s, inst_ret,
                               beta_window, corr_window, eff_mav_win,
                               eff_floor, corr_floor, trigger_assets):
    scores = {}
    for inst in hedges_l:
        if inst not in inst_ret or inst not in results: continue
        ...
        scores[inst] = {'corr': corr, 'eff': eff, 'beta': beta, 'effectiveness': eff}
    ...
    for inst, d in ranked:
        if inst in selected: continue
        if n_from_ranking >= 3: break
        corr = d['corr'] if not np.isnan(d.get('corr', np.nan)) else 0
        eff  = d['eff']  if not np.isnan(d.get('eff',  np.nan)) else 0
        if corr < corr_floor: break
        if eff >= eff_floor:
            selected[inst] = d
            n_from_ranking += 1
    return selected
...
"""
# Normal parameter tweaking — reuses cached cov matrices
results = run_backtest(..., mode='rebuild', rebuild_cov=False)

# After adding new stocks to DB — full rebuild including cov matrices
results = run_backtest(..., mode='rebuild', rebuild_cov=True)
...
results = run_backtest(
    ...
    universe_mult=5, risk_aversion=8, #universe_mult=5, risk_aversion=10 ########################################
    mode='rebuild', # 'rebuild' or 'incremental'
    rebuild_cov=True, ###############################
