...
ICS_CALC_DATES_TBL     = 'factor_ic_calc_dates'   # frozen calc_dates per model_version
...
def _ics_save_ic_cache(ic_results_new, model_version):
    """Save new IC values to cache. Checks per (factor, date) — not per date
    across all factors — so partial cache coverage is filled correctly."""
    if not ic_results_new:
        return
    # Load already-cached (factor, date) pairs
    try:
        with ENGINE.connect() as conn:
            df_ex = pd.read_sql(text(f"""
                SELECT date, factor FROM {ICS_IC_CACHE_TBL}
                WHERE model_version = :mv
            """), conn, params={'mv': model_version})
        df_ex['date'] = pd.to_datetime(df_ex['date'])
        cached_pairs = set(zip(df_ex['factor'], df_ex['date']))
    except Exception:
        cached_pairs = set()
    ...
    for factor, ic_df in ic_results_new.items():
        for dt in ic_df.index:
            if (factor, pd.Timestamp(dt)) in cached_pairs:
                continue   # never overwrite existing (factor, date)
    ...
    if not rows:
        print("  IC cache: no new (factor, date) pairs to save")
        return
...
def _ics_ensure_calc_dates_table():
    """Create factor_ic_calc_dates table if it doesn't exist."""
    with ENGINE.begin() as conn:
        conn.execute(text(f"""
            CREATE TABLE IF NOT EXISTS {ICS_CALC_DATES_TBL} (
                date           DATE        NOT NULL,
                model_version  VARCHAR(5)  NOT NULL,
...
def _ics_load_calc_dates(model_version):
    """Load frozen calc_dates from DB. Returns sorted DatetimeIndex or None."""
    try:
        with ENGINE.connect() as conn:
            df = pd.read_sql(text(f"""
                SELECT date FROM {ICS_CALC_DATES_TBL}
                WHERE model_version = :mv
                ORDER BY date
            """), conn, params={'mv': model_version})
        if df.empty:
...
def _ics_save_calc_dates(calc_dates, model_version):
    """Persist new calc_dates to DB. Never overwrites existing entries."""
    rows = [{'date': pd.Timestamp(d).date(), 'model_version': model_version}
            for d in calc_dates]
    if not rows:
        return
    with ENGINE.begin() as conn:
        conn.execute(text(f"""
            INSERT INTO {ICS_CALC_DATES_TBL} (date, model_version)
            VALUES (:date, :model_version)
            ON CONFLICT (date, model_version) DO NOTHING
        """), rows)
    print(f"  Saved {len(rows)} calc_dates to DB")
...
def compute_rolling_regime_weights(ic_results, ic_annual, Pxs_df,
    ...
    def _weights_at(cutoff_dt, r_target):
        """Similarity × recency weighted IC from all observations before cutoff_dt."""
        prior = [(dt, fn, ic, r_obs) for dt, fn, ic, r_obs in ic_obs
                 if dt < cutoff_dt]
        if not prior:
            return equal_w.copy()
        ...
        for fname in factors:
            num = den = 0.0
            for dt, fn, ic_val, r_obs in prior:
                if fn != fname:
                    continue
                sim = max(0.0, 1.0 - abs(r_target - r_obs))
                if sim == 0.0:
                    continue
                age_yrs = (cutoff_dt - dt).days / 365.25
    ...
    # Canonical factor list — always show all, even if IC data absent
    ALL_FACTORS = ['Mom_12M1', 'Value', 'Quality', 'Idio_Mom', 'OU']
...
def run_ic_study_alpha(Pxs_df, sectors_s,
    ...
    # ── Frozen calc_dates: load from DB, extend only at the end ──────────────
    # Never recompute from all_valid[::rebal_freq] — stride alignment must be
    # invariant to any changes in Pxs_df (new rows, backfilled NaNs, etc.)
    # NOTE: force_recompute applies to IC VALUES only, never to calc_dates.
    _ics_ensure_calc_dates_table()
    frozen_dates = _ics_load_calc_dates(model_version)

    if frozen_dates is None or len(frozen_dates) == 0:
        # First run: compute from scratch and freeze
        calc_dates = pd.DatetimeIndex(all_valid[::rebal_freq])
        _ics_save_calc_dates(calc_dates, model_version)
        print(f"  Calc dates: {len(calc_dates)} computed and frozen "
              f"({calc_dates[0].date()} → {calc_dates[-1].date()})")
    else:
        # Subsequent runs: load frozen dates, append only new dates beyond last
        last_frozen = frozen_dates[-1]
        new_valid   = all_valid[all_valid > last_frozen]
        if len(new_valid) >= rebal_freq:
            # Extend stride from last frozen date
            extension = new_valid[rebal_freq - 1::rebal_freq]
            _ics_save_calc_dates(extension, model_version)
            calc_dates = frozen_dates.append(extension)
        else:
            calc_dates = frozen_dates
        print(f"  Calc dates: {len(calc_dates)} loaded from DB "
              f"({calc_dates[0].date()} → {calc_dates[-1].date()})")
