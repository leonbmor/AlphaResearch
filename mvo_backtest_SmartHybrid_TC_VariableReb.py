...
SH_DD_HYBRID   = 0.15    # below this drawdown: blind hybrid; above: full MVO
...
def _make_params_hash(ic, max_weight, min_weight, zscore_cap, pca_var_threshold,
...
def _mb_plot(nav_baseline, nav_alpha, nav_mvo, regime_s, nav_hybrid=None, nav_smart=None):
...
def _params_hash(ic, max_weight, min_weight, zscore_cap, pca_var_threshold,
    ...
    params = dict(
        ic=ic, maxw=max_weight, minw=min_weight, zc=zscore_cap,
        pca=pca_var_threshold, um=universe_mult, ra=risk_aversion,
        n=top_n, conc=round(conc_factor, 4), pf=round(prefilt_pct, 4),
        mcm=min_cov_matrices, mv=model_version,
    )
    return hashlib.md5(
        json.dumps(params, sort_keys=True).encode()
...
def run_daily_cache_build(
    ...
    """
    ph = _params_hash(ic, max_weight, min_weight, zscore_cap, pca_var_threshold,
                      universe_mult, risk_aversion, top_n, conc_factor,
                      prefilt_pct, min_cov_matrices, model_version)
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    for i, dt in enumerate(calc_dates):
        ...
        # Load from cache if available (skip MVO solve for this date)
    ...
    # Yearly returns
    print(f"\n  Yearly returns:")
    print(f"  {'Year':<6} {'Baseline':>10} {'Pure Alpha':>12} {'MVO':>10} {'Hybrid':>10} {'Smart':>10}")
    print(f"  {'-'*66}")
    ...
    try:
        triggers_df = pd.read_sql(
            f"""SELECT date, implied_turnover, vol_diff, drawdown, live_strategy
                ...
                ORDER BY date""",
            ENGINE,
            params={'ph': params_hash, 'mv': MB_MODEL_VER},
