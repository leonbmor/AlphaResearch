...
**Entry point:** `run(Pxs_df, sectors_s, mav_window=252, threshold=50, use_cached_weights=True, force_recompute=False)`

### Run Modes
```python
# Daily fast path (default) — uses hardcoded GQF/CQF weights, only computes missing anchor dates
summary, annual, scores, gqf_w, cqf_w = run(Pxs_df, sectors_s)

# Occasional weight refresh — re-derives weights from IC analysis, prints copy-paste block
summary, annual, scores, gqf_w, cqf_w = run(Pxs_df, sectors_s, use_cached_weights=False)

# Full rebuild — recomputes all dates from scratch
summary, annual, scores, gqf_w, cqf_w = run(Pxs_df, sectors_s, force_recompute=True)
```

### Incremental Optimisation
`load_snapshots_for_dates()` loads only the anchor snapshots needed for new dates (not the full history). For a daily update with 1 new date, typically loads 1 snapshot instead of all historical ones.

### Weight Update Cadence
`GQF_WEIGHTS` / `CQF_WEIGHTS` are hardcoded constants — update **quarterly** or after fundamentals data refresh by running `use_cached_weights=False` and copying the printed output into the script.
...
**Entry points:**
```python
# Daily incremental (default) — loads cached IC values, only computes new dates
...
# Derive rolling point-in-time factor weights
...
### IC Cache
IC values cached in `factor_ic_cache` DB table `(date, factor, horizon, model_version, ic_value)`. First run computes all ~178 dates for 5 factors × 2 horizons. Subsequent daily runs compute 0–1 new dates — essentially instant.
...
```python
# Main backtest — 8 strategies
results = run_mvo_backtest(
    Pxs_df, sectors_s, weights_by_year, regime_s,
    volumeTrd_df     = volumeTrd_df,
    weights_by_date  = weights_by_date,    # point-in-time factor weights (preferred)
...
### Regime → Portfolio Construction Mapping
`run_mvo_backtest` computes the Smart Hybrid regime inline from `nav_smart` drawdown at each rebalance date. The cache stores `alpha`, `hybrid`, `mvo` weights separately — the dynamic loop selects the appropriate weights based on the actual computed drawdown. No `live_strategy` is cached.
...
DD events (lv1 at -17.5%, lv2+ at -30%+) always coincide with MVO territory — portfolio construction switches to MVO automatically via the regime signal. `DD_LEVEL_REGIME = ['mvo', 'mvo', 'mvo', 'mvo', 'mvo']`.
```
...
### Latest Results (IC=0.03, risk_aversion=10, N=25, AUM=$20M, net of 10bps costs)
```
Strategy                    CAGR    Vol   Sharpe    MDD   CAGR/DD
Baseline (quality)         28.4%  28.0%    1.01  -41.0%    0.69x
Pure Alpha (conc=2.0x)     64.5%  39.6%    1.63  -42.4%    1.52x
MVO (IC=0.03, max=8%)      55.8%  35.5%    1.57  -39.0%    1.43x
Hybrid (Alpha+MVO avg)     60.2%  37.5%    1.61  -40.7%    1.48x
Smart Hybrid               62.3%  38.2%    1.63  -41.2%    1.51x
Dynamic                    73.3%  38.3%    1.91  -40.9%    1.79x
Dynamic + Hedge            76.8%  38.5%    2.00  -40.8%    1.88x
Dyn+Hedge+DD (5 levels)    73.0%  35.6%    2.05  -33.4%    2.19x  ← best CAGR/DD
```

### Yearly Returns
```
Year   Baseline  Pure Alpha     MVO   Hybrid    Smart  Dynamic  Dyn+Hedge  DD Policy   Closing AUM
2019    +51.0%     +62.7%   +58.7%   +60.8%   +63.4%   +57.7%    +56.9%    +56.9%   $31.4M
2020    +75.9%    +127.7%  +107.8%  +117.7%  +129.9%  +150.9%   +162.3%   +148.1%   $77.9M
2021    +15.4%     +32.0%   +15.1%   +23.5%   +20.5%   +34.5%    +34.5%    +31.7%   $102.6M
2022    -31.3%      +0.9%    +4.5%    +2.7%    +1.5%    +0.6%    +10.3%    +14.1%   $117.0M
2023    +35.4%     +41.5%   +40.5%   +41.2%   +42.1%   +32.3%    +32.6%    +29.9%   $151.9M
2024    +64.0%     +95.4%   +79.9%   +87.7%   +93.6%  +114.2%   +111.8%    +97.2%   $299.6M
2025    +25.4%     +79.7%   +73.5%   +76.6%   +76.5%  +122.1%   +124.3%   +117.9%   $652.8M
2026     +6.3%     +56.9%   +47.8%   +52.3%   +56.0%   +66.1%    +68.3%    +68.3%  $1098.8M
```

### DD Policy Event Log (9 de-gross, 7 re-entries, 2019-2026)
```
2020-03-09  DE-GROSS lv1   -22.2%  100%→60%
2020-03-16  DE-GROSS lv2   -32.7%   60%→34%   (COVID crash)
2020-03-30  RE-ENTRY       +20.9%   34%→100%
2021-02-25  DE-GROSS lv1   -17.7%  100%→60%
2021-03-17  RE-ENTRY        +9.7%   60%→100%
2021-12-03  DE-GROSS lv1   -17.6%  100%→60%
2022-01-07  RE-ENTRY        -4.4%   60%→100%  (annual reset)
2023-09-20  DE-GROSS lv1   -18.8%  100%→60%
2023-11-20  RE-ENTRY       +12.7%   60%→100%
2024-08-05  DE-GROSS lv1   -19.1%  100%→60%
2024-08-19  RE-ENTRY       +17.1%   60%→100%
2025-02-25  DE-GROSS lv1   -19.0%  100%→60%
2025-04-04  DE-GROSS lv2   -31.4%   60%→34%   (tariff tantrum)
2025-04-15  RE-ENTRY       +14.4%   34%→100%
2025-11-20  DE-GROSS lv1   -22.0%  100%→60%
2025-11-28  RE-ENTRY       +14.2%   60%→100%
```
...
5. **Momentum penalty × OU factor interaction** — OU factor already tracks mean-reversion in the composite signal. Analyse: (a) do high-penalty stocks already have elevated OU scores? (b) does the penalty add value only when OU is NOT firing? (c) can joint (OU score, penalty excess) conditioning improve classification?
...
mvo_daily_portfolios  Daily alpha/mvo/hybrid weights        Per params_hash
mvo_daily_triggers    Daily implied_to, vol_diff            Per params_hash
...
macro_fitted_params   Monthly refit params per instrument   Per params_hash
```

**Key design change (April 2026):** `mvo_daily_portfolios` no longer stores `smart` weights — only `alpha`, `hybrid`, `mvo`. Regime selection (`live_strategy`) and `drawdown` are computed inline in `run_mvo_backtest` from actual `nav_smart` at each date. `mvo_daily_triggers` no longer stores `drawdown` or `live_strategy` — only `implied_to` and `vol_diff`. This eliminates circular dependency and ensures regime switching uses the correct portfolio NAV.

**Cache rebuild required when:** MVO parameters change (`ic`, `max_weight`, `risk_aversion`, etc.). Run `run_daily_cache_build(force_rebuild=True)`. Regime thresholds (`SH_DD_*`, `DYN_*`) do NOT require cache rebuild — computed inline.
...
### In Progress
1. **MR analysis** — OU scores standalone as independent signal; interaction with momentum penalty; joint (OU, penalty) conditioning for two-stage exclusion

### Next Session
1. **Momentum penalty × OU integration** — two-stage filter: penalise only when `penalty_score > threshold AND ou_score < ou_threshold` (stock above equilibrium + extreme recent return). `calibrate_k` updated with `ou_scores_df` and `ou_thresholds` parameters
2. **Calibrate joint `(k, ou_threshold)`** — run `calibrate_k` with `validation_days=42`, `ou_scores_df` loaded from `v2_ou_reversion_df`
3. **Implement two-stage exclusion in `_cb_build_composite_scores`**
4. **Value cache refresh** — `run_ic_study(use_cached_weights=False)` once to populate `VALUE_WEIGHTS`, then hardcode

### Completed This Session
- [DONE] `weights_by_date` now passed to and consumed by `_cb_build_composite_scores` (was using coarser annual snapshots)
- [DONE] `quality_factor.run()` made fully incremental — skips weight derivation and only computes missing anchor dates
- [DONE] `value_factor.run_ic_study()` made fully incremental — `VALUE_WEIGHTS` / `_VALUE_WEIGHTS_READY` constants for fast daily path
- [DONE] `factor_ic_study.run_ic_study_alpha()` made incremental — IC values cached in `factor_ic_cache` DB table
- [DONE] `mvo_daily_triggers` refactored — removed `drawdown` / `live_strategy` (computed inline in backtest)
- [DONE] `mvo_daily_portfolios` refactored — saves `alpha`/`hybrid`/`mvo` only, no `smart`
- [DONE] `run_mvo_backtest` dynamic loop: regime computed from actual `nav_smart` drawdown, not from cached `live_strategy`
- [DONE] DD policy / Smart Hybrid coordination — DD lv1 (-17.5%) coincides with MVO threshold; all DD events guaranteed in MVO mode
- [DONE] `momentum_penalty.calibrate_k()` updated with OU conditioning: `ou_scores_df` and `ou_thresholds` parameters
- [DONE] Reference doc updated (this session)
...
multi = run_macro_hedge_cached(
    ...
    engine           = ENGINE,
    force_rebuild    = False,
    incremental_only = True,    # default — only computes last date (fast daily update)
)

# Compute all missing dates (e.g. after a gap or parameter change)
multi = run_macro_hedge_cached(..., incremental_only=False)
...
**Run mode flags:**
- `incremental_only=True` (default) — only computes last date of `Pxs_df`; fast daily update (seconds)
- `incremental_only=False` — computes all missing dates since `hard_start`; use after gaps or parameter changes
- `force_rebuild=True` — wipes cache entirely and recomputes from scratch
