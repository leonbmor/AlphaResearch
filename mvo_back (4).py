...
def run_backtest(
    ...
    if HMM_REGIME_OVERLAY:
        ...
        _hmm_hi, _hmm_lo, _hmm_hi1, _hmm_lo1, _hmm_missing = \
            _load_hmm_consensus(pd.DatetimeIndex(_leadin_days(MIN_SP + 10)))
...
"""
# Normal calibration — vary MVO params; reuses all cached upstream data
results = run_backtest(..., mode='rebuild')             # all flags False (default)

# After adding/removing stocks in the universe — rebuild composite+factors+X
results = run_backtest(..., mode='rebuild', universe_changed=True)

# After changing HISTORICAL factor data (e.g. cutover to v2, corrected past values)
results = run_backtest(..., mode='rebuild', factor_model_changed=True)

# After regenerating alpha IC weights / regime / MR (composite only)
results = run_backtest(..., mode='rebuild', weights_changed=True)   # or regime_changed / mr_changed

# Burn absolutely everything and recompute from scratch
results = run_backtest(..., mode='rebuild', universe_changed=True,
                       factor_model_changed=True, weights_changed=True,
                       regime_changed=True, mr_changed=True)

# Daily / intraday incremental update — fast last-day refresh, ignores flags
results = run_backtest(..., mode='incremental')
"""

hedges_l  = ['QQQ', 'SPX', 'ARKK', 'IWM']
hedges_l  = ['QQQ', 'SPX', 'IGV', 'SOXX', 'XLC', 'XLE', 'XLF', 'ARKK', 'IWM']

MB_START_DATE  = pd.Timestamp('2019-01-01')  # ← change this temporarily ###############################################
hmm   = load_etf_hmm(windows=[1,3], engine=engine)
cusum = load_etf_cusum(windows=[1,3], engine=engine)
ss_hmm   = load_etf_hmm(windows=[1,3], model_version='subsec_v1', engine=engine)
ss_cusum = load_etf_cusum(windows=[1,3], model_version='subsec_v1', engine=engine)
...
results = run_backtest(
    ...
    volumeTrd_df         = volumeTrd_df,
    weights_by_date      = weights_by_date,
    ic=0.05, max_weight=0.075, min_weight=0.025, #ic=0.05 #########################################
    zscore_cap=2.5, pca_var_threshold=0.65,  # pca_var_threshold=0.65
    universe_mult=7, risk_aversion=5, #universe_mult=5, risk_aversion=10 ########################################
    mode='incremental', # 'incremental' (fast last-day) | 'rebuild' (flag-controlled)
    # Cache-invalidation flags — only honoured under mode='rebuild' (ignored if
    # incremental). Flip the matching one to True for the run AFTER you change it;
    # leave False to reuse cached data (normal calibration sweep = all False).
    universe_changed     = False,  # ← True after adding/removing stocks
    factor_model_changed = False,  # ← True after changing HISTORICAL factor data (not new dates)
    weights_changed      = False,  # ← True after regenerating alpha IC weights_by_date
    regime_changed       = False,  # ← True after changing the regime series
    mr_changed           = False,  # ← True after changing MR params / scores
    # Hedge layer — omit to run 6 strategies only
    hedge_multi          = multi,
    hedges_l             = hedges_l,
    hedge_trigger_assets = ['QQQ'],
    hmm=hmm, cusum=cusum, etf_map_s=etf_map_s,
    etf_penalty_scale=0.01,              # your ETF scale
    ss_hmm=ss_hmm, ss_cusum=ss_cusum,
    subsec_map_s=subsec_map_s, subsec_idx=subsec_idx,
    subsector_penalty_scale=0.75    
)
