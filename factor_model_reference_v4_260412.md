...
```python
# Main backtest (fixed-schedule rebalancing)
results = run_mvo_backtest(
    ...
    force_rebuild_cache=False,      # X snapshot cache
    portfolio_cache_override=False, # rebalance-date portfolio cache
)
...
Runs **6 portfolios in parallel** and compares performance (all net of `TRADING_COST_BPS=10` per side):
...
SH_DD_ALPHA    = 0.05    # dd >= -5%: full alpha
SH_DD_HYBRID   = 0.15   # dd >= -15%: hybrid; else MVO

# Dynamic rebalancing thresholds
DYN_TO_THRESHOLD   = 0.20    # rebalance if implied turnover > 20%
DYN_VOLDIFF_CAP    = 0.15    # only if new portfolio not > 15% more volatile
DYN_VOLDIFF_DERISK = -0.075  # de-risk: rebalance if new portfolio > 7.5% less volatile
DYN_MIN_HOLD_DAYS  = 5       # minimum calendar days between any rebalances
```

### MVO Solver (`_mb_solve_mvo`)
1. Build 4 cov matrices (Empirical EWMA, Ledoit-Wolf, Factor XFX', PCA) + ensemble
2. Run unconstrained MVO + post-solve floor/cap on each matrix
3. Eligibility: stocks in >= `min_cov_matrices` of the 4 matrices
4. Final solve on ensemble cov, eligible universe only
5. Trim to exactly `top_n` by nlargest weight

**Alpha signal:** `alpha_i = IC × vol_i × z_i` (Grinold-Kahn, daily units)

### Smart Hybrid
Drawdown tracked incrementally from running hybrid NAV. At each rebalance:
- `dd >= -5%`: use full Alpha portfolio
- `-15% <= dd < -5%`: use blind Hybrid portfolio
- `dd < -15%`: use full MVO portfolio

### Dynamic Strategy
Uses daily cached trigger variables from `mvo_daily_triggers`. Rebalances when (subject to `DYN_MIN_HOLD_DAYS`):
1. **Regime switch** — drawdown crosses a Smart Hybrid threshold
2. **Turnover** — implied daily TO > 20% AND vol_diff < +15%
3. **De-risk** — vol_diff < -7.5% (new portfolio materially less risky)

Rebalance log shows trigger type (`[REGIME SWITCH]` / `[TURNOVER]` / `[DE-RISK]` / `[INIT]`), forward-looking period stats, and per-stock diagnostics. Trigger summary at end shows breakdown by type.

### Latest Results (IC=0.03, risk_aversion=10, N=25, rebal=15d, net of 10bps costs)
```
Strategy                    CAGR    Vol   Sharpe    MDD   CAGR/DD
Baseline (quality)         26.8%  28.0%    0.96  -42.0%    0.64x
Pure Alpha (conc=2.0x)     82.6%  42.6%    1.94  -45.2%    1.83x
MVO (IC=0.03, max=8%)      56.8%  33.7%    1.68  -38.9%    1.46x
Hybrid (Alpha+MVO avg)     69.6%  37.8%    1.84  -41.1%    1.70x
Smart Hybrid               79.2%  39.8%    1.99  -40.0%    1.98x
Dynamic                    75.1%  40.0%    1.88  -37.8%    1.99x  <- best MDD & CAGR/DD
```

### Dynamic Rebalancing Stats (150 rebalances, 2019-2026)
```
Avg holding period : 17.3 days  |  Median: 12.0d  |  Min: 5d  |  Max: 97d
Trigger breakdown  : Regime switch 69%  |  Turnover 25%  |  De-risk 5%  |  Init 1%
```

### Smart Hybrid Regime Prevalence
```
2019: 76% alpha, 24% hybrid,   0% MVO
2020: 83% alpha,  4% hybrid,  12% MVO
2021: 48% alpha, 20% hybrid,  32% MVO
2022:  0% alpha,  0% hybrid, 100% MVO  <- full defensive year
2023: 33% alpha, 29% hybrid,  38% MVO
2024: 88% alpha, 12% hybrid,   0% MVO
2025: 58% alpha, 21% hybrid,  21% MVO
2026:100% alpha,  0% hybrid,   0% MVO
```

### Per-Rebalance Display (Smart Hybrid)
- Header: `regime=alpha (dd=-1.2%)  n=25  eff_N=23.0  TO=43%`
- Period stats (forward-looking): `port=+4.6%  avg=+8.2%  med=+5.1%  best=+41%  worst=-18%  std=12%`
- Per-stock: Weight%, AnnAlpha%, AnnVol%, Z-score, Contrib%, Sector, Source (matrix tags or `alpha`)

### Post-Run Displays
1. **Trading costs summary** — per year per strategy, total and annualized
2. **Dynamic rebalancing log** — per rebalance: trigger type, period stats, full portfolio with diagnostics
3. **Dynamic trigger summary** — count and % by trigger type, yearly rebalance frequency
4. **Live P&L table** — smart hybrid from last rebalance to today, drifted weights, top 3/3 contributors
5. **Current live portfolio** — rebalance weights vs today's drifted weights
6. **Hypothetical smart hybrid rebalance** — today's portfolio if we rebalanced now, with current regime
7. **Portfolio return statistics** — per holding period: mean, median, std, quintiles, turnover, regime prevalence by year
8. **Individual stock return statistics** — pooled across all periods, same stats + quintiles
9. **Two histograms** — portfolio returns (green) and individual stock returns (blue)

### Caching Architecture
Three cache layers, all keyed by `params_hash` (12-char MD5 of all construction parameters):
...
| `mvo_daily_triggers` | Daily implied_turnover, vol_diff, drawdown, live_strategy | Per params_hash |

- `force_rebuild_cache=False` / `portfolio_cache_override=False` (defaults): load cache, compute only missing dates
- `force_rebuild_cache=True` / `portfolio_cache_override=True`: clear and recompute all
- `run_daily_cache_build(..., force_rebuild=True)`: rebuild daily portfolios + triggers (~15 min, 1,899 dates)

### Trigger Variable Statistics (1,899 days, 2019-2026)
```
                  implied_turnover  vol_diff   drawdown
mean               9.4%             ~0.0%      -9.6%
median             7.9%              0.0%      -5.9%
std                7.5%              3.7%       9.7%
Q1                 5.1%             -0.6%     -16.9%
...
max              100.0%            +56.1%       0.0%
```
Vol_diff is extraordinarily tight (std=3.7%) — used as a contrarian filter (block rebalance if new portfolio is materially more volatile). Negative vol_diff when MVO regime active confirms MVO reduces portfolio vol as intended.

### Return Dict Keys
```python
results = {
    'nav_baseline', 'nav_alpha', 'nav_mvo', 'nav_hybrid', 'nav_smart', 'nav_dynamic',
    'port_baseline', 'port_alpha', 'port_mvo', 'port_hybrid',
    'alpha_weights_by_date', 'mvo_weights_by_date',
    'hybrid_weights_by_date', 'smart_hybrid_weights_by_date', 'dyn_weights_by_date',
    'composite_by_date', 'regime_s', 'weights_by_year',
    'daily_triggers',  # pd.DataFrame from mvo_daily_triggers
}
```

---

## 18. SCRIPT: `short_interest_updater.py`
...
### Next Session
1. **Smart hybrid threshold robustness** — grid search over `SH_DD_ALPHA` / `SH_DD_HYBRID` and dynamic trigger parameters to confirm current values aren't overfitted to sample
2. **Macro hedges** — regime-conditioned factor exposure hedges using X matrix macro betas
3. **Drawdown policy** — hard HWM-based boundary applied after all other overlays
4. **Capacity / liquidity sensitivity** — grid of `mktcap_floor` and volume filters, plot Sharpe/CAGR degradation
