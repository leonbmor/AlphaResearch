...
# momentum_exclusions is run before mvo_backtest in the pipeline —
# load_exclusions is available in the kernel namespace directly.
...
# ── MR exclusion filter (strategy 9: Dyn + Hedge + DD + Excl) ────────────────
# Fraction of liquidity-filtered candidates that can be excluded per rebalance.
# MR_CAP=0.0 → no exclusions → strategy 9 converges exactly to strategy 8.
# MR_CAP=0.05 → top 5% worst joint-score offenders excluded.
MR_CAP = 0.05
...
def run_daily_cache_build(
    ...
    model_version='v2',
    force_rebuild=False,
    pct_df=None,           # from momentum_exclusions.run_pct_analysis()
    ou_scores_df=None,     # from momentum_exclusions.build_ou_scores_df()
):
    """
    ...
    for idx, dt in enumerate(to_compute):
        ...
        # -- Save --------------------------------------------------------------
        # Standard portfolios saved unconditionally first
        try:
        ...
        # Exclusion-filtered portfolios — isolated block, failures never affect standard save
        try:
            # top_n = MR_CAP * number of liquidity-filtered candidates
            _mr_top_n = int(MR_CAP * len(cands)) if MR_CAP > 0 else 0
            excl_dt   = _load_momentum_exclusions(dt, top_n=_mr_top_n) if _mr_top_n > 0 else {}
            if excl_dt:
                cands_excl   = cands[~cands.index.isin(excl_dt.keys())]
                n_excl_avail = len(cands_excl)
                if n_excl_avail >= top_n:
                    # Alpha excl
                    ranked_excl     = cands_excl.sort_values(ascending=False)
                    alpha_excl_port = [t for t in ranked_excl.head(top_n).index
                                       if t in pxs_cols]
                    n_p_e = len(alpha_excl_port)
                    if conc_factor == 1.0 or n_p_e < 2:
                        w_alpha_excl = pd.Series(1.0 / n_p_e, index=alpha_excl_port)
                    else:
                        n_top_e = int(np.ceil(n_p_e / 2))
                        n_bot_e = n_p_e - n_top_e
                        top_a   = conc_factor / (conc_factor + 1.0)
                        bot_a   = 1.0 / (conc_factor + 1.0)
                        w_alpha_excl = pd.Series({
                            t: (top_a / n_top_e if j < n_top_e else bot_a / n_bot_e)
                            for j, t in enumerate(alpha_excl_port)
                        })
                    # MVO excl
                    try:
                        with _SuppressOutput():
                            w_mvo_excl, _ = _mb_solve_mvo(
                                dt=dt, candidates=cands_excl.index.tolist()[:n_cands],
                                composite_scores=comp_scores, Pxs_df=Pxs_df,
                                sectors_s=sectors_s, volumeTrd_df=volumeTrd_df,
                                model_version=model_version,
                                pca_var_threshold=pca_var_threshold,
                        ...
                        w_mvo_excl_nz = w_mvo_excl[w_mvo_excl > 1e-6]
                        # Hybrid excl
                        all_t_e    = list(set(w_alpha_excl.index) | set(w_mvo_excl_nz.index))
                        w_hyb_excl = (w_alpha_excl.reindex(all_t_e).fillna(0) +
                                      w_mvo_excl_nz.reindex(all_t_e).fillna(0)) / 2.0
                        w_hyb_excl = w_hyb_excl[w_hyb_excl > 0]
                        if w_hyb_excl.sum() > 0:
                            w_hyb_excl = w_hyb_excl / w_hyb_excl.sum()
                    except Exception:
                        w_mvo_excl_nz = w_mvo_nz.copy()
                        w_hyb_excl    = w_hybrid.copy()
                else:
                    w_alpha_excl  = w_alpha.copy()
                    w_mvo_excl_nz = w_mvo_nz.copy()
                    w_hyb_excl    = w_hybrid.copy()
            else:
                w_alpha_excl  = w_alpha.copy()
            ...
            _save_portfolios(dt, model_version, ph, {
                'alpha_excl' : w_alpha_excl,
                'mvo_excl'   : w_mvo_excl_nz,
                'hybrid_excl': w_hyb_excl,
            })
        except Exception as e:
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    for i, dt in enumerate(calc_dates):
        ...
        if not w_mvo.empty and w_mvo.sum() > 0:
            ...
            if _advp_capped.get(dt):
                print(f"  *** ADVP cap active (AUM=${AUM*_running_nav/1e6:.1f}M, "
    ...
    # ── Strategy 9: Dynamic + Exclusions ─────────────────────────────────────
    # Parallel dynamic loop using exclusion-filtered portfolios from cache.
    # Same trigger logic as Dynamic but reads alpha_excl/hybrid_excl/mvo_excl.
    dyn_excl_weights_by_date = {}
    _cost_by_date["dynamic_excl"] = {}
    _dyn_excl_rebal_log = []
    try:
        triggers_excl = load_daily_portfolios(params_hash, MB_MODEL_VER)
        # Check if excl portfolios exist in cache
        _has_excl = any(
            'alpha_excl' in v for v in triggers_excl.values()
        ) if triggers_excl else False

        if _has_excl and not trig_df.empty:
            w_excl_live         = pd.Series(dtype=float)
            w_excl_live_prev_dt = None
            last_rebal_excl_dt  = None
            deployed_regime_excl = None
            dyn_excl_hwm        = None

            for dt in sorted(trig_df.index):
                if dt not in triggers_excl:
                    continue
                row    = trig_df.loc[dt]
                vd_val = float(row['vol_diff'])

                # Regime from nav_smart (same as Dynamic)
                if dyn_excl_hwm is None:
                    dyn_excl_hwm = float(nav_smart.iloc[0]) if nav_smart is not None and not nav_smart.empty else 1.0
                if nav_smart is not None and dt in nav_smart.index:
                    _nav_v       = float(nav_smart.loc[dt])
                    dyn_excl_hwm = max(dyn_excl_hwm, _nav_v)
                    dd_excl      = _nav_v / dyn_excl_hwm - 1
                else:
                    dd_excl = 0.0

                if dd_excl >= -SH_DD_ALPHA:
                    regime_excl = 'alpha'
                elif dd_excl >= -SH_DD_HYBRID:
                    regime_excl = 'hybrid'
                else:
                    regime_excl = 'mvo'

                # Use exclusion-filtered weights
                w_new_excl = triggers_excl[dt].get(
                    f'{regime_excl}_excl', pd.Series(dtype=float))
                if w_new_excl.empty:
                    w_new_excl = triggers_excl[dt].get(
                        regime_excl, pd.Series(dtype=float))
                if w_new_excl.empty:
                    continue

                days_held_excl = (dt - last_rebal_excl_dt).days if last_rebal_excl_dt else 999

                # Drift deployed portfolio
                if not w_excl_live.empty and w_excl_live_prev_dt is not None:
                    tks_d = [t for t in w_excl_live.index if t in Pxs_df.columns]
                    if tks_d and w_excl_live_prev_dt in Pxs_df.index and dt in Pxs_df.index:
                        px_p = Pxs_df.loc[w_excl_live_prev_dt, tks_d]
                        px_c = Pxs_df.loc[dt, tks_d]
                        w_dr = w_excl_live.reindex(tks_d) * (px_c / px_p).fillna(1)
                        if w_dr.sum() > 0:
                            w_excl_live = w_dr / w_dr.sum()
                w_excl_live_prev_dt = dt

                # TO vs drifted
                if w_excl_live.empty:
                    to_excl = 1.0
                else:
                    all_t_e = list(set(w_new_excl.index) | set(w_excl_live.index))
                    to_excl = (w_new_excl.reindex(all_t_e).fillna(0) -
                               w_excl_live.reindex(all_t_e).fillna(0)).abs().sum() / 2

                # Triggers — same logic as Dynamic
                _dd_forced_excl   = _dd_forced_regimes.get(dt)
                _dd_excl_override = (_dd_forced_excl is not None and
                                     _dd_forced_excl != deployed_regime_excl)
                if _dd_excl_override:
                    _regime_order = {'alpha': 0, 'hybrid': 1, 'mvo': 2}
                    regime_excl = (_dd_forced_excl
                                   if _regime_order.get(_dd_forced_excl, 0) >
                                      _regime_order.get(regime_excl, 0)
                                   else regime_excl)
                    w_new_excl = triggers_excl[dt].get(
                        f'{regime_excl}_excl',
                        triggers_excl[dt].get(regime_excl, w_new_excl))

                _to_thr_e = (DYN_TO_THRESHOLD_MVO    if deployed_regime_excl == 'mvo'    else
                             DYN_TO_THRESHOLD_HYBRID if deployed_regime_excl == 'hybrid' else
                             DYN_TO_THRESHOLD_ALPHA)
                regime_switch_excl = (deployed_regime_excl is not None and
                                      regime_excl != deployed_regime_excl and
                                      days_held_excl >= DYN_MIN_HOLD_DAYS)
                to_trigger_excl    = (to_excl > _to_thr_e and
                                      vd_val < DYN_VOLDIFF_CAP and
                                      days_held_excl >= DYN_MIN_HOLD_DAYS)
                should_rebal_excl  = (w_excl_live.empty or regime_switch_excl or
                                      to_trigger_excl or _dd_excl_override)

                if should_rebal_excl:
                    if not w_excl_live.empty:
                        all_t_ec = list(set(w_new_excl.index) | set(w_excl_live.index))
                        to_ec    = (w_new_excl.reindex(all_t_ec).fillna(0) -
                                    w_excl_live.reindex(all_t_ec).fillna(0)).abs().sum() / 2
                        _cost_by_date["dynamic_excl"][dt] = to_ec * TRADING_COST_BPS / 10000
                    if w_excl_live.empty:
                        trig_lbl_e = 'init'
                    elif _dd_excl_override:
                        trig_lbl_e = f'DD-regime->{regime_excl}'
                    elif regime_switch_excl:
                        trig_lbl_e = f'regime->{regime_excl}'
                    else:
                        trig_lbl_e = f'TO={to_excl*100:.1f}%'
                    _dyn_excl_rebal_log.append({
                        'dt': dt, 'trigger': trig_lbl_e,
                        'regime': regime_excl, 'w': w_new_excl.copy(),
                        'to_dyn': to_excl, 'days_held': days_held_excl,
                        'eff_n': 1.0 / (w_new_excl**2).sum() if len(w_new_excl) > 0 else 0,
                    })
                    w_excl_live          = w_new_excl.copy()
                    w_excl_live_prev_dt  = dt
                    last_rebal_excl_dt   = dt
                    deployed_regime_excl = regime_excl

                dyn_excl_weights_by_date[dt] = w_excl_live

            n_excl_rebal = len(_cost_by_date['dynamic_excl'])
            print(f"\n  Strategy 9 (Dyn+Excl): {len(dyn_excl_weights_by_date)} daily weights, "
                  f"{n_excl_rebal} rebalances  "
                  f"(avg {len(dyn_excl_weights_by_date)/max(n_excl_rebal,1):.1f} days/rebal)")

            # -- Rebalance log display (shown instead of Dynamic for audit) ----
            print(f"\n  {'='*95}")
            print(f"  STRATEGY 9 (DYN+EXCL) REBALANCING LOG")
            print(f"  {'='*95}")
            excl_rebal_dates_list = [r['dt'] for r in _dyn_excl_rebal_log]
            for ri, rec in enumerate(_dyn_excl_rebal_log):
                dt_r  = rec['dt']
                w_r   = rec['w']
                next_dt = (excl_rebal_dates_list[ri+1]
                           if ri+1 < len(excl_rebal_dates_list)
                           else Pxs_df.index[-1])
                contrib_str = ''
                if dt_r in Pxs_df.index and next_dt in Pxs_df.index:
                    tks_r   = [t for t in w_r.index if t in Pxs_df.columns]
                ...
                print(f"\n  -- {dt_r.date()}  [rebal #{ri+1}]  "
                      ...
                      f"n={len(w_r)}  eff_N={rec['eff_n']:.1f}  "
                      f"held={rec['days_held']}d  TO={rec['to_dyn']*100:.1f}% --")
                # Show excluded stocks at this rebalance date
                excl_at_rebal = _load_momentum_exclusions(dt_r, top_n=int(MR_CAP * len(universe)) if MR_CAP > 0 else 0)
                if excl_at_rebal:
                    print(f"  *** Excluded ({len(excl_at_rebal)}): "
                          f"{', '.join(sorted(excl_at_rebal.keys()))}")
                if contrib_str:
                    print(f"  period [{dt_r.date()} -> {next_dt.date()}]{contrib_str}")
                print(f"  {'Ticker':<8}  {'Weight%':>7}  {'Sector':<28}")
                print(f"  {'-'*50}")
                for tkr, wt in w_r.sort_values(ascending=False).items():
                    sec = sectors_s.get(tkr, '')
                    excl_flag = ' [EXCL candidate]' if tkr in _load_momentum_exclusions(dt_r, top_n=int(MR_CAP * len(universe)) if MR_CAP > 0 else 0) else ''
                    print(f"  {tkr:<8}  {wt*100:>6.2f}%  {sec:<28}{excl_flag}")
        else:
            if not _has_excl:
                print("\n  Strategy 9: excl portfolios not in cache — "
                      "run run_daily_cache_build() with momentum_exclusions loaded")
    except Exception as e:
        print(f"  Strategy 9 failed: {e}")
        traceback.print_exc()

    nav_dynamic_excl = (_mb_run_nav(
        dyn_excl_weights_by_date,
        sorted(dyn_excl_weights_by_date.keys()),
        Pxs_df, cost_by_date=_cost_by_date["dynamic_excl"])
        if dyn_excl_weights_by_date else pd.Series(dtype=float))
    ...
    # -- Strategy 9 DD overlay ------------------------------------------------
    # Apply same DD policy to nav_dynamic_excl (hedged excl NAV)
    nav_excl_hedged = None
    nav_dd_excl     = None
    _dd_excl_log    = []

    # First apply hedge layer to dyn_excl
    if hedge_results is not None and dyn_excl_weights_by_date:
        try:
            hedge_results_excl = run_hedge_backtest(
                Pxs_df            = Pxs_df,
                multi             = hedge_multi,
                portfolio_weights = dyn_excl_weights_by_date,
                rebal_dates       = [r['dt'] for r in _dyn_excl_rebal_log],
                hedges_l          = hedges_l,
                trigger_assets    = hedge_trigger_assets or ['QQQ'],
            )
            if nav_dynamic_excl is not None and not nav_dynamic_excl.empty:
                nav_excl_hedged = nav_dynamic_excl.copy()
                sweep_by_date_excl = hedge_results_excl['hedge_sweep_by_date']
                cum_hedge_excl = 0.0
                for dt in nav_excl_hedged.index:
                    if dt in sweep_by_date_excl:
                        cum_hedge_excl += sweep_by_date_excl[dt]
                    nav_excl_hedged[dt] = nav_dynamic_excl[dt] * (1 + cum_hedge_excl)
                nav_excl_hedged.name = 'nav_excl_hedged'
        except Exception as _e:
            print(f"  Strategy 9 hedge layer failed: {_e}")
            nav_excl_hedged = nav_dynamic_excl.copy() if nav_dynamic_excl is not None else None
    else:
        nav_excl_hedged = nav_dynamic_excl.copy() if nav_dynamic_excl is not None else None

    # Now apply DD policy to nav_excl_hedged
    if nav_excl_hedged is not None and not nav_excl_hedged.empty:
        base_ret_excl  = nav_excl_hedged.pct_change().fillna(0)
        mvo_ret_excl   = nav_mvo.pct_change().fillna(0) if nav_mvo is not None else base_ret_excl

        dd_nav_excl      = pd.Series(index=nav_excl_hedged.index, dtype=float)
        dd_nav_excl.iloc[0] = nav_excl_hedged.iloc[0]
        exposure_excl_s  = pd.Series(index=nav_excl_hedged.index, dtype=float)
        exposure_excl_s.iloc[0] = 1.0

        hwm_excl       = nav_excl_hedged.iloc[0]
        exposure_excl  = 1.0
        trough_excl    = nav_excl_hedged.iloc[0]
        theo_nav_excl  = nav_excl_hedged.iloc[0]
        dd_active_excl = False
        confirm_excl   = 0
        next_lvl_excl  = 0

        for i, dt in enumerate(nav_excl_hedged.index[1:], 1):
            prev_nav_excl = dd_nav_excl.iloc[i-1]
            if dd_active_excl:
                theo_nav_excl = theo_nav_excl * (1 + mvo_ret_excl.get(dt, 0))
            raw_r_excl      = base_ret_excl.get(dt, 0)
            curr_nav_excl   = prev_nav_excl * (1 + raw_r_excl * exposure_excl)
            dd_nav_excl.iloc[i]     = curr_nav_excl
            exposure_excl_s.iloc[i] = exposure_excl
            if not dd_active_excl:
                hwm_excl = max(hwm_excl, curr_nav_excl)
            dd_curr_excl = curr_nav_excl / hwm_excl - 1

            # De-gross
            if next_lvl_excl < len(DD_LEVELS):
                thr_e, cut_e = DD_LEVELS[next_lvl_excl]
                if dd_curr_excl <= -thr_e:
                    old_exp_e     = exposure_excl
                    exposure_excl = exposure_excl * (1 - cut_e)
                    trough_excl   = curr_nav_excl
                    theo_nav_excl = curr_nav_excl
                    confirm_excl  = 0
                    next_lvl_excl += 1
                    dd_active_excl = True
                    port_cost_e   = (old_exp_e - exposure_excl) * TRADING_COST_BPS / 10000
                    curr_nav_excl = curr_nav_excl * (1 - port_cost_e)
                    dd_nav_excl.iloc[i]     = curr_nav_excl
                    exposure_excl_s.iloc[i] = exposure_excl
                    _dd_excl_log.append({
                        'date': dt, 'event': f'DE-GROSS lv{next_lvl_excl}',
                        'dd': dd_curr_excl,
                        'exposure_from': old_exp_e, 'exposure_to': exposure_excl,
                    })

            # Re-entry
            if dd_active_excl and exposure_excl < 1.0:
                trough_excl  = min(trough_excl, curr_nav_excl)
                theo_rec_e   = theo_nav_excl / trough_excl - 1
                yr_start_e   = pd.Timestamp(f'{dt.year}-01-01')
                ytd_dd_e     = curr_nav_excl / dd_nav_excl[dd_nav_excl.index >= yr_start_e].iloc[0] - 1 \
                               if not dd_nav_excl[dd_nav_excl.index >= yr_start_e].empty else 0.0
                jan_reset_e  = (dt.month == 1 and dt.day <= 10 and ytd_dd_e >= -DD_ANNUAL_RESET_PCT)
                if theo_rec_e >= DD_REENTRY_PCT:
                    confirm_excl += 1
                else:
                    confirm_excl  = 0
                if confirm_excl >= DD_REENTRY_CONFIRM or jan_reset_e:
                    old_exp_e     = exposure_excl
                    exposure_excl = 1.0
                    dd_active_excl = False
                    next_lvl_excl  = 0
                    hwm_excl       = curr_nav_excl
                    confirm_excl   = 0
                    port_cost_e    = old_exp_e * TRADING_COST_BPS / 10000
                    curr_nav_excl  = curr_nav_excl * (1 - port_cost_e)
                    dd_nav_excl.iloc[i]     = curr_nav_excl
                    exposure_excl_s.iloc[i] = exposure_excl
                    trigger_e = 'recovery' if not jan_reset_e else 'annual reset'
                    _dd_excl_log.append({
                        'date': dt, 'event': 'RE-ENTRY 100%',
                        'theo_recovery': theo_rec_e,
                        'exposure_from': old_exp_e, 'exposure_to': 1.0,
                        'trigger': trigger_e,
                    })

        nav_dd_excl = dd_nav_excl
        nav_dd_excl.name = 'nav_dd_excl'

    # Update nav_dynamic_excl to the full stack (hedge + DD)
    if nav_dd_excl is not None and not nav_dd_excl.empty:
        nav_dynamic_excl = nav_dd_excl
    print(f"\n  {'='*72}")
    print(f"  COMPARISON")
    print(f"  {'='*72}")
    ...
    for nav_s, lbl in [
        ...
        (nav_dynamic_excl, 'Dyn+Hedge+DD+Excl'),
    ]:
    ...
    _aum_nav = (nav_dynamic_excl if nav_dynamic_excl is not None and not nav_dynamic_excl.empty else
    ...
    print(f"  {'Year':<6} {'Baseline':>10} {'Pure Alpha':>11} {'MVO':>9} {'Hybrid':>9} {'Smart':>9} {'Dynamic':>9} {'Dyn+Hdg':>9} {'DD Pol':>8} {'Excl':>8} {'AUM($M)':>9}")
    print(f"  {'-'*113}")
    ...
    for yr in all_years:
        ...
        dyn_str  = f"{yr_ret(nav_dynamic):>+8.2f}%" if (nav_dynamic is not None and not nav_dynamic.empty) else f"{'n/a':>9}"
        hdg_str  = f"{yr_ret(nav_dyn_hedged):>+7.2f}%" if (nav_dyn_hedged is not None and not nav_dyn_hedged.empty) else f"{'n/a':>8}"
        dd_str   = f"{yr_ret(nav_dd):>+7.2f}%" if (nav_dd is not None and not nav_dd.empty) else f"{'n/a':>8}"
        excl_str = f"{yr_ret(nav_dynamic_excl):>+7.2f}%" if (nav_dynamic_excl is not None and not nav_dynamic_excl.empty) else f"{'n/a':>8}"
        aum_str  = f"${closing_aum/1e6:>7.1f}M"
        print(f"  {yr:<6} {yr_ret(nav_baseline):>+9.2f}%  "
              f"{yr_ret(nav_alpha):>+9.2f}%  "
              f"{yr_ret(nav_mvo):>+8.2f}%  "
              f"{yr_ret(nav_hybrid):>+8.2f}%  "
              f"{yr_ret(nav_smart):>+8.2f}%  "
              f"{dyn_str}  {hdg_str}  {dd_str}  {excl_str}  {aum_str}")
    ...
    # -- Strategy 9 live P&L and current portfolio ----------------------------
    _mr_top_n_today = int(MR_CAP * len(universe)) if MR_CAP > 0 else 0
    if dyn_excl_weights_by_date:
        print("\n  " + "=" * 72)
        print("  LIVE DYN+EXCL PORTFOLIO -- P&L SINCE LAST REBALANCE")
        print("  " + "=" * 72)
        excl_rebal_only = {dt: w for dt, w in dyn_excl_weights_by_date.items()
                           if dt in _cost_by_date.get('dynamic_excl', {})}
        if dyn_excl_weights_by_date:
            first_excl_dt = sorted(dyn_excl_weights_by_date.keys())[0]
            excl_rebal_only[first_excl_dt] = dyn_excl_weights_by_date[first_excl_dt]
        _live_pnl_hybrid(excl_rebal_only, Pxs_df)

        # Current live excl portfolio
        print("\n  " + "=" * 72)
        print("  CURRENT LIVE DYN+EXCL PORTFOLIO (as of last rebalance)")
        print("  " + "=" * 72)
        past_rebals_excl = sorted([d for d in excl_rebal_only if d <= today_ts])
        if past_rebals_excl:
            live_dt_excl = past_rebals_excl[-1]
            w_live_excl  = dyn_excl_weights_by_date[live_dt_excl]
            w_live_excl  = w_live_excl[w_live_excl > 1e-6].sort_values(ascending=False)
            tickers_excl = [t for t in w_live_excl.index if t in Pxs_df.columns]
            if live_dt_excl == today_ts:
                w_drift_excl = w_live_excl.reindex(tickers_excl) if tickers_excl else w_live_excl
            elif tickers_excl and live_dt_excl in Pxs_df.index:
                px_rebal_excl = Pxs_df.loc[live_dt_excl, tickers_excl]
                px_today_excl = Pxs_df.loc[today_ts, tickers_excl]
                w_drift_excl  = w_live_excl.reindex(tickers_excl) * (px_today_excl / px_rebal_excl).fillna(1)
                if w_drift_excl.sum() > 0:
                    w_drift_excl = w_drift_excl / w_drift_excl.sum()
                else:
                    w_drift_excl = w_live_excl.reindex(tickers_excl)
            else:
                w_drift_excl = w_live_excl.reindex(tickers_excl) if tickers_excl else w_live_excl

            last_excl_rec = next((r for r in reversed(_dyn_excl_rebal_log)
                                  if r['dt'] == live_dt_excl), None)
            trigger_excl_info = (f"  trigger={last_excl_rec['trigger']}  [REGIME]"
                                 if last_excl_rec else "")

            # Show exclusions active today
            excl_today = _load_momentum_exclusions(today_ts, top_n=_mr_top_n_today)

            print(f"\n  Rebalance: {live_dt_excl.date()}  |  As of: {today_ts.date()}"
                  f"  |  {len(w_live_excl)} positions{trigger_excl_info}")
            if excl_today:
                print(f"  *** Currently excluded ({len(excl_today)}): "
                      f"{', '.join(sorted(excl_today.keys()))}")
            print(f"  {'Ticker':<8}  {'Rebal Wt%':>10}  {'Curr Wt%':>10}  {'Drift':>8}  Sector")
            print("  " + "-" * 60)
            for tkr in w_live_excl.index:
                rw  = w_live_excl.get(tkr, 0.0) * 100
                cw  = w_drift_excl.get(tkr, 0.0) * 100 if tkr in tickers_excl else rw
                sec = sectors_s.get(tkr, '')
                print(f"  {tkr:<8}  {rw:>9.2f}%  {cw:>9.2f}%  {cw-rw:>+7.2f}%  {sec}")
    ...
    try:
        ...
        # Dynamic: if rebalanced today, use previous rebal as "old"; else use latest
        ...
        # Excl strategy: get deployed weights
        past_excl_ts = sorted([d for d in dyn_excl_weights_by_date if d <= today_ts])
        w_excl_deployed = (dyn_excl_weights_by_date[past_excl_ts[-1]]
                           if past_excl_ts else pd.Series(dtype=float))
        dyn_excl_rebal_today = today_ts in set([r['dt'] for r in _dyn_excl_rebal_log])

        if dyn_excl_rebal_today and len(past_excl_ts) >= 2:
            w_excl_new = w_excl_deployed
            prev_excl_recs = [r for r in _dyn_excl_rebal_log if r['dt'] < today_ts]
            if prev_excl_recs:
                prev_excl_dt  = prev_excl_recs[-1]['dt']
                w_prev_excl   = prev_excl_recs[-1]['w']
            else:
                prev_excl_dt = past_excl_ts[-2]
                w_prev_excl  = dyn_excl_weights_by_date[prev_excl_dt]
            tks_pe = [t for t in w_prev_excl.index if t in Pxs_df.columns]
            if tks_pe and prev_excl_dt in Pxs_df.index and today_ts in Pxs_df.index:
                px_pe  = Pxs_df.loc[prev_excl_dt, tks_pe]
                px_now = Pxs_df.loc[today_ts, tks_pe]
                w_de   = w_prev_excl.reindex(tks_pe) * (px_now / px_pe).fillna(1)
                w_excl_old = (w_de / w_de.sum()) if w_de.sum() > 0 else w_prev_excl
            else:
                w_excl_old = w_prev_excl
        else:
            last_excl_recs = [r for r in _dyn_excl_rebal_log if r['dt'] <= today_ts]
            if last_excl_recs:
                last_excl_dt = last_excl_recs[-1]['dt']
                w_le = last_excl_recs[-1]['w']
                tks_le = [t for t in w_le.index if t in Pxs_df.columns]
                if tks_le and last_excl_dt in Pxs_df.index and today_ts in Pxs_df.index:
                    px_le  = Pxs_df.loc[last_excl_dt, tks_le]
                    px_now = Pxs_df.loc[today_ts, tks_le]
                    w_de   = w_le.reindex(tks_le) * (px_now / px_le).fillna(1)
                    w_excl_old = (w_de / w_de.sum()) if w_de.sum() > 0 else w_le
                else:
                    w_excl_old = w_le
            else:
                w_excl_old = w_excl_deployed
            try:
                # Hypothetical: use excl-filtered version of today's smart hybrid
                excl_today_set = _load_momentum_exclusions(today_ts, top_n=_mr_top_n_today)
                if w_smart_t is not None and not w_smart_t.empty and excl_today_set:
                    w_excl_new = w_smart_t[~w_smart_t.index.isin(excl_today_set.keys())]
                    if w_excl_new.sum() > 0:
                        w_excl_new = w_excl_new / w_excl_new.sum()
                    else:
                        w_excl_new = w_smart_t
                else:
                    w_excl_new = w_smart_t if w_smart_t is not None else w_excl_deployed
            except Exception:
                w_excl_new = w_excl_deployed

        curr_exposure_excl = (exposure_excl_s.iloc[-1]
                              if nav_dd_excl is not None and not nav_dd_excl.empty
                              else 1.0)
        has_actual_excl = dyn_excl_rebal_today

        # Compute allocation changes per strategy
        all_tickers = (set(w_dyn_old.index) | set(w_dyn_new.index) |
                       set(w_excl_old.index) | set(w_excl_new.index))
        ...
        for tkr in sorted(all_tickers):
            ...
            if tkr in active_hedges_now:
                ...
                if hedge_event_today:
                    close_today = [e for e in hedge_results.get('hedge_log',[])
                                   if e['date'].date() == today_ts.date()
                                   and e['event'] == 'CLOSE'
                                   and tkr in e.get('instruments',[])]
                    open_today  = [e for e in hedge_results.get('hedge_log',[])
                                   if e['date'].date() == today_ts.date()
                                   and e['event'] == 'OPEN'
                                   and tkr in e.get('instruments',[])]
                    delta_hdg = (-h['weight'] * 100 if open_today else
                                 +h['weight'] * 100 if close_today else 0.0)
                else:
                    delta_hdg = 0.0
            else:
                delta_hdg = delta_dyn
            ...
            # Strategy 9 column
            old_excl   = w_excl_old.get(tkr, 0.0)
            new_excl   = w_excl_new.get(tkr, 0.0) if w_excl_new is not None else 0.0
            delta_excl = (new_excl - old_excl) * curr_exposure_excl * 100

            if (abs(delta_dyn) > 0.01 or abs(delta_hdg) > 0.01 or
                    abs(delta_dd) > 0.01 or abs(delta_excl) > 0.01):
                rows.append((tkr, delta_dyn, delta_hdg, delta_dd, delta_excl))

        has_actual = (dyn_rebal_today or hedge_event_today or
                      dd_event_today or has_actual_excl)
        label = ("[ACTUAL TRADES]" if has_actual else
                 "[THEORETICAL — hypothetical rebalance if triggered today]")
        # Show today's exclusions
        excl_today_display = _load_momentum_exclusions(today_ts, top_n=_mr_top_n_today)
        if excl_today_display:
            print(f"\n  Currently excluded ({len(excl_today_display)}): "
                  f"{', '.join(sorted(excl_today_display.keys()))}")
        print(f"\n  {label}")
        if rows:
            print(f"\n  {'Ticker':<10}  {'Δ Dynamic':>12}  {'Δ Dyn+Hedge':>13}"
                  f"  {'Δ DD Policy':>13}  {'Δ Excl':>10}")
            print(f"  {'-'*65}")
            for tkr, dd, dh, dp, de in sorted(rows, key=lambda x: x[3], reverse=True):
                hedge_tag  = '  [SHORT]' if tkr in active_hedges_now else ''
                excl_tag   = '  [EXCL]' if tkr in excl_today_display.keys() else ''
                print(f"  {tkr:<10}  {dd:>+11.2f}%  {dh:>+12.2f}%"
                      f"  {dp:>+12.2f}%  {de:>+9.2f}%{hedge_tag}{excl_tag}")
        else:
            if has_actual:
    ...
    return {
        ...
        'nav_dd'                      : nav_dd,
        'nav_dynamic_excl'            : nav_dynamic_excl,
        'hedge_results'               : hedge_results,
        'dyn_weights_by_date'         : dyn_weights_by_date,
        'dyn_rebal_dates'             : [r['dt'] for r in _dyn_rebal_log],
        'dyn_excl_weights_by_date'    : dyn_excl_weights_by_date,
        'dyn_excl_rebal_dates'        : [r['dt'] for r in _dyn_excl_rebal_log],
