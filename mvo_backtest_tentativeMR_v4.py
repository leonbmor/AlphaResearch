...
# momentum_exclusions runs before mvo_backtest in the pipeline
...
# ── MR exclusion filter (strategy 9) ──────────────────────────────────────────
# Fraction of liquidity-filtered candidates excluded per rebalance by joint score.
# MR_CAP=0.0 → no exclusions → strategy 9 converges naturally to strategy 8.
# MR_CAP=0.05 → top 5% worst offenders excluded.
MR_CAP = 0.05
...
def _run_excl_only_cache_build(
    ...
    """
    Rebuild ONLY alpha_excl/mvo_excl/hybrid_excl for dates that already have
    standard portfolios. Standard portfolios and triggers are never touched.
    """
    import time
    _ensure_tables()
    ...
    print("\n  Loading existing standard portfolios...")
    ...
    to_build  = [dt for dt in std_dates if dt not in excl_done]
    print(f"  Standard portfolios : {len(std_dates)} dates")
    print(f"  Excl already built  : {len(excl_done)} dates")
    print(f"  To build            : {len(to_build)} dates")

    if not to_build:
        print("  OK — excl portfolios fully up to date")
        return ph
    ...
    with _SuppressOutput():
        composite_by_date, _ = _cb_build_composite_scores(
            ...
            calc_dates      = pd.DatetimeIndex(to_build),
            Pxs_df          = Pxs_df,
            sectors_s       = sectors_s,
    ...
    print(f"\n  Building excl portfolios  MR_CAP={MR_CAP}...")
    for idx, dt in enumerate(to_build):
        std        = existing.get(dt, {})
        w_alpha    = std.get('alpha',  pd.Series(dtype=float))
        w_mvo_nz   = std.get('mvo',    pd.Series(dtype=float))
        w_hybrid   = std.get('hybrid', pd.Series(dtype=float))
        if w_alpha.empty:
            continue
        try:
            comp_scores = composite_by_date.get(dt, pd.Series(dtype=float))
            if comp_scores.empty:
                raise ValueError("no composite scores")
            cands = comp_scores.reindex(
                [t for t in comp_scores.index if t in pxs_cols]
            ).dropna().sort_values(ascending=False)
            if len(cands) < top_n:
                raise ValueError("insufficient candidates")
        except Exception:
            _save_portfolios(dt, model_version, ph, {
                'alpha_excl': w_alpha.copy(),
                'mvo_excl':   w_mvo_nz.copy(),
                'hybrid_excl':w_hybrid.copy(),
            })
            n_saved += 1
            continue
        try:
            ...
            cands_excl = cands[~cands.index.isin(excl_dt.keys())] if excl_dt else cands
            if len(cands_excl) >= top_n and excl_dt:
                ranked_e = cands_excl.sort_values(ascending=False)
                ap_e     = [t for t in ranked_e.head(top_n).index if t in pxs_cols]
                n_e      = len(ap_e)
                if conc_factor == 1.0 or n_e < 2:
                    w_ae = pd.Series(1.0 / n_e, index=ap_e)
                else:
                    n_top_e = int(np.ceil(n_e / 2))
                    n_bot_e = n_e - n_top_e
                    top_a   = conc_factor / (conc_factor + 1.0)
                    bot_a   = 1.0 / (conc_factor + 1.0)
                    w_ae = pd.Series({
                        t: (top_a / n_top_e if j < n_top_e else bot_a / n_bot_e)
                        for j, t in enumerate(ap_e)
                    })
                try:
                    with _SuppressOutput():
                    ...
                    all_t_e = list(set(w_ae.index) | set(w_me_nz.index))
                    w_he    = (w_ae.reindex(all_t_e).fillna(0) +
                               w_me_nz.reindex(all_t_e).fillna(0)) / 2.0
                    w_he    = w_he[w_he > 0]
                    if w_he.sum() > 0:
                        w_he = w_he / w_he.sum()
                except Exception:
                    w_me_nz = w_mvo_nz.copy()
                    w_he    = w_hybrid.copy()
                    w_ae    = w_alpha.copy()
            else:
                w_ae    = w_alpha.copy()
            ...
            warnings.warn(f"  Excl failed {dt.date()}: {e}")
            n_errors += 1
        if idx % 100 == 0 or idx == len(to_build) - 1:
            elapsed = time.time() - t_start
            eta     = elapsed / max(idx+1,1) * (len(to_build) - idx - 1)
            print(f"  [{idx+1:>4}/{len(to_build)}] {dt.date()}  "
                  f"elapsed={elapsed:>5.0f}s  eta={eta:>5.0f}s", end='\r')

    print(f"\n  OK: {n_saved} excl dates saved, {n_errors} errors  "
...
def run_daily_cache_build(
    ...
    model_version='v2',
    force_rebuild=False,
    excl_only=False,    # True = only rebuild excl portfolios, never touch standard
    pct_df=None,
    ou_scores_df=None,
):
    """
    ...
    force_rebuild=True  : clear and recompute all standard portfolios
    excl_only=True      : ONLY rebuild alpha_excl/mvo_excl/hybrid_excl.
                          Standard portfolios and triggers are NEVER touched.
                          Use this whenever MR_CAP changes.
    """
    ...
    if excl_only:
        if force_rebuild:
            print("  WARNING: force_rebuild=True ignored in excl_only mode.")
        print(f"  MODE: excl_only=True  MR_CAP={MR_CAP}  — standard portfolios untouched")
        return _run_excl_only_cache_build(
            ph=ph, model_version=model_version,
            Pxs_df=Pxs_df, sectors_s=sectors_s,
            weights_by_year=weights_by_year, regime_s=regime_s,
            volumeTrd_df=volumeTrd_df,
            ic=ic, max_weight=max_weight, min_weight=min_weight,
            zscore_cap=zscore_cap, pca_var_threshold=pca_var_threshold,
            universe_mult=universe_mult, risk_aversion=risk_aversion,
            top_n=top_n, conc_factor=conc_factor, prefilt_pct=prefilt_pct,
    ...
    for idx, dt in enumerate(to_compute):
        ...
        # -- Excl portfolios (isolated — never affects standard save) ----------
        try:
            _mr_top_n = int(MR_CAP * len(cands)) if MR_CAP > 0 else 0
            excl_dt   = _load_momentum_exclusions(dt, top_n=_mr_top_n) if _mr_top_n > 0 else {}
            if excl_dt:
            ...
            if len(cands_excl) >= top_n and excl_dt:
                ...
                n_e      = len(ap_e)
                if conc_factor == 1.0 or n_e < 2:
                    w_alpha_excl = pd.Series(1.0 / n_e, index=ap_e)
                else:
                    n_top_e = int(np.ceil(n_e / 2))
                    n_bot_e = n_e - n_top_e
                    top_a   = conc_factor / (conc_factor + 1.0)
                ...
                # No exclusions or insufficient candidates — copy standard
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    # ── Strategy 9: Dyn+Hedge+DD+Excl ────────────────────────────────────────
    # Parasitic on strategy 8: same rebalancing dates, same regime, same triggers.
    # Only difference: excl-filtered portfolios replace standard at each rebalance.
    # At MR_CAP=0, excl_dt={} for all dates → excl portfolios = standard portfolios
    # → nav_dd_excl converges naturally to nav_dd (reliable consistency check).
    nav_dd_excl      = None
    _dd_excl_log     = []
    _dyn_excl_wbd    = {}   # weights by date for strategy 9
    _cost_by_date_excl = {}

    try:
        all_port_cache = load_daily_portfolios(params_hash, MB_MODEL_VER)
        _has_excl = any('alpha_excl' in v for v in all_port_cache.values()) \
                    if all_port_cache else False

        if _has_excl and _dyn_rebal_log:
            # Build strategy 9 weights by following strategy 8's rebalance log
            # At MR_CAP=0: use dyn_weights_by_date directly — structurally identical
            # to what the loop would produce, eliminating any forward-fill differences
            if MR_CAP == 0.0:
                _dyn_excl_wbd      = dyn_weights_by_date
                _cost_by_date_excl = _cost_by_date.get('dynamic', {})
            else:
                w_excl_prev = pd.Series(dtype=float)
                for rec in _dyn_rebal_log:
                    dt_r      = rec['dt']
                    regime_r  = rec['regime']
                    port_r    = all_port_cache.get(dt_r, {})
                    w_new = port_r.get(f'{regime_r}_excl',
                            port_r.get(regime_r, pd.Series(dtype=float)))
                    if w_new.empty:
                        w_new = rec['w'].copy()
                    if not w_excl_prev.empty:
                        all_t = list(set(w_new.index) | set(w_excl_prev.index))
                        to_e  = (w_new.reindex(all_t).fillna(0) -
                                 w_excl_prev.reindex(all_t).fillna(0)).abs().sum() / 2
                        _cost_by_date_excl[dt_r] = to_e * TRADING_COST_BPS / 10000
                    _dyn_excl_wbd[dt_r] = w_new.copy()
                    w_excl_prev         = w_new.copy()

                # Fill forward between rebalances
                if dyn_weights_by_date and _dyn_excl_wbd:
                    last_w = pd.Series(dtype=float)
                    for dt in sorted(dyn_weights_by_date.keys()):
                        if dt in _dyn_excl_wbd:
                            last_w = _dyn_excl_wbd[dt]
                        elif not last_w.empty:
                            _dyn_excl_wbd[dt] = last_w

            # NAV for strategy 9 (pre-hedge)
            nav_dyn_excl = (_mb_run_nav(
                _dyn_excl_wbd, sorted(_dyn_excl_wbd.keys()),
                Pxs_df, cost_by_date=_cost_by_date_excl)
                if _dyn_excl_wbd else pd.Series(dtype=float))

            # Hedge layer — same trigger, independent account on excl portfolio
            # At MR_CAP=0: reuse strategy 8's hedge results directly
            nav_excl_hedged = None
            if hedge_results is not None and not nav_dyn_excl.empty:
                if MR_CAP == 0.0:
                    # Structurally identical — reuse strategy 8's hedged NAV
                    nav_excl_hedged = nav_dyn_hedged.copy() if nav_dyn_hedged is not None else nav_dyn_excl.copy()
                else:
                    try:
                        hr_excl = run_hedge_backtest(
                            Pxs_df            = Pxs_df,
                            multi             = hedge_multi,
                            portfolio_weights = _dyn_excl_wbd,
                            rebal_dates       = [r['dt'] for r in _dyn_rebal_log],
                            hedges_l          = hedges_l,
                            trigger_assets    = hedge_trigger_assets or ['QQQ'],
                        )
                        nav_excl_hedged = nav_dyn_excl.copy()
                        cum_h = 0.0
                        for dt in nav_excl_hedged.index:
                            if dt in hr_excl['hedge_sweep_by_date']:
                                cum_h += hr_excl['hedge_sweep_by_date'][dt]
                            nav_excl_hedged[dt] = nav_dyn_excl[dt] * (1 + cum_h)
                    except Exception as _he:
                        nav_excl_hedged = nav_dyn_excl.copy()
            else:
                nav_excl_hedged = nav_dyn_excl.copy() if not nav_dyn_excl.empty else None

            # DD policy — independent, applied to excl hedged NAV
            # At MR_CAP=0: reuse strategy 8's DD NAV directly
            if MR_CAP == 0.0:
                nav_dd_excl = nav_dd.copy() if nav_dd is not None else None
                if nav_dd_excl is not None:
                    nav_dd_excl.name = 'nav_dd_excl'
            elif nav_excl_hedged is not None and not nav_excl_hedged.empty:
                base_ret_e = nav_excl_hedged.pct_change().fillna(0)
                mvo_ret_e  = nav_mvo.pct_change().fillna(0) if nav_mvo is not None else base_ret_e
                dd_nav_e   = pd.Series(index=nav_excl_hedged.index, dtype=float)
                dd_nav_e.iloc[0] = nav_excl_hedged.iloc[0]
                exp_e_s    = pd.Series(index=nav_excl_hedged.index, dtype=float)
                exp_e_s.iloc[0] = 1.0
                hwm_e = nav_excl_hedged.iloc[0]; exp_e = 1.0
                tr_e  = nav_excl_hedged.iloc[0]; th_e  = nav_excl_hedged.iloc[0]
                dd_act_e = False; conf_e = 0; nxt_e = 0

                for i, dt in enumerate(nav_excl_hedged.index[1:], 1):
                    prev_e = dd_nav_e.iloc[i-1]
                    if dd_act_e:
                        th_e = th_e * (1 + mvo_ret_e.get(dt, 0))
                    raw_e   = base_ret_e.get(dt, 0)
                    cur_e   = prev_e * (1 + raw_e * exp_e)
                    dd_nav_e.iloc[i] = cur_e
                    exp_e_s.iloc[i]  = exp_e
                    if not dd_act_e:
                        hwm_e = max(hwm_e, cur_e)
                    dd_cur_e = cur_e / hwm_e - 1
                    if nxt_e < len(DD_LEVELS):
                        thr_e, cut_e = DD_LEVELS[nxt_e]
                        if dd_cur_e <= -thr_e:
                            old_e = exp_e; exp_e = exp_e * (1 - cut_e)
                            tr_e  = cur_e; th_e  = cur_e
                            conf_e = 0; nxt_e += 1; dd_act_e = True
                            pc_e  = (old_e - exp_e) * TRADING_COST_BPS / 10000
                            cur_e = cur_e * (1 - pc_e)
                            dd_nav_e.iloc[i] = cur_e; exp_e_s.iloc[i] = exp_e
                            _dd_excl_log.append({
                                'date': dt, 'event': f'DE-GROSS lv{nxt_e}',
                                'dd': dd_cur_e, 'exposure_from': old_e, 'exposure_to': exp_e,
                            })
                    if dd_act_e and exp_e < 1.0:
                        tr_e = min(tr_e, cur_e)
                        th_rec_e = th_e / tr_e - 1
                        yr_s_e   = pd.Timestamp(f'{dt.year}-01-01')
                        ytd_e    = (cur_e / dd_nav_e[dd_nav_e.index >= yr_s_e].iloc[0] - 1
                                    if not dd_nav_e[dd_nav_e.index >= yr_s_e].empty else 0.0)
                        jan_e    = (dt.month == 1 and dt.day <= 10 and
                                    ytd_e >= -DD_ANNUAL_RESET_PCT)
                        if th_rec_e >= DD_REENTRY_PCT:
                            conf_e += 1
                        else:
                            conf_e = 0
                        if conf_e >= DD_REENTRY_CONFIRM or jan_e:
                            old_e = exp_e; exp_e = 1.0
                            dd_act_e = False; nxt_e = 0
                            hwm_e = cur_e; conf_e = 0
                            pc_e  = old_e * TRADING_COST_BPS / 10000
                            cur_e = cur_e * (1 - pc_e)
                            dd_nav_e.iloc[i] = cur_e; exp_e_s.iloc[i] = exp_e
                            _dd_excl_log.append({
                                'date': dt, 'event': 'RE-ENTRY 100%',
                                'theo_recovery': th_rec_e,
                                'exposure_from': old_e, 'exposure_to': 1.0,
                                'trigger': 'recovery' if not jan_e else 'annual reset',
                            })

                nav_dd_excl = dd_nav_e
                nav_dd_excl.name = 'nav_dd_excl'

            # DD summary for strategy 9
            dg_e = [e for e in _dd_excl_log if 'DE-GROSS' in e['event']]
            re_e = [e for e in _dd_excl_log if 'RE-ENTRY' in e['event']]
            print(f"\n  Strategy 9 DD summary: {len(dg_e)} de-gross, {len(re_e)} re-entries")
    ...
    for nav_s, lbl in [
        ...
        (nav_dd_excl,      f'Dyn+Hedge+DD+Excl (MR_CAP={MR_CAP:.0%})'),
    ]:
    ...
    # Pick the primary NAV for AUM tracking — strategy 9 if available, else 8
    ...
    print(f"  {'Year':<6} {'Baseline':>10} {'PureAlpha':>11} {'MVO':>9} {'Hybrid':>9} {'Smart':>9} {'Dynamic':>9} {'Dyn+Hdg':>9} {'DD Pol':>8} {'Excl':>8} {'AUM($M)':>9}")
    print(f"  {'-'*113}")
    ...
    for yr in all_years:
        ...
        excl_str = f"{yr_ret(nav_dd_excl):>+7.2f}%" if (nav_dd_excl is not None and not nav_dd_excl.empty) else f"{'n/a':>8}"
