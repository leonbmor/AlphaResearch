...
ETF_MAV1             = 10      # fast EWM halflife on p_low (short reactivity)
ETF_MAV2             = 30      # slow EWM halflife on p_low (baseline)
ETF_THRESH           = -10.0   # MA-cross margin before flagging (negative = margin)
...
def _build_etf_stress_signals(hmm, cusum,
    ...
    for asset in assets:
        ...
        # ---- Combine (either leg calm -> do-not-penalise) ------------------
        ...
        risk_on  = (Fcusum_a + FFhmm_a).clip(0, 1)
...
def run_backtest(
    ...
    if etf_penalty_scale < 1.0 and hmm is not None and cusum is not None and etf_map_s is not None:
        try:
            _etf_stress_flags = _build_etf_stress_signals(hmm, cusum,
    ...
    # ── Helper: apply DD de-grossing / zone tracking ──────────────────────────
