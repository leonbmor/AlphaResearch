...
def run_backtest(
    ...
    print(hdr)
    # DD row — current drawdown per strategy
    _dd_row = f"  {'DD now':<6}"
    for s_idx in range(N_STRAT):
        dd = state[s_idx]['dd'] * 100
        _dd_row += f"  {dd:>+8.1f}%" if s_idx > 0 else f"  {'':>9}"
    _dd_row += f"  {'':>9}"
    print(_dd_row)
    print("  " + "-" * (len(hdr) - 2))
    ...
    print(hdr.replace('AUM($M)', '   Total'))
    print("  " + "-" * (len(hdr) - 2))
    for yr in all_years:
        def fmt_c(c): return f"{c*100:>+8.2f}%" if c > 0 else f"  {'':>8}"
        costs = [state[s]['costs_by_year'].get(yr, 0.0) for s in range(N_STRAT)]
        total_c = sum(costs)
        print(f"  {yr:<6} {fmt_c(costs[0])} {fmt_c(costs[1])} {fmt_c(costs[2])} "
              f"{fmt_c(costs[3])} {fmt_c(costs[4])} {fmt_c(costs[5])} "
              f"{fmt_c(costs[6])} {fmt_c(costs[7])} {fmt_c(costs[8])}  "
              f"{fmt_c(total_c)}")
