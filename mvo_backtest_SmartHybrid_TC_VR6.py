...
DYN_TO_THRESHOLD   = 0.20    # turnover trigger (rebalance if TO > this)
DYN_VOLDIFF_CAP    = 0.15    # max vol increase allowed alongside TO trigger
DYN_VOLDIFF_DERISK = -0.075  # de-risk trigger (rebalance if vol_diff < this)
DYN_MIN_HOLD_DAYS  = 5       # minimum calendar days between rebalances
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
        ...
        if not trig_df.empty and triggers_dyn:
            ...
            for dt in sorted(trig_df.index):
                if dt not in triggers_dyn:
                    continue
                row      = trig_df.loc[dt]
                to_val   = float(row['implied_turnover'])
                ...
                regime_switch = (prev_regime is not None and
                                 regime != prev_regime and
                                 days_held >= DYN_MIN_HOLD_DAYS)
                to_trigger    = (to_val > DYN_TO_THRESHOLD and
    ...
    for nav_s, lbl in [
        ...
        (nav_dynamic,  f'Dynamic (TO>{DYN_TO_THRESHOLD:.0%},vd<{DYN_VOLDIFF_CAP:.0%},hold={DYN_MIN_HOLD_DAYS}d)'),
    ]:
