    ...
    def _apply_dd(s_idx, dt, hi_c=False, lo_c=False, lo_fast=False,
        ...
        if (not virtual) and HMM_PAROLE and st['dd_active']:
            ...
            if (lo_c and not st.get('parole', False)
                    ...
                    and st['gross'] < 1.0):
                # Fast-track parole cooldown: if THIS grant is fast-track only
                # (CUSUM-latch path, lo_fast) and we're within PAROLE_COOLDOWN_DAYS
                # trading days of the last revoke, suppress it -- the stale 30d
                # down-latch + one-window-p_low bar would otherwise re-grant
                # immediately and flip-flop. The SLOW consensus (lo_c via the
                # sustained 5-day path, lo_fast=False) is NEVER cooled down.
                _cooled = False
                if lo_fast and PAROLE_COOLDOWN_DAYS > 0 and st.get('parole_revoke_dt') is not None:
                    _idx = Pxs_df.index
