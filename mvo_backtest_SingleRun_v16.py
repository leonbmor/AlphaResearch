    ...
    def _update_hedge(s_idx, dt, port_w, port_nav):
        ...
        if trigger_on and not st['active_hedges'] and hedges_l:
            ...
            if not port_ret_s.empty:
                ...
                if selected:
                    # Diagnostic: print scores for all candidates on first hedge open
                    if not hasattr(_update_hedge, '_scores_printed'):
                        _update_hedge._scores_printed = True
                        print(f"\n  [HEDGE OPEN {dt.date()}] Instrument scores:")
                        print(f"  {'Inst':<8} {'Corr':>8} {'Eff':>8} {'Beta':>8} {'Selected':>9}")
                        print(f"  {'-'*45}")
                        for inst in hedges_l:
                            if inst not in inst_ret or inst not in _hedge_results:
                                continue
                            corr = _compute_corr(port_ret_s, inst_ret[inst], CORR_WINDOW)
                            eff  = _get_effectiveness(_hedge_results[inst]['signal_df'],
                                                      dt, EFF_MAV_WINDOW)
                            beta = _compute_beta(port_ret_s, inst_ret[inst], BETA_WINDOW)
                            sel  = '✓' if inst in selected else ''
                            print(f"  {inst:<8} {corr:>8.3f} "
                                  f"{'NaN':>8}" if np.isnan(eff) else
                                  f"  {inst:<8} {corr:>8.3f} {eff:>8.3f} "
                                  f"{beta:>8.3f} {sel:>9}", flush=True)
                    n_inst      = len(selected)
                    total_hedge = min(n_inst * hedge_ratio, max_hedge)
