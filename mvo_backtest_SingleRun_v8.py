...
def _advp_filter_and_replace(weights: pd.Series,
    ...
    # Apply ADVP weight cap to liquid-but-constrained stocks, iteratively
    # until stable (renorm can push weights back above cap)
    _advp_caps = {}
    ...
    # Iteratively apply ADVP caps + renorm until stable
    _capped = set()
    for _ in range(20):
        changed = False
        for tkr, cap_w in _advp_caps.items():
            if tkr in w_new and w_new[tkr] > cap_w + 1e-9:
                w_new[tkr] = cap_w
                _capped.add(tkr)
                changed = True
        if w_new.sum() > 0:
...
def run_backtest(
    ...
    def _apply_advp(w, cands, dt, strat_idx, target_n=None):
        ...
        # Handle both old (weights only) and new (weights, affected) return
