...
def _pa_load_lambdas(model_version, lookback=None):
    ...
    Also returns factor_names list, sec_cols list, subsec_cols list.
    """
    from sqlalchemy import text as _text
    tbls   = RD_SCALAR_TABLES[model_version]
    frames = []

    def _load(tbl, sql=None):
        with ENGINE.connect() as conn:
            df = pd.read_sql(_text(sql or f"SELECT * FROM {tbl} ORDER BY date"), conn)
        df['date'] = pd.to_datetime(df['date'])
        df = df.set_index('date').sort_index()
        df = df.drop(columns=[c for c in RD_LAMBDA_META if c in df.columns],
                     errors='ignore')
        for c in df.columns:
    ...
    for fname, tbl in tbls.items():
        df = _load(tbl)
        if df.shape[1] >= 1:
            frames.append(df.iloc[:, 0].rename(fname))

    # Macro
    mdf = _load(RD_MACRO_TBL[model_version])
    for mc in MACRO_COLS:
        if mc in mdf.columns:
            frames.append(mdf[mc].rename(mc))

    # Sectors
    sdf      = _load(RD_SEC_TBL[model_version])
    sec_cols = sorted([c for c in sdf.columns if c not in RD_LAMBDA_META])
    for sc in sec_cols:
        frames.append(sdf[sc].rename(sc))

    # Sub-sectors
    subsec_cols = []
    subsec_tbl  = RD_SUBSEC_TBL.get(model_version)
    if subsec_tbl:
        try:
            ssdf = _load(subsec_tbl)
            subsec_cols = sorted([c for c in ssdf.columns if c not in RD_LAMBDA_META])
            for ssc in subsec_cols:
                frames.append(ssdf[ssc].rename(ssc))
        except Exception as e:
            warnings.warn(f"Sub-sector lambdas not available ({e})")
    ...
    ordered  = (
        ...
        + [c for c in subsec_cols  if c in combined.columns]
        + [c for c in tail         if c in combined.columns]
    )
    lam_df       = combined[ordered]
    factor_names = ordered
    return lam_df, factor_names, sec_cols, subsec_cols
...
def run_pnl_attribution(port_df, Pxs_df, sectors_df,
    ...
    port_df       : pd.DataFrame — dates x tickers, % weight allocations
    Pxs_df        : pd.DataFrame — price/macro panel
    sectors_df    : pd.Series   (ticker -> sector, backward compat)
    ...
    Returns
    -------
    attrib_df     : pd.DataFrame — dates x factors (decimal daily PnL)
    stock_idio_df : pd.DataFrame — dates x tickers (per-stock idio PnL)
    """
    Pxs_df  = Pxs_df.loc[:, ~Pxs_df.columns.duplicated(keep='first')]
    port_df = port_df.loc[:, ~port_df.columns.duplicated(keep='first')]

    # Backward compat: accept Series or DataFrame
    ...
    print(f"  Universe: {len(universe)} stocks  |  "
          ...
          f"({len(sec_cols)} sectors · {len(subsec_cols)} sub-sectors)")

    # ── Lambda time series ────────────────────────────────────────────────────
    print("[2/4] Loading factor lambda series...")
    lam_df, _, _, _ = _pa_load_lambdas(model_version)
    ...
    port_dates = port_dates.intersection(Pxs_df.index).sort_values()
    ...
    for i, xdt in enumerate(x_snap_dates):
        ...
        xdf = _rd_build_X_on_date(
            xdt, universe, factor_names, sec_cols, subsec_cols,
            Pxs_df, sectors_s, subsec_s, volumeTrd_df, model_version
        )
        if xdf is not None:
    ...
    for i, dt in enumerate(port_dates):
        ...
        if not valid_snaps:
            missing_X += 1
            continue
        X_t = X_snapshots[valid_snaps[-1]]

        if dt not in lam_df.index:
            continue
        lam_t = lam_df.loc[dt].reindex(factor_names).fillna(0.0).values

        Xw         = X_t.values.T @ w.values
        factor_pnl = Xw * lam_t
        pred_t     = X_t.values @ lam_t
        epsilon_t  = r_t.values - pred_t
        stock_idio_t = w.values * epsilon_t
    ...
    _pa_print_summary(attrib_df, stock_idio_df, factor_names,
                      sec_cols, subsec_cols, model_version)
    _pa_plot(attrib_df, factor_names, sec_cols, subsec_cols, model_version)
...
def _pa_print_summary(attrib_df, stock_idio_df, factor_names,
                      sec_cols, subsec_cols, model_version):
    ...
    _block("Sub-sectors",   subsec_cols)
...
def _pa_plot(attrib_df, factor_names, sec_cols, subsec_cols, model_version):
    """
    Three panels:
      1. Cumulative PnL by group (structural/macro/sectors/sub-sectors/alpha/idio)
      2. Rolling realized R²
      3. Daily factor PnL heatmap — recent 60 days
    """
    COLORS = {
        'Structural'    : '#378ADD',
        'Macro'         : '#EF9F27',
    ...
    subsec_c     = [c for c in subsec_cols     if c in attrib_df.columns]
    total        = attrib_df['Total']
    ...
    groups = [
        ...
        ('Sub-sectors',    [attrib_df[c] for c in subsec_c]),
