...
EARNINGS_ONLY = False   # set by the main() prompt: when True, skip any
                        # stock whose FEP quarter has NOT released
ACCEPT_GAAP = object()  # sentinel: user chose "accept GAAP as-is, mark done"
NO_ADJUST   = object()  # sentinel: AV NaN + no override -> proceed, no non-GAAP adj
...
def set_stock_fep(ticker: str, fep: str):
    """ONE FEP per stock: every frontier write goes through here and lands
    identically on income, cash and summary. Income is the source of truth;
    the other categories are mirrors. Never call update_estimation_status
    directly for frontier moves."""
    for cat in ('income', 'cash', 'summary'):
        update_estimation_status(ticker, cat, fep)


def reconcile_flags_to_fep(ticker: str, fep: str):
    """Reconcile the LATEST-VINTAGE estimated_values flag of each period to FEP:
    period <  FEP -> False (actual);  period >= FEP -> True (estimate).

    FEP is the SINGLE SOURCE OF TRUTH for CURRENT actual-vs-estimate status.
    Only the latest download_date row per period is updated: older vintages are
    frozen point-in-time records (a period fetched while it was still a
    forecast is correctly est=True as-of that download_date, and PIT reads
    depend on that history), so they must NEVER be rewritten. This corrects
    current-state drift (e.g. LHX latest-vintage inverted flags) without
    touching history. One UPDATE per table, scoped to the latest vintage.

    Runs across all three category tables."""
    fep_i = _period_idx(fep)
    pidx = ("(CAST(SUBSTRING(period, 1, 4) AS INT) * 4 "
            "+ CAST(SUBSTRING(period, 6, 1) AS INT) - 1)")
    for cat in ('income', 'cash', 'summary'):
        tbl = f'{cat}_data'
        with ENGINE.begin() as conn:
            # Update only the row with the max download_date for each period.
            conn.execute(text(f"""
                UPDATE {tbl} d SET estimated_values = ({pidx.replace('period','d.period')} >= :fi)
                WHERE d.ticker = :t
                  AND d.download_date = (
                        SELECT MAX(d2.download_date) FROM {tbl} d2
                        WHERE d2.ticker = d.ticker
                          AND d2.period = d.period
                          AND d2.metric_name = d.metric_name)
                  AND d.estimated_values IS DISTINCT FROM ({pidx.replace('period','d.period')} >= :fi)
            """), {"t": ticker, "fi": fep_i})


def _period_idx(p: str) -> int:
...
def fetch_av_earnings(ticker: str, ortex_period: Optional[str] = None):
    ...
    V3 validation (replaces the calendar-grid expected-date check):
      1. PATTERN: expected fiscalDateEnding = quarterly[4].fiscalDateEnding
         + 1 year (the company's own history IS its fiscal calendar; the same
         quarter one year ago is 4 quarters back = index 4).
         Mismatch beyond ±AV_EXPECTED_TOLERANCE_DAYS -> data not ready / glitch
         -> return None (skip & retry). <5 quarters of history -> check
    ...
    while attempts < AV_MAX_RETRIES:
        try:
            ...
            # The same fiscal quarter ONE YEAR AGO is 4 quarters back. In AV's
            # descending-by-date quarterlyEarnings list that is index 4
            # (idx 0=latest; 1,2,3 = the three prior quarters; 4 = year-ago).
            # Using index 3 (only 3 quarters back) was an off-by-one that made
            # `expected` land ~1 quarter (~90d) off `latest`, exceeding the
            # tolerance and spuriously rejecting valid data for EVERY name that
            # reached this check (e.g. ZM: latest 2026-04-30 vs bad-expected
            # 2026-07-31 = 92d miss). Year-ago index 4 makes expected == latest.
            if len(quarterly) >= 5:
                try:
                    year_ago = pd.Timestamp(quarterly[4].get('fiscalDateEnding'))
                ...
                # <5 quarters of history (recent IPOs etc.): cannot derive the
                # year-ago pattern; the EPS sanity check below is the only gate.
                logger.warning(f"    AV: {t} has <5 quarters of AV history -- "
...
def resolve_nongaap_eps(ticker: str, quarter: str, av_data: Optional[Dict],
                        ortex_eps=None, ortex_diluted_eps=None,
                        prev_spread=0.0):
    """Two-stage non-GAAP EPS resolution, keeping BASIC and DILUTED independent.

    The AV suggestion (`reportedEPS`) is non-GAAP DILUTED. Ortex `eps` is GAAP
    BASIC and Ortex `dilutedEps` is GAAP DILUTED -- different share bases. To
    compare like with like we convert AV's diluted figure to a basic-equivalent
    using the immediately-prior quarter's basic-vs-diluted spread:

        nongaap_basic = av_nongaap_diluted + prev_spread
        where prev_spread = prev_qtr(eps) - prev_qtr(dilutedEps)   (>= 0 usually)

    (If the prior-quarter spread was unavailable, the caller passes 0.0, i.e.
    basic == diluted -- a convenience default; the user validates anyway.)

    Displays two comparison rows, then prompts TWICE:
      1. non-GAAP BASIC (AV + spread)  vs  Ortex eps (GAAP basic)
         -> validated value is the basis for ALL cash-metric adjustments
            (netIncome, operatingIncome, ebitda) and becomes stored `eps`.
      2. non-GAAP DILUTED (raw AV)     vs  Ortex dilutedEps (GAAP diluted)
         -> validated value OVERRIDES Ortex `dilutedEps`.

    Each prompt: type a number to OVERRIDE, or Enter to accept the shown
    suggestion. If AV is NaN/missing, returns NO_ADJUST (no adjustment at all).

    Returns: (nongaap_basic, nongaap_diluted) as floats, or the NO_ADJUST
    sentinel. Distinguishes 'typed 0' from 'pressed Enter' on the raw string.
    """
    av_raw = av_data.get('reportedEPS') if av_data else None
    try:
        av_diluted = float(av_raw)
        av_finite  = math.isfinite(av_diluted)
    except (TypeError, ValueError):
        av_diluted, av_finite = None, False

    def _fmt(x):
        try:
            xf = float(x)
            return f"{xf:.4f}" if math.isfinite(xf) else "n/a"
        except (TypeError, ValueError):
            return "n/a"

    # ---- AV NaN/missing: no adjustment path (unchanged semantics) ----
    if not av_finite:
        print(f"\n  {ticker} | {quarter}: AV non-GAAP EPS UNAVAILABLE (NaN/missing)")
        print(f"    Ortex GAAP:  eps = {_fmt(ortex_eps)}   dilutedEps = {_fmt(ortex_diluted_eps)}")
        raw = input(f"    Enter = proceed with NO adjustment, or type non-GAAP "
                    f"DILUTED EPS to force one: ").strip()
        if raw == "":
            logger.info(f"  {ticker} | {quarter}: AV NaN, no override -> NO adjustment")
            return NO_ADJUST
        try:
            av_diluted = float(raw)
            assert math.isfinite(av_diluted)
        except (TypeError, ValueError, AssertionError):
            print(f"    '{raw}' not a finite number -- NO adjustment.")
            return NO_ADJUST
        av_finite = True   # fall through with the forced diluted number

    # ---- implied non-GAAP BASIC from the AV diluted number + prior spread ----
    try:
        ...
        spread = 0.0
    nongaap_basic_suggest   = av_diluted + spread
    nongaap_diluted_suggest = av_diluted

    # ---- display: two comparison rows (basic row, then diluted row) ----
    print(f"\n  {ticker} | {quarter}: non-GAAP EPS resolution "
          f"(prev-qtr basic-diluted spread = {spread:+.4f})")
    print(f"    {'':<26}{'non-GAAP':>12}   {'Ortex GAAP':>12}")
    print(f"    {'BASIC   (AV+spread)':<26}{nongaap_basic_suggest:>12.4f}   "
          f"{_fmt(ortex_eps):>12}")
    print(f"    {'DILUTED (AV raw)':<26}{nongaap_diluted_suggest:>12.4f}   "
          f"{_fmt(ortex_diluted_eps):>12}")
    # order-of-magnitude divergence flag on the BASIC comparison (like-for-like)
    try:
        oe = float(ortex_eps)
        if math.isfinite(oe) and abs(oe) > 1e-9:
            r = nongaap_basic_suggest / oe
            if r > 2.0 or r < 0.5:
                print(f"    ** NOTE: non-GAAP-basic / Ortex-eps = {r:.2f}x "
                      f"-- large divergence, verify **")
    except (TypeError, ValueError):
        pass

    def _prompt_value(label, suggestion):
        raw = input(f"    [{label}] Enter = accept {suggestion:.4f}, "
                    f"or type override: ").strip()
        if raw == "":
            return suggestion
        try:
            ov = float(raw)
            if not math.isfinite(ov):
                print(f"    non-finite -- accepting {suggestion:.4f}.")
                return suggestion
            return ov
        except ValueError:
            print(f"    '{raw}' not a number -- accepting {suggestion:.4f}.")
            return suggestion

    nongaap_basic   = _prompt_value("validate BASIC non-GAAP", nongaap_basic_suggest)
    nongaap_diluted = _prompt_value("validate DILUTED non-GAAP", nongaap_diluted_suggest)

    logger.info(f"  {ticker} | {quarter}: non-GAAP basic={nongaap_basic} "
                f"(AV diluted {av_diluted} + spread {spread}), "
                f"diluted={nongaap_diluted}")
    return (nongaap_basic, nongaap_diluted)

def delete_vintage_for_period(ticker: str, period: str, download_date):
    """Delete all rows for (ticker, period) at a specific download_date across
    the three tables. Used to ROLL BACK a raw actual save when the non-GAAP
    adjustment does not complete -- so an unadjusted vintage can never become
    the latest row of a quarter that was previously adjusted. Preserves the
    prior (adjusted) vintage, which then remains latest."""
    t  = clean_ticker(ticker)
    dt = to_ts(download_date)
    deleted = {}
    with ENGINE.begin() as conn:
        for cat in ('income', 'cash', 'summary'):
            r = conn.execute(text(f"""
                DELETE FROM {cat}_data
                WHERE ticker = :t AND period = :p AND download_date = :d
            """), {"t": t, "p": period, "d": dt})
            deleted[cat] = r.rowcount
    logger.warning(f"  {t} | {period}: rolled back raw vintage {dt} "
                   f"(adjustment did not complete) -- "
                   f"{', '.join(f'{k}={v}' for k,v in deleted.items())}")
    return deleted
...
def apply_nongaap_adjustments(ticker: str, last_actual: str,
    ...
    """
    V3: returns 'applied' | 'gaap' (accept GAAP as-is, mark done) |
    'noadj' (AV NaN + no override: save actuals, no non-GAAP adj, mark done) |
    ...
    # Pre-read Ortex GAAP eps/dilutedEps for the quarter so the prompt can
    # show them next to AV's suggestion. These are plain reads; the
    # authoritative resolution happens in the loop below.
    _disp_eps  = get_metric_value_strict(t, 'eps',        last_actual, dt, 'income_data')
    _disp_deps = get_metric_value_strict(t, 'dilutedEps', last_actual, dt, 'income_data')

    # Prior-quarter basic-vs-diluted spread, used to convert AV's non-GAAP
    # DILUTED suggestion into a non-GAAP BASIC equivalent:
    #   spread = prev_qtr(eps) - prev_qtr(dilutedEps)   (latest-vintage actual)
    # If either is unavailable -> spread = 0 (convenience default; user
    # validates the resulting basic number anyway).
    _prev_q    = add_quarters(last_actual, -1)
    _prev_eps  = get_metric_value_ffill(t, 'eps',        _prev_q, dt, 'income_data')
    _prev_deps = get_metric_value_ffill(t, 'dilutedEps', _prev_q, dt, 'income_data')
    try:
        _spread = float(_prev_eps) - float(_prev_deps)
        if not math.isfinite(_spread):
            _spread = 0.0
    except (TypeError, ValueError):
        _spread = 0.0
        logger.info(f"  {t}: prev-qtr ({_prev_q}) spread unavailable -> 0.0")

    # AV is a SUGGESTION. Two-stage resolution keeps BASIC and DILUTED
    # independent: prompt 1 validates non-GAAP BASIC (AV diluted + spread) ->
    # basis for cash-metric adjustments + stored eps; prompt 2 validates
    # non-GAAP DILUTED (raw AV) -> overrides dilutedEps.
    resolved = resolve_nongaap_eps(t, last_actual, av_data,
                                   ortex_eps=_disp_eps,
                                   ortex_diluted_eps=_disp_deps,
                                   prev_spread=_spread)
    if resolved is NO_ADJUST:
        logger.warning(f"  {t}: proceeding WITHOUT non-GAAP adjustment for "
                       f"{last_actual} (AV NaN, no override)")
        return 'noadj'
    nongaap_basic, nongaap_diluted = resolved
    av_non_gaap_eps = nongaap_basic   # BASIC drives the per-share adjustment
    ...
    for metric, table in needed:
        ...
        if v is ACCEPT_GAAP:
            return 'gaap'
        try:
            _finite = v is not None and v is not ACCEPT_GAAP and math.isfinite(float(v))
        except (TypeError, ValueError):
            _finite = False
        if not _finite:
            logger.info(f"  {t}: '{metric}' unresolved or non-finite ({v!r}) "
                        f"-- deferring adjustments (retry); NaN must NEVER be "
                        f"written over stored values")
            return 'retry'
        vals[metric] = float(v)
    ...
    # eps <- validated non-GAAP BASIC ; dilutedEps <- validated non-GAAP DILUTED
    # (both independently validated at the two prompts; no blend, no mirror).
    adjusted_eps  = nongaap_basic
    adjusted_dEPS = nongaap_diluted
    ...
    print(f"    EPS adj (basic): {adjustment_per_share:+.4f}  ({ortex_eps:.4f} -> {adjusted_eps:.4f})")
    print(f"    dilutedEps     : {ortex_dEPS:.4f} -> {adjusted_dEPS:.4f}  [<- validated non-GAAP diluted (AV)]")
...
def should_skip_stock_v2(ticker: str, credits_tracker: Dict,
                         force_full: bool = False) -> Tuple[bool, Optional[str], Optional[Dict], Optional[str]]:
    t               = clean_ticker(ticker)
    ...
    # FEP is the single source of truth. We probe the FEP quarter and read the
    # PAYLOAD's estimatedValues to learn whether earnings released (payload flag
    # False = released). We never derive FEP from stored flags anymore -- FEP
    # advances only by consuming releases (below), and the stored flags are
    # reconciled to FEP after each advance via reconcile_flags_to_fep.
    ...
    # Reached here => FEP quarter is still an ESTIMATE (no release this run).
    # In EARNINGS-ONLY mode we skip such stocks entirely: no estimate re-fetch,
    # regardless of any forward totalRevenues forecast change. We only spend
    # time/credits on stocks that actually reported.
    if EARNINGS_ONLY:
        logger.info(f"{t} -- earnings-only: {first_estimated} still estimate, "
                    f"no release -> skipping stock")
        return True, "earnings_only_no_release", None, None
    ...
    if not materially_different(total_rev, last_rev):
        if force_full:
            logger.info(f"{t} -- FORCE MODE: totalRevenues unchanged in "
                        f"{first_estimated} but gate bypassed by user")
            return False, None, None, None
...
def process_ticker_v2(ticker: str, category: str, all_periods: List[str],
                       ...
                       force_full: bool = False) -> Tuple[bool, Optional[str]]:
    """
    ...
    # ONE FEP per stock: selection for EVERY category uses the income
    # frontier (cash/summary are mirrors, never independent).
    first_estimated = get_estimation_status(t, 'income')

    if force_full:
        # Corporate-action refetch: historical rows in the DB may be on a
        # pre-split scale; every period in range must be re-pulled so the
        # whole panel lands on Ortex's current (post-action) scale. The
        # same-day guard (fetched_today) stays -- rerunning force twice in
        # one day would only duplicate rows.
        logger.info(f"{t} | {category} -- FORCE MODE: ignoring fetched-ever "
                    f"history ({len(fetched_ever)} periods will be refetched)")
        fetched_ever = set()

    # FEP is authoritative; no detection/self-heal. Selection uses the stored
    # income FEP directly (cash/summary mirror it).
    if first_estimated is None:
    ...
    for period in sorted(periods_to_fetch):
        ...
        # INVARIANT: a RELEASED quarter (Ortex payload estimatedValues == False)
        # is an ACTUAL and belongs to the earnings-release path, which fetches
        # it AND applies the non-GAAP adjustment. This estimate-region path has
        # no adjustment logic, so writing a released quarter here would store
        # RAW, unadjusted values -- and if the quarter was previously adjusted
        # (e.g. FEP was wrongly regressed by one quarter onto an already-
        # adjusted actual), that raw write silently clobbers the adjustment.
        # Never write an actual from here. The release path handles it (this
        # run or next); adjustments can never be dropped by a redundant refetch.
        if not estimated:
            logger.warning(f"{t} | {category} | {period} -- Ortex reports ACTUAL "
                           f"(released); NOT saving raw from estimate path "
                           f"(release/adjust path owns it). Skipping.")
            continue
    ...
    if category == 'income':
        # FEP is NOT derived from flags anymore. It advances only in the
        # earnings-release path (main loop). Here we just reconcile the stored
        # flags to whatever FEP currently is, so they never drift, and copy
        # net-income estimates from the current frontier.
        current_fep = get_estimation_status(t, 'income')
        if current_fep:
            reconcile_flags_to_fep(t, current_fep)
            copy_normalized_to_netincome_estimates(t, current_fep, today)
...
def run_restatement_maintenance(stock_list: List[str], trailing_n: int,
    ...
    for idx, ticker in enumerate(stock_list, 1):
        t   = clean_ticker(ticker)
        fep = get_estimation_status(t, 'income')
        if fep is None:
...
# ==============================================================================
# CORRECTIVE RE-FETCH (mode 6)
# ==============================================================================

def run_corrective_refetch(stock_list: List[str],
                            credits_tracker: Optional[Dict] = None):
    """
    Corrective mode: UNCONDITIONALLY re-fetch the quarter BEFORE FEP (FEP-1,
    the most recently reported actual) for the named stocks, then re-apply the
    non-GAAP adjustment via the AV-suggestion prompt.

    Purpose: fix quarters whose non-GAAP EPS was written from BAD AlphaVantage
    data. Unlike restatement maintenance (mode 5), this does NOT gate on a
    revenue change -- AV corruption does not move revenue, so a gate would
    miss it. It always re-fetches FEP-1 and re-prompts.

    Per stock:
      1. FEP-1 = the last actual. Re-fetch income/cash/summary for it, saved
         under today's vintage (old vintages preserved -> PIT intact).
      2. Resolve the non-GAAP EPS via resolve_nongaap_eps (AV shown as an
         untrusted suggestion; user overrides with the correct value).
      3. Re-apply the adjustment (overwrites the bad non-GAAP figures) OR, on
         NO_ADJUST, leave the fresh GAAP actuals unadjusted.

    NEVER touches estimation_status (purely corrective on an existing actual).
    """
    today = pd.Timestamp(date.today())
    if credits_tracker is None:
        credits_tracker = {'used_this_round': 0, 'total_used': 0, 'remaining': None}

    print(f"\nCorrective re-fetch: {len(stock_list)} stock(s), quarter FEP-1")
    print(f"Estimated cost: ~{len(stock_list) * 3} credits (3 categories x FEP-1)")
    print("This OVERWRITES the last actual's data + re-prompts for non-GAAP EPS.")
    if input("Proceed? (y/n): ").strip().lower() != 'y':
        print("Cancelled.")
        return
    ...
    for idx, ticker in enumerate(stock_list, 1):
        ...
        fep = get_estimation_status(t, 'income')
        if fep is None:
            print(f"  [{idx}/{len(stock_list)}] {t}: no FEP -- skipping")
            logger.info(f"[corrective] {t}: no estimation status -- skipping")
            continue
        target = add_quarters(fep, -1)   # FEP-1 = last reported actual
        print(f"\n[{idx}/{len(stock_list)}] {t}: re-fetching {target} (FEP={fep})")

        # 1. re-fetch all three categories for FEP-1
        got_income = False
        for category in ['income', 'cash', 'summary']:
            response = fetch_fundamentals(t, target, category)
            if response is None:
                print(f"    {category}: no response")
                continue
            update_credits(credits_tracker,
                           response.get('creditsUsed', 0),
                           response.get('creditsLeft'))
            if not check_credits_and_confirm(credits_tracker):
                logger.info("[corrective] aborted by user")
                return
            data = response['data']
            if _is_empty_response(data):
                print(f"    {category}: empty response for {target}")
                continue
            estimated = data.get('estimatedValues', False)
            save_fundamentals_data(t, target, category, data, estimated, today)
            if category == 'income':
                got_income = True
            time.sleep(0.1)

        if not got_income:
            print(f"    {t}: no income data for {target} -- cannot re-adjust")
            logger.warning(f"[corrective] {t} {target}: no income -- skipped")
            continue

        # 2. fetch AV as a suggestion (may be the bad value -- shown, not trusted)
        av_data = fetch_av_earnings(t, ortex_period=target)
        if av_data is ACCEPT_GAAP:
            av_data = None   # normalize: resolver will show "unavailable"

        # 3. re-apply adjustment via the AV-suggestion prompt (overwrites)
        res = apply_nongaap_adjustments(t, target, av_data, today)
        print(f"    {t} {target}: re-adjustment result = {res}")
        logger.warning(f"[corrective] {t} {target}: result = {res}")
        corrected.append((t, target, res))
        time.sleep(0.1)

    print(f"\nCorrective re-fetch done. {len(corrected)} stock(s) processed:")
    for t, p, r in corrected:
        print(f"  - {t} {p}: {r}")
    logger.info(f"[corrective] complete | {corrected} | "
                f"credits used: {credits_tracker['total_used']:.2f}")
...
def main(stock_list: List[str], start_period: str, end_period: str,
    ...
    # Earnings-only focus: when ON, a stock is processed ONLY if its FEP
    # quarter has actually RELEASED (new actual to fetch + adjust). Stocks whose
    # only change is a forward totalRevenues forecast wiggle are skipped -- no
    # estimate re-fetch. Ideal during earnings season to spend time/credits on
    # new reports rather than forecast noise. Applies to the normal fetch modes
    # (1-4) only; the deliberate operations (5/6) are not offered under it.
    global EARNINGS_ONLY
    eo = input("\nFocus ONLY on new earnings releases? (y/N): ").strip().lower()
    EARNINGS_ONLY = (eo == 'y')

    print("\n" + "="*70)
    print("SELECT FETCH MODE" + ("   [EARNINGS-ONLY]" if EARNINGS_ONLY else ""))
    print("="*70)
    ...
    if not EARNINGS_ONLY:
        ...
        print("6. CORRECTIVE RE-FETCH -- re-pull FEP-1 + re-prompt non-GAAP EPS")
    print("="*70)

    valid = ['1', '2', '3', '4'] if EARNINGS_ONLY else ['1','2','3','4','5','6']
    prompt = ("\nSelect mode (1/2/3/4): " if EARNINGS_ONLY
              else "\nSelect mode (1/2/3/4/5/6): ")
    while True:
    ...
    # Single-ticker override: offer to bypass the revenue-change gate AND the
    # fetched-ever history guard, refetching the FULL period range. Use case:
    # corporate actions (e.g. splits -- KLAC) rescale Ortex's historical data
    # server-side while leaving FEP revenue estimates unchanged, so the gate
    # sees "nothing new" and the stale-scale history is never refreshed. Only
    # offered for single-name runs so it cannot be fat-fingered across the
    # universe.
    force_full = False
    if len(stock_list) == 1 and mode != 5:
        t_single = clean_ticker(stock_list[0])
        print(f"\nSingle-ticker run detected: {t_single}")
        raw = input("Force FULL fetch (bypass revenue-change gate + refetch "
                    "entire period range, e.g. after a split)? (y/n): ").strip().lower()
        if raw == 'y':
            force_full = True
            logger.info(f"{t_single} -- FORCE MODE: revenue gate bypassed, "
                        f"full period range will be refetched")
    ...
    if mode == 6:
        # Corrective: unconditionally re-fetch FEP-1 and re-adjust via the
        # AV-suggestion prompt. Fixes quarters with bad AV-sourced non-GAAP EPS.
        initialize_database()
        run_corrective_refetch(stock_list)
        return
    ...
    for idx, ticker in enumerate(filtered_stocks, 1):
        ...
        try:
            should_skip, skip_reason, av_data, last_actual = should_skip_stock_v2(t, credits_tracker, force_full=force_full)
        ...
        if av_data is not None and last_actual is not None:
            ...
            if av_data is ACCEPT_GAAP:
                ...
                elif adj_result == 'noadj':
                    print(f"  {t}: proceeding without non-GAAP adjustment (AV NaN, no override).")
        ...
        for cat_idx, category in enumerate(categories, 1):
            ...
            if last_actual is not None and adj_result in ('applied', 'gaap', 'noadj'):
                new_fep = get_next_period(last_actual)
                for cat in ['income', 'cash', 'summary']:
                    update_estimation_status(t, cat, new_fep)
                # Reconcile stored flags to the new FEP (single source of truth):
                # the just-released quarter (< new_fep) becomes actual, the rest
                # estimate. Keeps estimated_values consistent with FEP always.
                reconcile_flags_to_fep(t, new_fep)
                logger.info(f"{t} -- estimation_status advanced to {new_fep} "
                            f"({adj_result}); flags reconciled")
...
SOFTWARE_L = ['DDOG', 'SNOW', 'MDB', 'ZS', 'PLTR', 'RBRK', 'APP', 'CRWD', 'NET', 'TWLO', 'SHOP', 'AXON', 
              'ADSK', 'SNPS', 'CDNS', 'GTLB']
HARDWARE_L = ['NVDA', 'AVGO', 'MRVL', 'AMD', 'AMAT', 'LRCX', 'KLAC', 'MU', 'SNDK', 'LITE', 'AAOI', 'TER', 'VIAV', 
              'FORM', 'ALAB', 'CRDO', 'VRT', 'SITM', 'CLS', 'AXTI']
...
if __name__ == "__main__":
    ...
    stock_l = ['COHR', 'LITE']  # Replace with your ~700 ticker list
    # stock_l = HARDWARE_L
    START_PERIOD = '2018Q1'
    END_PERIOD = '2030Q4'    
