...
def run_daily_cache_build(
    ...
    """
    force_rebuild=False : incremental (safe daily)
    force_rebuild=True  : clear and rebuild all standard portfolios
    excl_only=True      : ONLY rebuild alpha_excl/mvo_excl/hybrid_excl.
                          Standard portfolios + triggers NEVER touched.
    ...
    if excl_only:
        if force_rebuild:
            print("  WARNING: force_rebuild ignored in excl_only mode.")
...
def _run_excl_only_cache_build(
    ...
    with _SuppressOutput():
        composite_by_date, _ = _cb_build_composite_scores(
            ...
            model_version=model_version, exclude_factors=['OU'], weights_by_date=None,
        )
    with _SuppressOutput():
        X_snapshots,_,_,_ = _mb_build_x_snapshots(
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    if _s9_rebal_only:
        ...
        if _past_s9:
            ...
            print(f"\n  Rebalance: {_ldt9.date()}  |  As of: {today_ts.date()}  |  {len(_wl9)} positions{_trig9}")
            print(f"  {'Ticker':<8}  {'Rebal Wt%':>10}  {'Curr Wt%':>10}  {'Drift':>8}  Sector")
            print("  " + "-" * 60)
        ...
        if comp_today is not None:
            ...
            if valid_snap_t:
                ...
                if not w_mvo_t.empty and w_mvo_t.sum() > 0:
                    ...
                    if _excl_t:
                        print(f"  Excluded ({len(_excl_t)}): {', '.join(sorted(_excl_t.keys()))}")
