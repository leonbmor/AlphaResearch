...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    try:
        ...
        if dyn_rebal_today and len(past_dyn) >= 2:
            ...
            # Hypothetical new portfolio = cached today portfolio with ADVP filter applied
            # This is exactly what an actual rebalance would produce
            try:
                ...
                _dd_today = _sh_nav_running / _sh_hwm - 1
                _reg_today = ('alpha' if _dd_today >= -SH_DD_ALPHA else
                              'hybrid' if _dd_today >= -SH_DD_HYBRID else 'mvo')
                _w_hyp_base = _p_today.get('mvo' if _reg_today == 'mvo' else _reg_today,
