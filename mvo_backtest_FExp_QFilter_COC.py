...
def _cb_build_composite_scores(universe, calc_dates, Pxs_df, sectors_s,
    ...
    for i, dt in enumerate(calc_dates):
        ...
        fundamental_parts   = []
        momentum_parts      = []
        coverage            = pd.Series(0, index=active_u)   # count of non-NaN factors
...
def run_daily_cache_build(
    ...
    for idx, dt in enumerate(to_compute):
        ...
        # Quality-based pre-filter — keep top prefilt_pct by quality score
        if prefilt_pct < 1.0 and dt in quality_wide.index:
            q_scores = quality_wide.loc[dt].dropna()
            n_keep   = max(n_cands, int(np.ceil(len(cands) * prefilt_pct)))
            q_keep   = q_scores.reindex(cands.index).dropna().nlargest(
                           min(n_keep, len(cands)))
            cands    = cands.reindex(q_keep.index).dropna()
        else:
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    quality_wide = _mb_load_quality_scores(all_tickers, calc_dates_idx)
...
def run_backtest(
    ...
    def _load_factor_scores_wide(calc_dates):
        ...
        # Idio Momentum — from v2_factor_residuals_quality
        try:
            with ENGINE.connect() as conn:
            ...
            scores['Idio_Mom'] = mom_ff.apply(_mb_zscore, axis=1)
        except Exception:
