...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    for i, dt in enumerate(calc_dates):
        ...
        if not w_mvo.empty and w_mvo.sum() > 0:
            ...
            # ── Turnover vs previous hybrid portfolio ─────────────────────────
            prev_hyb_dates = sorted([d for d in hybrid_weights_by_date if d < dt])
            if prev_hyb_dates:
                w_prev   = hybrid_weights_by_date[prev_hyb_dates[-1]]
                all_t_to = list(set(w_hyb.index) | set(w_prev.index))
                turnover = (w_hyb.reindex(all_t_to).fillna(0.0) -
            ...
            # Period: dt → next rebal date (or last Pxs_df date for live portfolio)
            ...
            if period_end in Pxs_df.index and dt in Pxs_df.index:
                px_start = Pxs_df.loc[dt,          [t for t in w_hyb.index if t in Pxs_df.columns]]
                px_end   = Pxs_df.loc[period_end,  [t for t in w_hyb.index if t in Pxs_df.columns]]
                stk_ret   = (px_end / px_start - 1).fillna(0.0)
                # Portfolio return (weighted), individual returns (unweighted) for stats
                port_ret  = (w_hyb.reindex(stk_ret.index).fillna(0.0) * stk_ret).sum()
                contrib_s = stk_ret   # unweighted individual stock returns
    ...
    print("\n  " + "=" * 72)
    print("  LIVE HYBRID PORTFOLIO -- P&L SINCE LAST REBALANCE")
    print("  " + "=" * 72)
    ...
    _live_pnl_hybrid(hybrid_weights_by_date, Pxs_df)

    # ── Current live portfolio (most recent rebalance, held today) ────────────
    print("\n  " + "=" * 72)
    print("  CURRENT LIVE PORTFOLIO (as of last rebalance)")
    print("  " + "=" * 72)
    today_ts    = Pxs_df.index[-1]
    past_rebals = sorted([d for d in hybrid_weights_by_date if d <= today_ts])
    if past_rebals:
        live_dt = past_rebals[-1]
        w_live  = hybrid_weights_by_date[live_dt]
    ...
    # ── Hypothetical rebalance: what would portfolio look like if today ─────
    print("\n  " + "=" * 72)
    print(f"  HYPOTHETICAL REBALANCE AS OF {today_ts.date()}")
    print("  " + "=" * 72)
    try:
        ...
        if comp_today is not None:
            ...
            if valid_snap_t:
                ...
                alpha_port_t = ranked_t.head(top_n).index.tolist()
                alpha_port_t = [t for t in alpha_port_t if t in pxs_cols]
                ...
                if not w_mvo_t.empty and w_mvo_t.sum() > 0:
                    ...
                    w_hyp      = (w_alpha_t.reindex(all_t_hyp).fillna(0.0) +
                                  w_mvo_t_nz.reindex(all_t_hyp).fillna(0.0)) / 2.0
                    w_hyp      = w_hyp[w_hyp > 0]
                    w_hyp      = w_hyp / w_hyp.sum()
                    w_hyp      = w_hyp.sort_values(ascending=False)

                    mvo_set_t   = set(w_mvo_t_nz.index)
                    alpha_set_t = set(w_alpha_t.index)
                    top_n_by_t  = mvo_diag_t.get('top_n_by', {})

                    n_common_t  = len(mvo_set_t & alpha_set_t)
                    n_ao_t      = len(alpha_set_t - mvo_set_t)
                    n_mo_t      = len(mvo_set_t  - alpha_set_t)
                    eff_n_t     = 1.0 / (w_hyp**2).sum()

                    print(f"\n  n={len(w_mvo_t_nz)} mvo / {len(w_hyp)} hybrid  "
                          f"eff_N={eff_n_t:.1f}  "
                          f"min={w_hyp.min():.1%}  max={w_hyp.max():.1%}")
                    print(f"  {n_common_t} common  |  {n_ao_t} alpha-only  |  {n_mo_t} mvo-only")
                    ...
                    print("  " + "-" * 95)
                    for tkr, wt in w_hyp.items():
                        sec  = sectors_s.get(tkr, '')
                        a    = alpha_ann_t.get(tkr, 0.0) * 100
                        v    = vol_ann_t.get(tkr,   0.0) * 100
                        z    = z_s_t.get(tkr,       0.0)
                        if tkr in (alpha_set_t - mvo_set_t):
                            src = 'alpha'
                        else:
                            in_m = '/'.join(m for m in ['E','L','F','P']
