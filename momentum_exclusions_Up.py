...
def run_exclusions(Pxs_df, pct_df, ou_scores_df,
    ...
    if force_rebuild:
        ...
        dates_to_compute = pd.DatetimeIndex(
            [d for d in all_dates if d not in processed or d == last_date])
        # Always wipe last date so it gets cleanly recomputed
        with ENGINE.begin() as conn:
            conn.execute(text(f"DELETE FROM {EXCLUSIONS_TBL} WHERE date = :d"),
                         {'d': last_date.date()})
            conn.execute(text(f"DELETE FROM {EXCLUSIONS_PROC_TBL} WHERE date = :d"),
                         {'d': last_date.date()})
        print(f"  Exclusions cache: {len(processed)} dates processed, "
              f"{len(dates_to_compute)} dates to compute "
              f"(last date always recomputed)")
    else:
        dates_to_compute = all_dates
