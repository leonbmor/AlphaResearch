...
all discussed and agreed:
  ...
  modes                : validate (subset -> side tables + diff/coverage report),
                         ...
                         resume (continue an interrupted rebuild from a given date,
                         keeping all previously written dates untouched),
...
TABLES          = ['valuation_metrics_v2']   # incremental now writes to v2
                                             # (anchors retired -- historical only)
VALIDATE_TABLES = ['valuation_metrics_anchors_validate']
# Reads (canonical calc-date set + diff baseline) now come from v2, so the
# incremental "already present?" check and the diff baseline use the live table.
DATE_SOURCE_TABLE = 'valuation_metrics_v2'
...
def run(Pxs_df, ed_df, vayf_df, exc_l, mode='validate', validate_tickers=None,
    ...
    mode='resume'      -> continue an interrupted rebuild: same inherited+thinned date
                          set as rebuild, but REBUILD_TABLE is NOT dropped and only
                          dates >= resume_date are (re)computed. All earlier dates are
                          preserved. Pass resume_date (a date-like). The resume date
                          itself is always recomputed (per-date writes are atomic, so
                          an interrupted date is absent-or-complete, never partial).
    ...
    if mode == 'rebuild':
        ...
        with ENGINE.begin() as conn:
            # SELF-REFERENCE GUARD: since the config repoint, DATE_SOURCE_TABLE
            # == REBUILD_TABLE == valuation_metrics_v2. A rebuild inherits its
            # schema AND its canonical date set FROM DATE_SOURCE_TABLE, but also
            # DROPs REBUILD_TABLE -- if they are the same table, the drop destroys
            # the very source being inherited from. Rebuild must therefore inherit
            # from a DIFFERENT table (e.g. the retired 'valuation_metrics_anchors'
            # or a backup). Fail loudly rather than corrupt.
            if DATE_SOURCE_TABLE == REBUILD_TABLE:
                raise RuntimeError(
                    f"[rebuild] DATE_SOURCE_TABLE and REBUILD_TABLE are both "
                    f"'{REBUILD_TABLE}'. Rebuild would DROP the table it inherits "
                    f"schema/dates from. Point DATE_SOURCE_TABLE at a historical "
                    f"table (e.g. 'valuation_metrics_anchors') for the rebuild run, "
                    f"then repoint back to v2 for incremental.")
            if insp_exists:
    ...
    if mode == 'resume':
        # NON-DESTRUCTIVE CONTINUATION of an interrupted rebuild. Same inherited+
        # thinned date set as rebuild, same FCF rebuild, same per-date atomic
        # writes to REBUILD_TABLE -- but the target table is NOT dropped (all
        # previously written dates are preserved) and only dates >= resume_date
        # are (re)computed. The resume date itself is always recomputed: per-date
        # writes are atomic (DELETE-then-INSERT in one txn), so an interrupted run
        # left that date either absent or complete -- never partial -- and
        # replace_date=True handles both cleanly.
        resume_date = pd.Timestamp(resume_date) if resume_date else None
        if resume_date is None:
            logger.error("[resume] no resume_date provided")
            return {}

        # Target MUST already exist -- resume continues a prior rebuild, never starts one.
        insp_exists = pd.read_sql_query(
            "SELECT to_regclass(%(t)s) AS t", ENGINE,
            params={'t': REBUILD_TABLE}).iloc[0]['t'] is not None
        if not insp_exists:
            logger.error(f"[resume] {REBUILD_TABLE} does not exist -- nothing to resume. "
                         f"Run mode='rebuild' to start a fresh build.")
            return {}

        # Identical inherited+thinned date set as rebuild, THEN filter to >= resume_date.
        # Filtering AFTER thin_dates guarantees the resumed dates are exactly the dates
        # rebuild would have produced from this point (same thinning boundaries).
        try:
            existing = pd.read_sql_query(f"SELECT DISTINCT date FROM {DATE_SOURCE_TABLE}", ENGINE)
            all_dates = sorted(pd.to_datetime(existing['date']).tolist())
        except Exception:
            all_dates = []
        if not all_dates:
            logger.warning("[resume] no existing dates found in prior table; nothing to do")
            return {}
        thinned = thin_dates(all_dates)
        dates   = [d for d in thinned if pd.Timestamp(d) >= resume_date]
        if not dates:
            logger.warning(f"[resume] no thinned calc dates on/after {resume_date.date()} "
                           f"-- nothing to resume (already complete?)")
            return {}

        # Report what is preserved vs (re)computed so the operator can sanity-check.
        preserved = [d for d in thinned if pd.Timestamp(d) < resume_date]
        logger.info(f"[resume] {REBUILD_TABLE} kept intact; "
                    f"{len(preserved)} prior date(s) preserved"
                    + (f" ({pd.Timestamp(preserved[0]).date()} .. {pd.Timestamp(preserved[-1]).date()})"
                       if preserved else "")
                    + f"; (re)computing {len(dates)} date(s) "
                    f"({pd.Timestamp(dates[0]).date()} .. {pd.Timestamp(dates[-1]).date()}) "
                    f"from resume_date={resume_date.date()}")

        # FCF is a prerequisite read; its builder replaces the table wholesale and is
        # date-independent, so rebuilding it reproduces the exact basis the original
        # run used. Idempotent and safe.
        all_tickers = [c for c in eligibility.columns]
        build_fcf_table(all_tickers, FCF_TABLE, restatement_df)
        ...
        for calc_date, mdf in _compute_dates(
                ...
                restatement_df, save=True, replace_date=True):
            out[calc_date] = mdf
        logger.info(f"[resume] done -- {len(out)} date(s) (re)written to {REBUILD_TABLE} "
                    f"from {resume_date.date()}. Prior dates untouched.")
        return out
...
if __name__ == "__main__":
    ...
    print("Modes: validate (subset->side tables) | rebuild (burn+regen) | "
          "resume (continue rebuild from a date) | incremental")
    print("=" * 80)
    mode = input("mode [validate/rebuild/resume/incremental]: ").strip().lower() or 'validate'
    ...
    if mode == 'validate':
        ...
        # Continue an interrupted rebuild: keep all prior dates, recompute from a date.
        _rd = input("resume_date [YYYY-MM-DD]: ").strip()
        try:
            _rd_ts = pd.Timestamp(_rd)
        except Exception:
            print(f"Could not parse '{_rd}' as a date. Cancelled.")
            _rd_ts = None
        if _rd_ts is not None:
            confirm = input(f"Resume rebuild into '{REBUILD_TABLE}' from {_rd_ts.date()} "
                            f"(prior dates kept)? (yes/n): ").strip().lower()
            if confirm == 'yes':
                results = run(Pxs_df, ed_df, vayf_df, DEFAULT_EXC_L,
                              mode='resume', resume_date=_rd_ts)
            else:
                print("Cancelled.")
    else:
