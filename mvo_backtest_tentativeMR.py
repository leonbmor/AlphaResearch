...
# Optional — momentum exclusions (strategy 9)
try:
    from momentum_exclusions import load_exclusions as _load_momentum_exclusions
    _MOMENTUM_EXCLUSIONS_AVAILABLE = True
except ImportError:
    _MOMENTUM_EXCLUSIONS_AVAILABLE = False
    def _load_momentum_exclusions(dt): return set()
...
def run_daily_cache_build(
    ...
    for idx, dt in enumerate(to_compute):
        ...
        # -- Save --------------------------------------------------------------
        try:
            # Build exclusion-filtered portfolios if available
            excl_dt = _load_momentum_exclusions(dt) if _MOMENTUM_EXCLUSIONS_AVAILABLE else set()
            if excl_dt:
                cands_excl   = cands[~cands.index.isin(excl_dt)]
                ...
                if n_excl_avail >= top_n:
                    ...
                    # Not enough stocks after exclusion — fall back to unfiltered
    ...
    try:
        ...
        if _has_excl and not trig_df.empty:
            ...
            for ri, rec in enumerate(_dyn_excl_rebal_log):
                ...
                excl_at_rebal = _load_momentum_exclusions(dt_r)
                if excl_at_rebal:
                    print(f"  *** Excluded ({len(excl_at_rebal)}): "
                          f"{', '.join(sorted(excl_at_rebal))}")
                if contrib_str:
                    print(f"  period [{dt_r.date()} -> {next_dt.date()}]{contrib_str}")
                ...
                for tkr, wt in w_r.sort_values(ascending=False).items():
                    sec = sectors_s.get(tkr, '')
                    excl_flag = ' [EXCL candidate]' if tkr in _load_momentum_exclusions(dt_r) else ''
