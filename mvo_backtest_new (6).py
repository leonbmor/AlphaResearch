...
def _mb_solve_mvo(dt, candidates, composite_scores, Pxs_df, sectors_s,
    ...
    # -- ETF sector-stress penalty --------------------------------------------
    # Shrink alpha for names whose mapped sector-ETF is in a flagged (stressed)
    # regime, so the optimizer rotates out of deteriorating sectors at
    # construction time. No-op when etf_penalty_scale >= 1.0 or inputs absent.
    if etf_penalty_scale < 1.0 and etf_stress_flags is not None and etf_map_s is not None:
        _pen = _etf_penalty_multiplier(dt, valid, etf_stress_flags, etf_map_s,
                                       etf_penalty_scale)
...
def run_backtest(
    ...
    def _build_alpha(dt, cands, excl_set=None):
        ...
        if (etf_penalty_scale < 1.0 and _etf_stress_flags is not None
                and not _etf_stress_flags.empty and etf_map_s is not None
                and not c.empty):
            _pen = _etf_penalty_multiplier(dt, list(c.index), _etf_stress_flags,
    ...
    def _check_penalty_edge(s_idx, dt):
        ...
        ETF sector-stress penalty NEWLY activated (0->1) since the last check. If so,
        set st['penalty_rebal_forced'] so _check_trigger forces an immediate rebalance
        this date, skipping the calendar/turnover gate.

        Asymmetric by design: only activation (0->1) triggers; a penalty going 1->0
        sets nothing (it relaxes naturally at the next scheduled rebalance). Held
        names are the strategy's actually-deployed weights. No-op when the penalty is
        disabled, flags are absent, or s_idx is not dynamic.
        """
        if (_etf_stress_flags is None or _etf_stress_flags.empty
                or etf_map_s is None or etf_penalty_scale >= 1.0):
            return
        ...
        if w is None or w.empty:
            ...
            return
        # names currently held whose mapped ETF is flagged on/before dt (PIT)
        _pen = _etf_penalty_multiplier(dt, list(w.index), _etf_stress_flags,
                                       etf_map_s, etf_penalty_scale)
            ...
            if _any_eligible:
                ...
                for s_idx in _dyn_list:
                    ...
                    _was_pen_forced = st.pop('_pen_newly_activated', None) \
                        if st.get('penalty_rebal_forced', False) else None
