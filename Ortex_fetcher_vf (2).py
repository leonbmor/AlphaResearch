...
def resolve_nongaap_eps(ticker: str, quarter: str, av_data: Optional[Dict],
    ...
    """AV non-GAAP EPS is a SUGGESTION, never ground truth (observed
    unreliable). Always display it -- alongside Ortex's GAAP eps and
    dilutedEps for the same quarter, so an implausible AV number (e.g. AV
    0.16 vs Ortex eps 0.72) is obvious at a glance -- then prompt and resolve:

        user types a number   -> OVERRIDE with that number (any AV state)
        user presses Enter:
            AV is a finite num -> use AV's number (including exactly 0.0)
            AV is NaN/missing  -> return NO_ADJUST (proceed w/o adjustment)

    Returns: a float (the non-GAAP EPS to use), or the NO_ADJUST sentinel.
    Distinguishes 'typed 0' from 'pressed Enter' on the RAW STRING, never on
    the numeric value.
    """
    av_raw = av_data.get('reportedEPS') if av_data else None
    try:
        av_val = float(av_raw)
        av_finite = math.isfinite(av_val)
    except (TypeError, ValueError):
        av_val, av_finite = None, False

    if av_finite:
        disp = f"AV suggests non-GAAP EPS = {av_val:.4f}" + (" (ZERO)" if av_val == 0 else "")
    else:
        disp = f"AV non-GAAP EPS UNAVAILABLE (NaN/missing)"
    print(f"\n  {ticker} | {quarter}: {disp}")

    # Ortex GAAP reference for a quick sanity comparison against AV.
    def _fmt(x):
        try:
            xf = float(x)
            ...
            return "n/a"
    e_s, de_s = _fmt(ortex_eps), _fmt(ortex_diluted_eps)
    print(f"    Ortex GAAP for comparison:  eps = {e_s}   dilutedEps = {de_s}")
    # flag an obvious order-of-magnitude divergence between AV and Ortex eps
    try:
        if av_finite and ortex_eps is not None:
            oe = float(ortex_eps)
            if math.isfinite(oe) and abs(oe) > 1e-9:
                ratio = av_val / oe
                if ratio > 2.0 or ratio < 0.5:
                    print(f"    ** NOTE: AV/Ortex eps ratio = {ratio:.2f}x "
                          f"-- large divergence, verify before accepting **")
    except (TypeError, ValueError):
        pass
    if av_finite:
        hint = ("Enter = accept AV, or type the correct non-GAAP EPS to "
                "override: ")
    else:
        hint = ("Enter = proceed with NO adjustment, or type the correct "
                "non-GAAP EPS: ")
    raw = input(f"    {hint}").strip()

    if raw != "":                       # user typed something -> OVERRIDE
        try:
            override = float(raw)
        except ValueError:
            print(f"    '{raw}' is not a number -- treating as no input.")
            raw = ""                    # fall through to Enter semantics
        else:
            if not math.isfinite(override):
                print(f"    non-finite override rejected -- no input.")
                raw = ""
            else:
                logger.info(f"  {ticker} | {quarter}: non-GAAP EPS OVERRIDE "
                            f"{override} (AV suggested {av_val!r})")
                return override

    # Enter pressed (or invalid input fell through)
    if av_finite:
        logger.info(f"  {ticker} | {quarter}: accepted AV non-GAAP EPS {av_val}")
        return av_val
    logger.info(f"  {ticker} | {quarter}: AV NaN and no override -> NO adjustment")
    return NO_ADJUST
...
def apply_nongaap_adjustments(ticker: str, last_actual: str,
    ...
    # show them next to AV's suggestion (a quick 3-number sanity check). These
    # are plain reads; the authoritative resolution happens in the loop below.
    ...
    # AV is a SUGGESTION, not ground truth. Resolve via the user prompt
    # (always shows AV + the Ortex GAAP figures, allows override, handles NaN).
    # NO_ADJUST means: keep the fresh GAAP actuals already saved, apply no
    # non-GAAP adjustment, and mark done ('noadj').
...
if __name__ == "__main__":
    ...
    # stock_l = ['AXTI']  # Replace with your ~700 ticker list
    stock_l = HARDWARE_L
    START_PERIOD = '2018Q1'
    END_PERIOD = '2030Q4'    
