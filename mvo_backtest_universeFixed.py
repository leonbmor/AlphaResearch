...
_MB_F_EWMA_HL   = 12    # EWMA half-life for F matrix (months)
...
def _mb_build_cov_matrices(dt, candidates, Pxs_df, sectors_s,
    ...
    # Keep stocks with >= _MIN_HISTORY_DAYS valid returns in the window
    # (not necessarily the full MVO_LOOKBACK) to include recent IPOs/re-listings
    ...
    # Fill remaining NaN with 0 for stocks with partial history in window
...
def _mb_solve_mvo(dt, candidates, composite_scores, Pxs_df, sectors_s,
    ...
    ret_raw = (Pxs_df.loc[:dt, candidates].pct_change()
               ...
               .iloc[-MVO_LOOKBACK:])
    min_obs  = min(_MIN_HISTORY_DAYS, len(ret_raw))
    ret_df   = ret_raw.loc[:, ret_raw.notna().sum() >= min_obs].fillna(0.0)
