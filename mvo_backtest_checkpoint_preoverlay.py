...
MOM_FILTER    = 2     # alpha/hybrid candidate pool = MOM_FILTER x top_n by composite momentum
...
def run_backtest(
    ...
    mom_filter          = MOM_FILTER,    # alpha/hybrid pool = mom_filter x top_n by momentum
):
    """
                    ...
                    if s_idx == S_DD:
                        ...
                        for _tkr in w_hyp.sort_values(ascending=False).index:
                            ...
                            print(f"  {_tkr:<8} {w_hyp[_tkr]*100:>7.2f}%  "
                                  f"{_sec:<28}  {exp_str} {_flag}", flush=True)
                        # Portfolio totals
                        tot_exp = '  '.join(
