...
def _advp_filter_and_replace(weights: pd.Series,
                              ...
                              max_weight: float,
                              top_n: int,
                              conc_factor: float = 1.0) -> pd.Series:
    """
    ...
    replacements = []
    for tkr in candidates.sort_values(ascending=False).index:
        if len(liquid) + len(replacements) >= top_n:
            break
        if tkr in liquid or tkr in illiquid or tkr not in pxs_cols:
            continue
        if _is_liquid(tkr):
    ...
    n     = len(final)
    if n == 0:
        return weights   # fallback — nothing survived

    # Rebuild weights: concentrated if conc_factor > 1, else equal weight
    n_top = int(np.ceil(n / 2))
    n_bot = n - n_top
    if conc_factor == 1.0 or n_bot == 0:
        w_new = pd.Series(1.0 / n, index=final)
    else:
...
def _select_hedge_instruments(dt:            pd.Timestamp,
    ...
    for ta in trigger_assets:
        if ta in scores:
            ...
            eff  = d['eff']  if not np.isnan(d.get('eff',  np.nan)) else 1.0  # NaN → neutral
...
def run_backtest(
    ...
    pxs_cols     = set(Pxs_df.columns)
    n_cands      = top_n * universe_mult
    ext_st       = start_date - pd.Timedelta(days=365)
    ...
    # ── 2. Build X snapshots (for MVO covariance) ────────────────────────────
    print("  Building X snapshots...")
    if mode == 'rebuild':
        _mb_clear_x_cache(model_version)
    with _SuppressOutput():
        X_snapshots, snapshot_dates = _mb_build_x_snapshots(
            rebal_dates   = calc_dates,
            Pxs_df        = Pxs_df,
            sectors_s     = sectors_s,
            volumeTrd_df  = volumeTrd_df,
            model_version = model_version,
    ...
    print(f"  X snapshots: {len(X_snapshots)}")
    ...
    state = [{
        ...
        'last_rebal':   None,
        'days_held':    0,
        'regime':       'alpha',      # alpha / hybrid / mvo
        'regime_days':  0,            # days in current regime (for persistence)
    ...
    def _apply_advp(w, cands, dt, strat_idx):
        if volumeRaw_df is None or w.empty: return w
        cur_aum = aum * state[strat_idx]['nav'] * state[strat_idx]['gross']
        if cur_aum <= 0: return w
        return _advp_filter_and_replace(
    ...
    def _record_cost(s_idx, w_new, w_old):
        to  = _turnover(w_new, w_old)
        c   = to * trading_cost_bps / 10000
        state[s_idx]['costs'] += c
        return c

    # ── Helper: determine regime from DD ─────────────────────────────────────
    def _dd_regime(dd, current_regime, persist_counter, exog_regime):
        """
        Determine portfolio regime based on drawdown thresholds.
        Applies hysteresis (exit thresholds differ from entry thresholds).
        exog_regime: exogenous rates regime from regime_s (alpha/hybrid/mvo)
        """
        # DD override takes priority over exogenous regime
        if dd <= -sh_dd_hybrid:
            target = 'mvo'
        elif dd <= -sh_dd_alpha:
            target = 'hybrid'
        else:
            target = exog_regime   # rates regime when DD is benign

        # Hysteresis: use exit thresholds when recovering
        if current_regime == 'mvo' and target != 'mvo':
            if dd > -(sh_dd_hybrid - sh_dd_exit_hybrid):
                target = 'hybrid'
            if dd > -(sh_dd_alpha - sh_dd_exit_alpha):
                target = exog_regime

        # Persistence filter
        if target != current_regime:
            persist_counter += 1
            if persist_counter >= sh_persist_days:
                return target, 0
            return current_regime, persist_counter
        return target, 0
    ...
    def _check_trigger(s_idx, dt, w_hyp, regime):
        ...
        if st['days_held'] < min_hold_days: return False
        if w_hyp.empty: return False

        # TO trigger
        to_thr = (to_thresh_alpha  if regime == 'alpha' else
                  to_thresh_hybrid if regime == 'hybrid' else to_thresh_mvo)
        # Drift current weights to today
        w_drift = _drift(st['weights'], st['last_rebal'], dt)
        to      = _turnover(w_hyp, w_drift)
        if to < to_thr: return False

        # Vol filter: don't trigger if portfolio vol has jumped significantly
        if volumeTrd_df is not None:
            pv = _portfolio_vol(w_hyp,    Pxs_df, dt)
            ov = _portfolio_vol(w_drift,  Pxs_df, dt)
            if ov > 0 and (pv - ov) / ov > voldiff_cap: return False

        return True

    # ── Helper: apply DD de-grossing ─────────────────────────────────────────
    def _apply_dd(s_idx, dt, daily_ret):
        """Update DD state and return gross exposure."""
        st = state[s_idx]
        # Update nav (gross nav before de-grossing)
        gross_nav = st['nav']
        hwm       = st['hwm']
        dd        = gross_nav / hwm - 1

        # Check de-gross triggers
        for lvl_idx, (thr, cut) in enumerate(dd_levels):
            if dd <= -thr and st['dd_level'] < lvl_idx:
                old_gross = st['gross']
                st['gross'] *= (1 - cut)
                st['dd_level'] = lvl_idx
                st['regime']   = dd_level_regime[lvl_idx]
                st['dd_log'].append({
                    'dt': dt, 'event': f'DE-GROSS lv{lvl_idx+1}',
                    'dd': dd, 'exp_from': old_gross, 'exp_to': st['gross']
                })
                break

        # Check re-entry
        if st['dd_level'] >= 0:
            recovery = gross_nav / (hwm * (1 - dd_levels[st['dd_level']][0])) - 1
            if recovery >= dd_reentry_pct:
                st['reentry_count'] += 1
                if st['reentry_count'] >= dd_reentry_confirm:
                    ...
                    st['dd_log'].append({
                        ...
                        'dd': recovery, 'exp_from': old_gross, 'exp_to': 1.0
                    })
            else:
                st['reentry_count'] = 0
    ...
    def _update_hedge(s_idx, dt, port_w, port_nav):
        """Check hedge triggers and update hedge account."""
        if not hedge_enabled: return
        st = state[s_idx]
        if dt not in Pxs_df.index: return

        # Check trigger assets for hedge signal
        prev_dates = [d for d in trading_days if d < dt]
        if len(prev_dates) < 2: return
        prev_dt = prev_dates[-1]

        trigger_fired = False
        for ta in hedge_trigger_assets:
            if ta not in Pxs_df.columns: continue
            r = Pxs_df.loc[dt, ta] / Pxs_df.loc[prev_dt, ta] - 1
            if r < -0.02:   # 2% daily drop triggers hedge
                trigger_fired = True
                break

        # If hedged and trigger no longer firing, close hedges
        if st['active_hedges'] and not trigger_fired:
        ...
        # Open new hedges if trigger fired and not already hedged
        if trigger_fired and not st['active_hedges']:
            port_ret_s = _portfolio_returns(
                {d: state[S_DYN]['weights'] for d in prev_dates[-BETA_WINDOW:]},
                Pxs_df,
            ) if state[S_DYN]['weights_by_date'] else pd.Series(dtype=float)

            if not port_ret_s.empty:
                insts = _select_hedge_instruments(
                    dt=dt, Pxs_df=Pxs_df, port_ret_s=port_ret_s,
                    hedge_assets=hedge_trigger_assets,
                    eff_floor=eff_floor, corr_floor=corr_floor,
                    hedge_ratio=hedge_ratio, max_hedge=max_hedge,
                    eff_mav_window=EFF_MAV_WINDOW,
                )
                for inst, h in insts.items():
    ...
    for day_idx, dt in enumerate(trading_days):
        ...
        if is_calc_date:
            ...
            if comp is not None:
                ...
                _record_cost(S_BASE, w_bl, state[S_BASE]['weights'])
                state[S_BASE]['weights'] = w_bl
                state[S_BASE]['weights_by_date'][dt] = w_bl.copy()
                state[S_BASE]['last_rebal'] = dt

                # -- Pure Alpha (S1) --------------------------------------
                w_a1 = _apply_advp(w_alpha, cands, dt, S_ALPHA)
                _record_cost(S_ALPHA, w_a1, state[S_ALPHA]['weights'])
                state[S_ALPHA]['weights'] = w_a1
                state[S_ALPHA]['weights_by_date'][dt] = w_a1.copy()
                state[S_ALPHA]['last_rebal'] = dt

                # -- MVO (S2) ---------------------------------------------
                w_m2 = _apply_advp(w_mvo, cands, dt, S_MVO)
                _record_cost(S_MVO, w_m2, state[S_MVO]['weights'])
                state[S_MVO]['weights'] = w_m2
                state[S_MVO]['weights_by_date'][dt] = w_m2.copy()
                state[S_MVO]['last_rebal'] = dt

                # -- Hybrid (S3) ------------------------------------------
                w_h3 = _apply_advp(w_hyb, cands, dt, S_HYB)
                _record_cost(S_HYB, w_h3, state[S_HYB]['weights'])
                ...
                w_s4   = _apply_advp(w_s4, cands, dt, S_SMART)
                _record_cost(S_SMART, w_s4, state[S_SMART]['weights'])
        ...
        if last_calc_dt is not None and last_calc_dt in composite_by_date:
            ...
            # Load cached portfolios for this calc_date
            _cached_dyn = {}
            ...
            for s_idx in [S_DYN, S_HEDGE, S_DD, S_EXCL]:
                ...
                # Apply ADVP using this strategy's own AUM
                w_hyp = _apply_advp(w_hyp, cands_s, last_calc_dt, s_idx)

                # Check rebalance trigger
                if _check_trigger(s_idx, dt, w_hyp, reg):
                    to   = _turnover(w_hyp, _drift(st['weights'],
                                                    st['last_rebal'] or dt, dt))
                    cost = _record_cost(s_idx, w_hyp, st['weights'])
                    st['weights']          = w_hyp
        ...
        exog = _exog_regime(dt)
        for s_idx in range(N_STRAT):
            st = state[s_idx]
            new_reg, persist = _dd_regime(
                st['dd'], st['regime'], st['regime_days'], exog)
            st['regime']      = new_reg
            st['regime_days'] = persist
        ...
        # -- Apply DD de-grossing for S7/S8 -----------------------------------
        for s_idx in [S_DD, S_EXCL]:
            if prev_dt is not None:
                _apply_dd(s_idx, dt, 0)  # updates gross exposure in state
    ...
    # Yearly returns
    _aum_nav = nav_series[S_DD]
    print(f"\n  Yearly returns  (starting AUM: ${aum/1e6:.1f}M)")
    hdr = (f"  {'Year':<6} {'Baseline':>10} {'Pure Alpha':>12} {'MVO':>9} "
           f"{'Hybrid':>9} {'Smart':>9} {'Dynamic':>9} {'Dyn+Hdg':>9} "
           f"{'DD Pol':>8} {'Excl':>8} {'AUM($M)':>9}")
    print(hdr)
    print("  " + "-" * (len(hdr) - 2))
    ...
    for yr in all_years:
        ...
        rets  = [yr_ret(nav_series[s]) for s in range(N_STRAT)]
        nav_to_yr = _aum_nav[_aum_nav.index.year <= yr]
        closing   = aum * float(nav_to_yr.iloc[-1]) if not nav_to_yr.empty else aum
        def fmt(r): return f"{r:>+8.2f}%" if not np.isnan(r) else f"{'n/a':>9}"
        print(f"  {yr:<6} {fmt(rets[0])} {fmt(rets[1])} {fmt(rets[2])} "
              f"{fmt(rets[3])} {fmt(rets[4])} {fmt(rets[5])} "
              f"{fmt(rets[6])} {fmt(rets[7])} {fmt(rets[8])}  "
              f"${closing/1e6:>7.1f}M")
    ...
    # Collect old weights (drifted to today from prev rebal)
    _old_w = {}
    _new_w = {}
    for lbl, s_idx in _dyn_strats:
        st  = state[s_idx]
        ldt = st['last_rebal']
        if ldt is None: continue
        _new_w[lbl] = st['weights']
        # Find previous rebal
        prev_rebals = [r for r in st['rebal_log'] if r['dt'] < ldt]
        if prev_rebals:
            pr     = prev_rebals[-1]
            w_prev = pr['w']
            w_old  = _drift(w_prev, pr['dt'], ldt)
        else:
            w_old = pd.Series(dtype=float)
        _old_w[lbl] = w_old

    all_trade_tks = set()
    for lbl in [l for l, _ in _dyn_strats]:
        if lbl in _new_w: all_trade_tks |= set(_new_w[lbl].index)
        if lbl in _old_w: all_trade_tks |= set(_old_w[lbl].index)

    deltas = {}
    for lbl in [l for l, _ in _dyn_strats]:
        deltas[lbl] = {}
        if lbl not in _new_w: continue
        for tkr in all_trade_tks:
            n = _new_w[lbl].get(tkr, 0.0)*100
            o = _old_w.get(lbl, pd.Series()).get(tkr, 0.0)*100
            d = n - o
            if abs(d) >= 0.01:
                deltas[lbl][tkr] = d
    ...
    if all_delta_tks:
        col_w = max(len(l) for l in deltas) + 2
        print(f"\n  {'Ticker':<8}" +
              "".join(f"  {l:>{col_w}}" for l in deltas))
        print("  " + "-"*8 + ("  " + "-"*col_w)*len(deltas))
        for tkr in all_delta_tks:
            row = f"  {tkr:<8}"
            for lbl in deltas:
                d = deltas[lbl].get(tkr, 0.0)
                row += f"  {d:>+{col_w-1}.2f}%" if abs(d) >= 0.01 else f"  {'':>{col_w}}"
            print(row)
