...
def _apply_advp_cap(weights: pd.Series,
    ...
    for tkr in tickers:
        ...
        if len(common) < 3:
            adv_cap_w[tkr] = max_weight
            continue
        dollar_vol = (px_s.reindex(common) * vol_s.reindex(common)).median()
    ...
    for _ in range(20):
        ...
        capped_set |= newly_capped
        if excess < 1e-6:
            break
        uncapped = [t for t in w.index if t not in capped_set]
        if not uncapped:
            break
