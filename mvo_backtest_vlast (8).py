...
# ── ETF sector-stress alpha penalty ──────────────────────────────────────────
# When a stock's mapped sector-ETF is flagged as being in a high-vol / stressed
# regime (per the per-ETF HMM+CUSUM signals built by etf_regime_signals.py), that
# stock's alpha is multiplied by ETF_PENALTY_SCALE, shrinking its appeal to the
# optimizer so the book rotates out of deteriorating sectors at construction time
# (upstream of the reactive macro hedge / risk-reduction). Set to 1.0 to DISABLE
# the scheme entirely (multiplier becomes a no-op). Range [0, 1]; 0.5 = halve the
# alpha of flagged-sector names.
ETF_PENALTY_SCALE    = 0.5
# SECOND LAYER: a finer sub-sector mapping (30 floored buckets) provides an
# independent stress read per stock, combined with the broad-ETF layer as a
# PRODUCT of alpha-multipliers: alpha_i *= (etf_mult_i * subsec_mult_i). A stock
# flagged by its ETF gets *= ETF_PENALTY_SCALE, flagged by its sub-sector gets
# *= SUBSECTOR_PENALTY_SCALE, flagged by both gets *= (product). Each layer is
# INDEPENDENTLY DISABLED at scale 1.0: a scale of 1.0 means the layer is ignored
# ENTIRELY -- its multiplier is 1.0 for every stock AND it triggers no off-cycle
# rebalances (a disabled layer must be fully inert, so setting
# SUBSECTOR_PENALTY_SCALE=1.0 reproduces the ETF-only behaviour exactly).
SUBSECTOR_PENALTY_SCALE = 1.0  # 1.0 = sub-sector layer OFF (backward-compatible)

# ── Sector / sub-sector allocation caps ──────────────────────────────────────
# Hard caps on the TOTAL weight of any one sector / sub-sector, enforced as a
# post-processing step on the final weights of EVERY strategy (the alpha sleeve
# never goes through the optimiser, so this cannot be a solver constraint).
# 1.0 = 100% = cap disabled.
#
# When a bucket breaches, its names are scaled down PRO-RATA to the cap (the
# relative conviction ordering inside the bucket is preserved) and the freed
# weight is redistributed PRO-RATA across names that are not in a capped bucket
# and not at the individual cap. The process iterates, because redistribution
# can push another bucket or name through its own limit. Any residual that has
# nowhere to go is held in SPX (netted against an SPX hedge if one is on).
#
# This is a CONSTRUCTION-TIME constraint only: later price drift may push a
# bucket above its cap and that does NOT trigger a rebalance -- it simply shows
# up as turnover in the next calculated portfolio, helping to trip the normal
# turnover trigger.
SECTOR_CAP    = 0.35     # max total weight per sector      (1.0 = off)
SUBSECTOR_CAP = 0.25     # max total weight per sub-sector  (1.0 = off)
CAP_RESIDUAL_TICKER = 'SPX'   # where un-redistributable residual is parked

# ── FACTOR-regime penalty (HMM/CUSUM on the factor-return lambdas) ────────────
# Per-factor activation signal built INSIDE the backtest from the raw factor_v1
# HMM/CUSUM caches (run_etf_signals on the factor level series). When a factor's
# activation signal is ON, that factor's weight w_t is multiplied by its scale
# BEFORE the composite is summed and z-scored (the z-score then redistributes the
# freed weight pro-rata across the other factors).
#
# Scales are PER FACTOR (composite factor names). 1.0 = that factor's signals are
# IGNORED ENTIRELY (no weight change, no rebalance trigger, flags not built).
FACTOR_PENALTY_SCALES = {'Mom_12M1': 1.0, 'Idio_Mom': 1.0,
                         'Quality': 1.0, 'Value': 1.0}
# factor_v1 cache asset name -> composite factor name
FACTOR_NAME_MAP = {'12M1 Mom': 'Mom_12M1', 'Idio Momentum': 'Idio_Mom',
                   'Quality': 'Quality', 'Value': 'Value'}
# Factors whose activation (0->1) forces an off-cycle rebalance (dynamic
# strategies). De-activation never forces anything. Also requires scale < 1.0.
FACTOR_REBAL_TRIGGER = {'Mom_12M1'}

FACTOR_SHORT_WIN = 1          # HMM/CUSUM window_years for the short leg input
FACTOR_LONG_WIN  = 3          # HMM/CUSUM window_years for the long leg input

# Leg model selection (numbering as in the research notebook). The building
# blocks keep the RISK-ON convention (1 = calm); only each leg's FINAL output is
# inverted into an ACTIVATION signal (1 = penalise) before the legs are combined.
#   1 _AND      memoryless, short AND long windows calm
#   2 _OR       memoryless, short OR long window calm
#   3 /TA_AND   1 + trend veto          4 /TA_OR    2 + trend veto
#   5 _ST_AND   state machine, AND      6 _ST_OR    state machine, OR
#   7 /TA_ST_AND  5 + trend veto        8 /TA_ST_OR 6 + trend veto
FACTOR_HMM_MODEL   = 5
FACTOR_CUSUM_MODEL = 1
# How the two ACTIVATION signals combine (risk-off convention):
#   'AND' -> penalise only if BOTH the HMM and the CUSUM leg are activated
#   'OR'  -> penalise if EITHER leg is activated
FACTOR_LEG_COMBINE = 'AND'

# HMM leg -- input: 100 * p_low (1y / 3y windows)
FACTOR_HMM_MAV1      = 5       # fast EWM halflife on the input
FACTOR_HMM_MAV2      = 20      # slow EWM halflife on the input
FACTOR_HMM_THRESH_UP = 0.0     # calm when fast-slow >= this (memoryless: >)
FACTOR_HMM_THRESH_DN = -2.5    # state machine: risk-off when fast-slow < this
FACTOR_HMM_MAVTA1    = 5       # trend gate fast EWM halflife (factor level)
FACTOR_HMM_MAVTA2    = 25      # trend gate slow EWM halflife
FACTOR_HMM_TA_THRESH = 1.0     # trend risk-off when fast/slow < this

# CUSUM leg -- input: 100 * (1 - inc_up) (1y / 3y windows)
FACTOR_CUSUM_MAV1      = 5
FACTOR_CUSUM_MAV2      = 20
FACTOR_CUSUM_THRESH_UP = 0.0
FACTOR_CUSUM_THRESH_DN = -5.0
FACTOR_CUSUM_MAVTA1    = 5
FACTOR_CUSUM_MAVTA2    = 25
FACTOR_CUSUM_TA_THRESH = 1.0
# Signal-construction knobs (the binary flag is built INSIDE this script from the
# raw cached HMM/CUSUM signals, so these fast-moving knobs tune without re-running
# the slow overnight detection). Mirror the prototype: self-normalised p_low
# MA-cross (fast/slow EWM halflives), threshold margin, short+long window AND.
ETF_MAV1             = 5      # fast EWM halflife on p_low (short reactivity)
ETF_MAV2             = 20      # slow EWM halflife on p_low (baseline)
ETF_THRESH           = 0.0     # MA-cross margin before flagging (0 = stricter HMM leg)
ETF_SHORT_WIN        = 1       # HMM/CUSUM window_years for the fast leg
ETF_LONG_WIN         = 3       # HMM/CUSUM window_years for the slow (confirming) leg
ETF_TA_FAST          = 5       # price trend fast MA (trading days)
ETF_TA_SLOW          = 20      # price trend slow MA (trading days)
ETF_VOL_COMBINE      = 'AND'   # how to combine the HMM and CUSUM vol legs: 'AND'
                               # (both detectors must flag stress -- restrictive) or
                               # 'OR' (either detector -- looser, more firing). The
                               # price/trend gate is ANDed on top either way, so the
                               # full rule is (HMM <combine> CUSUM) AND downtrend.
...
# ── HMM regime overlay (exogenous vol-state consensus) ────────────────────────
# Filtered state probabilities from the vol-quintile HMM (hmm_vol_regimes.py
# cache). Consensus = ALL THREE windows (1y/3y/5y) agree, sustained MIN_SP
# consecutive days (backward-looking -> PIT).
#
# ARCHITECTURE (two independent dials):
#  EXPOSURE (fast, revocable) -- DD ladder tracking runs fully independent of
#    HMM. While in the risk-reduction zone, a sustained p_low consensus grants
#    a REVOCABLE re-gross ("parole"): gross back to 100% only while the
#    consensus holds; the day it breaks, gross snaps back to the ladder level.
#    The PERMANENT zone exit remains the theo-NAV recovery rule, additionally
#    BLOCKED while a p_high consensus is active (mirror rule -- don't re-enter
#    on a bear-market rally the vol state calls unstable).
#  CONSTRUCTION (slow, gated) -- the DD zone GATES alpha: in-zone construction
#    is pinned 'mvo' unconditionally (p_low restores exposure, not aggression).
#    Out of zone: p_high consensus -> 'mvo' (conservative override, always
#    honored); p_low consensus -> 'alpha'; else the DD-threshold rule.
#  Full aggression (alpha at 100%) therefore requires BOTH the HMM (calm) and
#  the DD machine (recovery confirmed) to agree. Non-ladder regime consumers
#  (Smart/Dynamic/Dyn+Hedge) run a VIRTUAL zone: same entry (rolling-dd breach
#  of lv1) and exit (theo recovery + mirror rule) driving the same construction
#  gate, with no gross action.
HMM_REGIME_OVERLAY  = True              # master switch
HMM_ASSET           = 'CUSTOM_VOLQ5'    # cache asset whose state probs are used
HMM_MODEL_VER       = 'gauss3_v1'       # must match hmm_vol_regimes MODEL_VERSION
HMM_TBL             = 'hmm_regime_cache'
HMM_WINDOWS         = (1, 3, 5)         # all must agree (consensus filter)
HMM_THRESH_HIGH     = 0.20              # p_high consensus threshold .3
HMM_THRESH_LOW      = 0.20              # p_low  consensus threshold .3
MIN_SP              = 5                 # consensus must hold N consecutive days
HMM_PAROLE          = True              # p_low consensus -> revocable re-gross in-zone
HMM_BLOCK_REENTRY   = True              # p_high consensus blocks the permanent zone exit
# After a parole is REVOKED, no parole may be re-granted for this many trading
# days (scope below). Rationale: both tiers can re-grant on STALE evidence --
# the 30d CUSUM down-latch keeps the fast path armed, and a one-day fast-HIGH
# suppression of lo_eff revokes while the slow consensus stays True, so the slow
# path re-grants the very next day. Either way gross exposure flip-flops daily.
# The cooldown forces parole to wait for genuinely new regime evidence; 5d is
# about what p_low needs to settle across windows. Set 0 to disable.
PAROLE_COOLDOWN_DAYS = 5
# Which grant paths the cooldown gates:
#   'all'  -> NO parole may be re-granted within the cooldown (recommended).
#             Needed because a revocation can be driven by the FAST HIGH tier
#             (up-latch AND a single window above p_high, with no persistence
#             requirement) suppressing lo_eff for ONE day while the slow
#             consensus stays True -- the next day the slow path re-grants
#             immediately, flip-flopping gross exposure day to day.
#   'fast' -> only the CUSUM-latch fast path is gated; a slow multi-day
#             consensus may re-grant during the cooldown.
PAROLE_COOLDOWN_SCOPE = 'all'

# ── CUSUM fast tier (two-tier trigger machinery) ──────────────────────────────
# The HMM consensus above is the SLOW tier: 3-window agreement sustained MIN_SP
# days (temporal persistence as the noise filter). The CUSUM tier is the FAST
# path: cross-model corroboration replaces temporal persistence. If any CUSUM
# window raises an alarm (cusum_vol_regimes cache, same asset), the alarm is
# latched active for CUSUM_ACTIVATION_WINDOW trading days (renewed by same-
# direction alarms, killed by an opposite-direction alarm). While latched, a
# SINGLE HMM window with the corresponding raw probability above threshold --
# no MIN_SP wait -- completes the composite flag:
#   hi_eff = hi_slow OR (cusum_up_latched  AND any-window p_high > THRESH_HIGH)
#   lo_eff = lo_slow OR (cusum_dn_latched  AND any-window p_low  > THRESH_LOW)
#   (conservative side wins every conflict: lo_eff suppressed while hi_eff)
# The composites feed the SAME v2 machinery (construction chain, parole,
# mirror rule) -- every action inherits the acceleration, incl. fast parole
# revocation via hi_eff suppressing lo_eff. CUSUM has no standalone authority:
# with HMM_REGIME_OVERLAY off, the fast tier is inert.
CUSUM_REGIME_OVERLAY    = True          # master switch for the fast tier
CUSUM_MODEL_VER         = 'cusum_v1'
CUSUM_TBL               = 'cusum_regime_cache'
CUSUM_ACTIVATION_WINDOW = 30             # trading days an alarm stays latched 10
CUSUM_MIN_WINDOWS       = 1             # windows that must be latched (same
                                        # direction) for the fast key to turn:
                                        # 1 = any-of-3 (default -- the HMM
                                        # corroboration key is the noise filter)
                                        # 2 = stricter 2-of-3 latched variant
...
def _cb_build_composite_scores(universe, calc_dates, Pxs_df, sectors_s,
    ...
    composite_by_date = {}
    weights_used = {}     # dt -> {'pre': {...}, 'post': {...}} factor weights
    n = len(calc_dates)
    _diag_dt = calc_dates[0] if len(calc_dates) > 0 else None
    for i, dt in enumerate(calc_dates):
        if (i + 1) % 250 == 0 or i == 0 or i == n - 1:
            print(f"  Building composite [{i+1}/{n}] {dt.date()}", flush=True)
        ...
        _w_pre_pen = w_t.copy()      # weights after the Quality floor, pre-penalty
        # ── FACTOR-regime penalty: scale the weight of any factor whose
        # activation flag is ON at dt (PIT: last flag on/before dt). Applied
        # AFTER the Quality floor so the floor can't undo it; the composite's
        # final z-score redistributes the freed weight pro-rata.
        if factor_flags is not None and not factor_flags.empty and factor_scales:
            _ff = factor_flags.loc[:dt]
            if not _ff.empty:
                _frow = _ff.iloc[-1]
                _fpen = {f: sc for f, sc in factor_scales.items()
                         if sc < 1.0 and f in w_t.index and f in _frow.index
                         and int(_frow[f]) == 1}
                if _fpen:
                    w_t = w_t.copy()
                    for f, sc in _fpen.items():
                        w_t[f] = w_t[f] * sc
        weights_used[dt] = {'pre':  {k: float(v) for k, v in _w_pre_pen.items()},
                            'post': {k: float(v) for k, v in w_t.items()}}
    ...
    return composite_by_date, score_dfs, weights_used
...
def _make_composite_cache_key(universe, model_version, factor_pen_sig=None):
    """
    ...
    # Factor-regime penalty changes historical composites, so its configuration
    # is part of the key -- but ONLY when some factor is actually penalised
    # (scale < 1.0). With every scale at 1.0 the key is unchanged, so the
    # existing frozen cache stays valid.
    if factor_pen_sig is not None:
        params['fpen'] = factor_pen_sig
    return hashlib.md5(
...
def _ensure_composite_cache_table():
    with ENGINE.begin() as conn:
        ...
        """))
        # factor weights actually used at each date (pre/post factor penalty),
        # added later -> idempotent ALTER so existing caches keep working
        conn.execute(text(f"ALTER TABLE {MB_COMPOSITE_CACHE_TBL} "
                          f"ADD COLUMN IF NOT EXISTS weights_json TEXT"))
...
def _load_composite_cache(cache_key, before_dt):
    ...
    Returns (composite_by_date, score_dfs, set_of_cached_dates, weights_used).
    """
    try:
        _ensure_composite_cache_table()
        with ENGINE.connect() as conn:
            rows = conn.execute(text(f"""
                SELECT date, composite_json, scores_json, weights_json
    ...
    for r in rows:
        ...
        if len(r) > 3 and r[3]:          # NULL on rows cached before this column
            weights_used[dt] = json.loads(r[3])

    score_dfs = {}
    for fname, by_dt in _factor_rows.items():
        score_dfs[fname] = pd.DataFrame.from_dict(by_dt, orient='index').sort_index()

    return composite_by_date, score_dfs, set(composite_by_date.keys()), weights_used
...
def _save_composite_cache(cache_key, dates, composite_by_date, score_dfs,
    ...
    with ENGINE.begin() as conn:
        for dt in dates:
            ...
            _wj = (json.dumps((weights_used or {}).get(dt))
                   if (weights_used or {}).get(dt) is not None else None)
            conn.execute(text(f"""
                INSERT INTO {MB_COMPOSITE_CACHE_TBL}
                    (cache_key, date, composite_json, scores_json, weights_json)
                VALUES (:k, :dt, :cj, :sj, :wj)
            ...
            """), {'k': cache_key, 'dt': pd.Timestamp(dt).strftime('%Y-%m-%d'),
                   'cj': comp_j, 'sj': json.dumps(sc), 'wj': _wj})
...
def _make_make_params_hash(ic, max_weight, min_weight, zscore_cap, pca_var_threshold,
    ...
    # Individual sector/sub-sector caps and universe exclusions change the
    # resulting portfolios, so they must key the NAV/state/rebal caches -- but
    # ONLY when set, so an unconstrained run keeps its existing hash and cache.
    if group_caps:
        params['gcaps'] = json.loads(json.dumps(group_caps, sort_keys=True, default=str))
    return hashlib.md5(
...
# ===============================================================================
# ETF SECTOR-STRESS SIGNAL CONSTRUCTION
# ===============================================================================

def prompt_group_caps(sectors_s, subsec_map_s=None,
                      sector_cap=None, subsector_cap=None):
    """
    Interactive helper: per-sector / per-sub-sector allocation caps.

    Lists every sector (and sub-sector) with a number; the user picks a number,
    then enters a cap in [0, 1]:
        empty at the NUMBER prompt -> finish this stage
        empty at the VALUE  prompt -> skip that bucket
        0                          -> EXCLUDE the bucket: its stocks are removed
                                      from the universe BEFORE anything is built
                                      (a universe filter, not a post-hoc overlay)
        0 < v < 1                  -> cap, applied after construction like the
                                      global caps
    A value above the prevailing global cap cannot loosen it (effective cap is
    min(global, user)), so the user is warned and asked to confirm.

    Returns {'sector_caps': {...}, 'subsector_caps': {...}} ready to pass to
    run_backtest. Buckets left untouched simply don't appear.
    """
    sector_cap    = SECTOR_CAP    if sector_cap    is None else sector_cap
    subsector_cap = SUBSECTOR_CAP if subsector_cap is None else subsector_cap
    out = {'sector_caps': {}, 'subsector_caps': {}}

    stages = [('sector', sectors_s, sector_cap, 'sector_caps')]
    if subsec_map_s is not None:
        stages.append(('sub-sector', subsec_map_s, subsector_cap, 'subsector_caps'))

    for label, m, gcap, key in stages:
        if m is None:
            continue
        names = sorted(pd.Series(m).dropna().unique())
        if not names:
            continue
        print("\n" + "=" * 66)
        print(f"  {label.upper()} CAPS   (global cap = {gcap:.0%})")
        print("=" * 66)
        for i, nm in enumerate(names, 1):
            print(f"  {i:>3}. {nm}")
        print("=" * 66)
        print("  Enter a NUMBER to set its cap, or empty to move on.")
        while True:
            raw = input(f"  {label} number (empty = done): ").strip()
            if raw == "":
                break
            if not raw.lstrip('+-').isdigit() or not (1 <= int(raw) <= len(names)):
                print("  Invalid selection")
                continue
            nm = names[int(raw) - 1]
            while True:
                vraw = input(f"    cap for {nm} (0-1, empty = skip): ").strip()
                if vraw == "":
                    break
                try:
                    v = float(vraw)
                ...
                if not (0.0 <= v <= 1.0):
                    ...
                    continue
                if v == 0.0:
                    print(f"    -> {nm} EXCLUDED: its stocks are removed from the "
                          f"universe entirely (changes every downstream step).")
                    out[key][nm] = 0.0
                    break
                if v > gcap:
                    print(f"    WARNING: {v:.0%} is above the global {label} cap "
                          f"({gcap:.0%}), which prevails -> effective cap {gcap:.0%}.")
                    if input("    confirm anyway? (yes = keep, else re-enter): "
                             ).strip().lower() != 'yes':
                        continue
                out[key][nm] = v
                print(f"    -> {nm} capped at {v:.0%} "
                      f"(effective {min(v, gcap):.0%})")
                break

    _n = len(out['sector_caps']) + len(out['subsector_caps'])
    print(f"\n  {_n} individual cap(s) set: "
          f"sectors={out['sector_caps']}  sub-sectors={out['subsector_caps']}\n")
    return out


def _apply_group_caps(w, sectors_s, subsec_map_s=None,
                      sector_cap=None, subsector_cap=None,
                      max_weight=None, residual_ticker=CAP_RESIDUAL_TICKER,
                      max_iter=50, sector_caps=None, subsector_caps=None):
    """
    Enforce sector / sub-sector allocation caps on a final weight vector.

    Breaching bucket -> its names scaled PRO-RATA down to the cap (relative
    conviction inside the bucket preserved). Freed weight redistributed PRO-RATA
    across names that are neither in a capped bucket nor at the individual cap,
    iterating until every constraint holds. Whatever cannot be placed is returned
    as `residual` (parked in `residual_ticker`).

    Returns (w_new, residual, log) where log lists the buckets that bound:
    [{'level': 'sector'|'subsector', 'name': str, 'pre': float, 'post': float}].
    `w` is not modified in place. Any existing residual_ticker position is kept
    out of the cap arithmetic (it belongs to no sector).
    """
    sector_cap    = SECTOR_CAP    if sector_cap    is None else sector_cap
    subsector_cap = SUBSECTOR_CAP if subsector_cap is None else subsector_cap
    ...
    subsector_caps = subsector_caps or {}
    caps_off = ((sector_cap >= 1.0) and (subsector_cap >= 1.0)
                and not sector_caps and not subsector_caps)
    if caps_off:
        return w, 0.0, []

    w = w.astype(float).copy()
    residual = float(w.pop(residual_ticker)) if residual_ticker in w.index else 0.0

    # Effective cap per bucket = min(global, user). A user cap can only TIGHTEN.
    maps = []
    if sectors_s is not None and (sector_cap < 1.0 or sector_caps):
        maps.append(('sector', sectors_s, sector_cap, sector_caps))
    if subsec_map_s is not None and (subsector_cap < 1.0 or subsector_caps):
        maps.append(('subsector', subsec_map_s, subsector_cap, subsector_caps))
    if not maps:
        if residual:
            w[residual_ticker] = residual
        return w, 0.0, []

    ind_cap = max_weight if max_weight is not None else 1.0
    frozen  = set()     # names in a bucket sitting at its cap -> cannot receive
    log     = []

    for _ in range(max_iter):
        freed = 0.0
        breached = False
        for level, m, gcap, ucaps in maps:
            grp = pd.Series({t: m.get(t) for t in w.index})
            grp = grp[grp.notna()]
            if grp.empty:
                continue
            for gname, sub in w.groupby(grp):
                cap = min(gcap, float(ucaps.get(gname, 1.0)))
                if cap >= 1.0:
                    continue
                _src = 'user' if float(ucaps.get(gname, 1.0)) < gcap else 'global'
                tot = float(sub.sum())
                if tot > cap + 1e-12:
                    breached = True
                    scale = cap / tot
                    for t in sub.index:
                        w[t] = w[t] * scale
                    freed += tot - cap
                    log.append({'level': level, 'name': str(gname), 'cap': cap,
                                'src': _src, 'pre': tot, 'post': cap})
                if tot > cap - 1e-12:          # at (or now at) the cap
                    frozen.update(sub.index)
        if not breached:
            break

        # ── redistribute the freed weight pro-rata across eligible names ──────
        for _inner in range(max_iter):
            if freed <= 1e-12:
                break
            elig = [t for t in w.index
                    if t not in frozen and w[t] < ind_cap - 1e-12]
            base = sum(w[t] for t in elig)
            if not elig or base <= 0:
                break                          # no capacity -> residual
            room = {t: ind_cap - w[t] for t in elig}
            add  = {t: freed * (w[t] / base) for t in elig}
            spill = 0.0
            for t in elig:
                give = min(add[t], room[t])
                w[t] += give
                spill += add[t] - give
            freed = spill
        residual += max(freed, 0.0)

    if residual > 1e-12:
        w[residual_ticker] = w.get(residual_ticker, 0.0) + residual
    return w, float(max(residual, 0.0)), log


# ── FACTOR-regime penalty helpers ────────────────────────────────────────────
_FACTOR_MODEL_MAP = {1: '_AND', 2: '_OR', 3: '/TA_AND', 4: '/TA_OR',
                     5: '_ST_AND', 6: '_ST_OR', 7: '/TA_ST_AND', 8: '/TA_ST_OR'}


def _factor_leg_riskon(s_in, l_in, level_s, model, mav1, mav2, mavTA1, mavTA2,
                       thresh_dn, thresh_up, ta_thresh):
    """
    One leg (HMM or CUSUM) of the factor signal, RISK-ON convention (1 = calm).
    Faithful port of the research notebook's generate_signals(): builds the
    memoryless and state-machine variants on the short/long inputs, optionally
    OR'd with a trend veto from the factor level, and returns the variant
    selected by `model` (1..8, see FACTOR_HMM_MODEL). All transforms are
    backward-looking (EWM, ffill), so the series is point-in-time.
    """
    idx = s_in.index.union(l_in.index)
    s_in = s_in.reindex(idx).ffill()
    l_in = l_in.reindex(idx).ffill()
    s_sig = s_in.ewm(halflife=mav1).mean() - s_in.ewm(halflife=mav2).mean()
    l_sig = l_in.ewm(halflife=mav1).mean() - l_in.ewm(halflife=mav2).mean()

    kind = _FACTOR_MODEL_MAP[int(model)]
    use_ta  = kind.startswith('/TA')
    use_st  = '_ST_' in kind
    use_and = kind.endswith('_AND')

    if use_st:
        def _state(sig):
            st = pd.Series(np.nan, index=sig.index)
            st[sig < thresh_dn]  = -1
            st[sig >= thresh_up] = 1
            return st.ffill().clip(0, 1).fillna(1)
        fs, fl = _state(s_sig), _state(l_sig)
    else:
        fs = (s_sig > thresh_up).astype(int)
        fl = (l_sig > thresh_up).astype(int)

    ro = (fs * fl) if use_and else (fs + fl).clip(0, 1)

    if use_ta:
        perf = level_s.reindex(idx).ffill()
        perf = perf / perf.dropna().iloc[0] if perf.notna().any() else perf
        ta = perf.ewm(halflife=mavTA1).mean() / perf.ewm(halflife=mavTA2).mean()
        ta_sig = pd.Series(1, index=idx)
        ta_sig[ta < ta_thresh] = 0
        ro = (ro + ta_sig).clip(0, 1)

    return ro.astype(int)


def _build_factor_activation_flags(fac_hmm, fac_cusum, factor_px, scales,
                                   name_map=None,
                                   short_win=None, long_win=None,
                                   hmm_model=None, cusum_model=None,
                                   leg_combine=None):
    """
    Per-factor ACTIVATION flags (1 = penalise), built from the raw factor_v1
    HMM/CUSUM caches. Only factors with scale < 1.0 are built (scale 1.0 = ignore
    the factor entirely). Returns a DataFrame dates x composite-factor-names, or
    an empty frame if nothing is active / inputs are missing.

      HMM leg   input: 100 * p_low          (short_win / long_win)
      CUSUM leg input: 100 * (1 - inc_up)   (short_win / long_win)
      each leg -> risk-on via _factor_leg_riskon -> inverted to activation
      legs combined with leg_combine ('AND': both activated; 'OR': either)
    """
    name_map    = name_map or FACTOR_NAME_MAP
    short_win   = FACTOR_SHORT_WIN   if short_win   is None else short_win
    long_win    = FACTOR_LONG_WIN    if long_win    is None else long_win
    hmm_model   = FACTOR_HMM_MODEL   if hmm_model   is None else hmm_model
    cusum_model = FACTOR_CUSUM_MODEL if cusum_model is None else cusum_model
    leg_combine = (FACTOR_LEG_COMBINE if leg_combine is None else leg_combine).upper()

    active = {f for f, sc in (scales or {}).items() if sc < 1.0}
    if not active or fac_hmm is None or fac_cusum is None or factor_px is None:
        return pd.DataFrame()

    inv = {v: k for k, v in name_map.items()}     # composite name -> cache asset
    h = fac_hmm.copy();   h['date'] = pd.to_datetime(h['date'])
    c = fac_cusum.copy(); c['date'] = pd.to_datetime(c['date'])

    def _series(df, asset, wy, col, fn):
        sub = df[(df['asset'] == asset) & (df['window_years'] == wy)]
        sub = sub.sort_values('date').drop_duplicates('date', keep='last')
        return fn(sub.set_index('date')[col].astype(float)).round(2)

    out = {}
    for fname in sorted(active):
        asset = inv.get(fname)
        if asset is None or asset not in factor_px.columns:
            warnings.warn(f"  factor penalty: no cache asset / level series for "
                          f"{fname} -- skipped")
            continue
        level_s = factor_px[asset].astype(float)
        hs = _series(h, asset, short_win, 'p_low', lambda x: 100 * x)
        hl = _series(h, asset, long_win,  'p_low', lambda x: 100 * x)
        cs = _series(c, asset, short_win, 'inc_up', lambda x: 100 * (1 - x))
        cl = _series(c, asset, long_win,  'inc_up', lambda x: 100 * (1 - x))
        if hs.empty or hl.empty or cs.empty or cl.empty:
            warnings.warn(f"  factor penalty: missing HMM/CUSUM windows for "
                          f"{asset} -- skipped")
            continue

        hmm_ro = _factor_leg_riskon(hs, hl, level_s, hmm_model,
                                    FACTOR_HMM_MAV1, FACTOR_HMM_MAV2,
                                    FACTOR_HMM_MAVTA1, FACTOR_HMM_MAVTA2,
                                    FACTOR_HMM_THRESH_DN, FACTOR_HMM_THRESH_UP,
                                    FACTOR_HMM_TA_THRESH)
        cus_ro = _factor_leg_riskon(cs, cl, level_s, cusum_model,
                                    FACTOR_CUSUM_MAV1, FACTOR_CUSUM_MAV2,
                                    FACTOR_CUSUM_MAVTA1, FACTOR_CUSUM_MAVTA2,
                                    FACTOR_CUSUM_THRESH_DN, FACTOR_CUSUM_THRESH_UP,
                                    FACTOR_CUSUM_TA_THRESH)
        # invert each leg to ACTIVATION (1 = penalise), common index, ffill
        idx = hmm_ro.index.union(cus_ro.index)
        hmm_act = (1 - hmm_ro.reindex(idx).ffill()).fillna(0)
        cus_act = (1 - cus_ro.reindex(idx).ffill()).fillna(0)
        if leg_combine == 'AND':
            act = hmm_act * cus_act
        else:
            act = (hmm_act + cus_act).clip(0, 1)
        out[fname] = act.astype(int)

    if not out:
        return pd.DataFrame()
    return pd.DataFrame(out).sort_index().ffill().fillna(0).astype(int)


def _factor_pen_signature(scales, fac_flags):
    """Config signature for the composite cache key -- None when no factor is
    penalised (keeps the pre-existing key and cache untouched)."""
    active = {f: sc for f, sc in (scales or {}).items() if sc < 1.0}
    if not active or fac_flags is None or fac_flags.empty:
        return None
    return dict(
        scales=active, hmm_model=FACTOR_HMM_MODEL, cusum_model=FACTOR_CUSUM_MODEL,
        combine=FACTOR_LEG_COMBINE, wins=[FACTOR_SHORT_WIN, FACTOR_LONG_WIN],
        hmm=[FACTOR_HMM_MAV1, FACTOR_HMM_MAV2, FACTOR_HMM_THRESH_UP,
             FACTOR_HMM_THRESH_DN, FACTOR_HMM_MAVTA1, FACTOR_HMM_MAVTA2,
             FACTOR_HMM_TA_THRESH],
        cusum=[FACTOR_CUSUM_MAV1, FACTOR_CUSUM_MAV2, FACTOR_CUSUM_THRESH_UP,
               FACTOR_CUSUM_THRESH_DN, FACTOR_CUSUM_MAVTA1, FACTOR_CUSUM_MAVTA2,
               FACTOR_CUSUM_TA_THRESH],
        name_map=FACTOR_NAME_MAP)


def _build_etf_stress_signals(hmm, cusum, etf_px=None,
                              mav1=ETF_MAV1, mav2=ETF_MAV2, thresh=ETF_THRESH,
                              short_win=ETF_SHORT_WIN, long_win=ETF_LONG_WIN,
                              ta_fast=ETF_TA_FAST, ta_slow=ETF_TA_SLOW,
                              vol_combine=ETF_VOL_COMBINE):
    """
    Build per-ETF binary "penalise?" flags from the raw cached HMM + CUSUM signals
    (as produced by etf_regime_signals.py). Vectorised across every ETF present in
    the inputs. Faithful to the single-asset prototype:

      HMM leg (self-normalised p_low MA-cross, short+long AND):
        For each window w in {short_win, long_win}:
          x   = 100 * p_low
          sig = EWM(x, hl=mav1).mean() - EWM(x, hl=mav2).mean()
          F_w = 1 where sig > thresh else 0          # 1 = calm / do-not-penalise
        FFhmm = F_short * F_long                      # BOTH windows must agree (AND)

      CUSUM leg (state machine, short+long OR-to-risk-on):
        For each window w: alarm_dn -> 1 (calm), alarm_up -> 0 (stress), hold last
        state in between (ffill), default 1 (do-not-penalise) before first alarm.
        Fcusum = (S_state + L_state).clip(0, 1)       # either window calm -> calm

      Combine (deliberate, matching the prototype):
        risk_on = (Fcusum + FFhmm).clip(0, 1)         # EITHER leg calm -> do-not-penalise
        penalise = 1 - risk_on                        # penalise only when BOTH legs say stress

    The default everywhere is DO-NOT-PENALISE (risk_on = 1): a flag is raised only
    on solid affirmative evidence of deterioration from BOTH the HMM and CUSUM legs.
    All operations (ewm, ffill) are strictly backward-looking, so the resulting
    flags are PIT-safe: slicing to <= dt at lookup time never peeks ahead.

    Returns
    -------
    DataFrame (index = dates, columns = ETF tickers), values in {0, 1}:
        1 = penalise (both legs flag stress), 0 = do not penalise.
    """
    if hmm is None or cusum is None or len(hmm) == 0 or len(cusum) == 0:
        return pd.DataFrame()
    ...
    assets = sorted(set(hmm['asset'].unique()) & set(cusum['asset'].unique()))

    # Loud guard: the short/long windows must actually exist in the cached data.
    _hw = set(hmm['window_years'].unique())
    _cw = set(cusum['window_years'].unique())
    _missing = [w for w in (short_win, long_win) if w not in _hw or w not in _cw]
    if _missing:
        import warnings as _w
        _w.warn(f"  ETF stress: requested window(s) {_missing} not in cached signals "
                f"(HMM has {sorted(_hw)}, CUSUM has {sorted(_cw)}). "
                f"Pass etf_short_win/etf_long_win to match, or rebuild signals. "
                f"No flags built.")
        return pd.DataFrame()
    if not assets:
        import warnings as _w
        _w.warn("  ETF stress: no common assets between HMM and CUSUM frames. "
                "No flags built.")
        return pd.DataFrame()
    penal_by_asset = {}
    ...
    for asset in assets:
        ...
        FF = {}
        ok = True
        for w in (short_win, long_win):
            sub = hmm[(hmm['asset'] == asset) & (hmm['window_years'] == w)]
            if sub.empty:
                ok = False
                break
            x = (100.0 * sub.set_index('date')['p_low']).sort_index()
            sig = x.ewm(halflife=mav1).mean() - x.ewm(halflife=mav2).mean()
            F = pd.Series(0, index=sig.index)
            F[sig > thresh] = 1                     # 1 = calm / do-not-penalise
            FF[w] = F
        if not ok:
            continue
        # align the two windows on their common dates, AND them
        FFhmm = (FF[short_win].reindex(FF[long_win].index).fillna(0)
                 * FF[long_win]).astype(float)
        FFhmm = FFhmm.reindex(FF[short_win].index.union(FF[long_win].index)).fillna(0)

        # ---- CUSUM leg -----------------------------------------------------
        cstate = {}
        for w in (short_win, long_win):
            sub = cusum[(cusum['asset'] == asset) & (cusum['window_years'] == w)]
            if sub.empty:
                cstate[w] = None
                continue
            sub = sub.set_index('date').sort_index()
            # alarm_dn -> 1 (calm), alarm_up -> 0 (stress), else NaN -> hold
            st = pd.Series(np.nan, index=sub.index)
            st[sub['alarm_dn'].astype(bool)] = 1
            st[sub['alarm_up'].astype(bool)] = 0
            st = st.ffill().fillna(1)               # default 1 = do-not-penalise
            cstate[w] = st
        legs = [s for s in cstate.values() if s is not None]
        if not legs:
            continue
        # union index, ffill each, OR-to-calm via sum+clip
        cidx = legs[0].index
        for s in legs[1:]:
            cidx = cidx.union(s.index)
        legs = [s.reindex(cidx).ffill().fillna(1) for s in legs]
        Fcusum = sum(legs)
        Fcusum = Fcusum.clip(0, 1)

        # ---- Price/trend leg (TA): 1 = uptrend (fast MA > slow MA) ----------
        # Read the ETF's own price from etf_px (or Pxs_df passed as etf_px). The
        # trend gate encodes "only bad vol": an UPtrend vetoes the penalty (high
        # vol while rising = good vol, do-not-penalise); a DOWNtrend lets the vol
        # regime decide. Absent price -> TA=1 (uptrend/veto default -> never
        # penalise on price grounds, so the asset falls back to vol-only behaviour).
        if etf_px is not None and asset in etf_px.columns:
            _p = etf_px[asset].dropna()
            _ta = (_p.rolling(ta_fast).mean() > _p.rolling(ta_slow).mean()).astype(float)
            TAsig = _ta  # 1 = uptrend (calm/veto), 0 = downtrend
        else:
            TAsig = None

        # ---- Combine --------------------------------------------------------
        # Vol legs: HMM and CUSUM, combined per vol_combine. Both are risk-on
        # coded (1 = calm). 'AND'-to-penalise means BOTH must flag stress, i.e.
        # OR-to-calm: (FFhmm | Fcusum) calm -> vol_calm=1. 'OR'-to-penalise means
        # EITHER flags stress, i.e. AND-to-calm: both must be calm for vol_calm=1.
        allidx = FFhmm.index.union(Fcusum.index)
        FFhmm_a  = FFhmm.reindex(allidx).ffill().fillna(0)     # HMM absent -> 0 (not calm)
        Fcusum_a = Fcusum.reindex(allidx).ffill().fillna(1)    # CUSUM absent -> 1 (calm default)
        if str(vol_combine).upper() == 'OR':
            # penalise if EITHER vol leg flags -> calm requires BOTH calm (AND)
            vol_calm = ((FFhmm_a > 0.5) & (Fcusum_a > 0.5)).astype(float)
        else:
            # 'AND' (default): penalise only if BOTH flag -> calm if EITHER calm (OR)
            vol_calm = (Fcusum_a + FFhmm_a).clip(0, 1)

        # Price gate ANDed on top: uptrend vetoes penalty (risk_on via OR-to-calm).
        if TAsig is not None:
            TAsig_a = TAsig.reindex(allidx).ffill().fillna(1)  # absent -> uptrend/veto
            risk_on = (vol_calm + TAsig_a).clip(0, 1)          # uptrend OR vol-calm -> do-not-penalise
        else:
            risk_on = vol_calm
        penalise = (1.0 - risk_on).astype(int)
        penal_by_asset[asset] = penalise

    if not penal_by_asset:
        return pd.DataFrame()
    flags = pd.DataFrame(penal_by_asset).sort_index()
    return flags


def _etf_penalty_multiplier(dt, tickers, etf_stress_flags, etf_map_s, scale):
    """
    Per-ticker alpha multiplier for the ETF sector-stress penalty at date dt.

    For each ticker: look up its mapped ETF (etf_map_s), read that ETF's most recent
    stress flag on/before dt (PIT: flags.loc[:dt].iloc[-1]); if flagged (==1), the
    multiplier is `scale`, else 1.0. Tickers with no mapping, no ETF column, or no
    flag on/before dt default to 1.0 (do-not-penalise).

    Returns a pd.Series indexed by `tickers`.
    """
    if (etf_stress_flags is None or etf_map_s is None
            or etf_stress_flags.empty or scale >= 1.0):
        return pd.Series(1.0, index=tickers)

    # most-recent flags row on/before dt (PIT)
    past = etf_stress_flags.loc[:dt]
    if past.empty:
        return pd.Series(1.0, index=tickers)
    row = past.iloc[-1]   # Series indexed by ETF

    mult = {}
    for t in tickers:
        etf = etf_map_s.get(t, None)
        if etf is None or etf not in row.index:
            mult[t] = 1.0
            continue
        flagged = row.get(etf, 0)
        mult[t] = scale if (pd.notna(flagged) and int(flagged) == 1) else 1.0
    return pd.Series(mult, index=tickers)


def _combined_penalty_multiplier(dt, tickers,
                                 etf_stress_flags, etf_map_s, etf_scale,
                                 ss_stress_flags, subsec_map_s, ss_scale):
    """
    Two-layer per-ticker alpha multiplier: the PRODUCT of the broad-ETF layer
    multiplier and the sub-sector layer multiplier at date dt.

        mult_i = etf_mult_i(scale=etf_scale) * subsec_mult_i(scale=ss_scale)

    Each layer is independently disabled at scale 1.0 (its per-ticker multiplier is
    then 1.0 everywhere, via _etf_penalty_multiplier's scale>=1.0 guard), so:
      - etf_scale=1.0                 -> ETF layer inert
      - ss_scale=1.0                  -> sub-sector layer inert (product = ETF only)
      - both < 1.0                    -> a name flagged by both gets etf_scale*ss_scale
    A name flagged by only one active layer gets that layer's scale; flagged by
    neither gets 1.0. Reproduces the single-layer behaviour exactly when
    ss_scale=1.0.
    """
    etf_mult = _etf_penalty_multiplier(dt, tickers, etf_stress_flags,
                                       etf_map_s, etf_scale)
    ss_mult = _etf_penalty_multiplier(dt, tickers, ss_stress_flags,
                                      subsec_map_s, ss_scale)
    return (etf_mult * ss_mult)
...
def _mb_solve_mvo(dt, candidates, composite_scores, Pxs_df, sectors_s,
                   ...
                   etf_stress_flags=None, etf_map_s=None,
                   etf_penalty_scale=ETF_PENALTY_SCALE,
                   ss_stress_flags=None, subsec_map_s=None,
                   subsector_penalty_scale=SUBSECTOR_PENALTY_SCALE):
    """
    ...
    # -- Sector-stress penalty (two-layer: ETF x sub-sector) ------------------
    # Shrink alpha for names whose mapped sector-ETF and/or sub-sector is in a
    # flagged (stressed) regime, via the PRODUCT of both layers' multipliers.
    # No-op when both scales >= 1.0 or inputs absent; a layer at scale 1.0 is
    # individually inert.
    _pen_active = ((etf_penalty_scale < 1.0 and etf_stress_flags is not None and etf_map_s is not None)
                   or (subsector_penalty_scale < 1.0 and ss_stress_flags is not None and subsec_map_s is not None))
    if _pen_active:
        _pen = _combined_penalty_multiplier(
            dt, valid, etf_stress_flags, etf_map_s, etf_penalty_scale,
            ss_stress_flags, subsec_map_s, subsector_penalty_scale)
        alpha = (alpha * _pen.reindex(valid).fillna(1.0)).fillna(0.0)
...
# ── HMM regime-overlay loader ─────────────────────────────────────────────────
def _load_hmm_consensus(trading_days: pd.DatetimeIndex,
                        asset: str = None,
                        verbose: bool = True):
    """
    Pull the filtered vol-state probabilities for HMM_ASSET from the
    hmm_regime_cache table and build the two consensus override series.

    Returns (hi_ok, lo_ok, missing):
      hi_ok   : pd.Series[bool] over trading_days -- ALL HMM_WINDOWS have
                p_high > HMM_THRESH_HIGH, sustained >= MIN_SP consecutive days
                (backward-looking rolling window -> strictly PIT)
      lo_ok   : pd.Series[bool] -- same for p_low > HMM_THRESH_LOW
      missing : pd.DatetimeIndex of trading_days with no usable HMM data
                (never counts as consensus in either direction; caller warns)

    hi_ok drives the conservative construction override AND the mirror rule
    (blocking permanent zone exits); lo_ok drives the alpha override (out of
    zone) AND the revocable re-gross parole (in zone).

    On any failure (table absent, no rows) returns (None, None, all-days) so
    the backtest degrades gracefully to the DD-based rules.
    """
    a = asset or HMM_ASSET
    try:
        q = f"""SELECT date, window_years, p_low, p_high
                FROM {HMM_TBL}
                WHERE model_version = :mv AND asset = :a
                  AND window_years = ANY(:ws)
                ORDER BY date, window_years"""
        raw = pd.read_sql(text(q), ENGINE,
                          params={'mv': HMM_MODEL_VER, 'a': a,
                                  'ws': list(HMM_WINDOWS)},
                          parse_dates=['date'])
    except Exception as e:
        if verbose:
            print(f"  [HMM overlay] load failed ({e}) -- regime falls back to "
                  f"DD rules for the whole run")
        return None, None, None, None, pd.DatetimeIndex(trading_days)

    if raw.empty:
        if verbose:
            print(f"  [HMM overlay] no cached rows for asset={a} "
                  f"(model_version={HMM_MODEL_VER}) -- DD rules only")
        return None, None, None, None, pd.DatetimeIndex(trading_days)

    p_hi = raw.pivot_table(index='date', columns='window_years', values='p_high')
    p_lo = raw.pivot_table(index='date', columns='window_years', values='p_low')
    p_hi = p_hi.reindex(columns=list(HMM_WINDOWS)).reindex(trading_days)
    p_lo = p_lo.reindex(columns=list(HMM_WINDOWS)).reindex(trading_days)

    usable = p_hi.notna().all(axis=1) & p_lo.notna().all(axis=1)
    missing = pd.DatetimeIndex(trading_days[~usable])

    # raw per-day consensus (NaN never counts as consensus)
    hi_raw = (p_hi > HMM_THRESH_HIGH).all(axis=1) & usable
    lo_raw = (p_lo > HMM_THRESH_LOW ).all(axis=1) & usable

    # persistence: condition must have held on MIN_SP consecutive days INCLUDING
    # today (backward-looking rolling window -> strictly PIT). Entry needs
    # MIN_SP consecutive days; exit is 1 day (one raw miss breaks the window) --
    # asymmetric by design for a risk control.
    hi_ok = hi_raw.rolling(MIN_SP, min_periods=MIN_SP).sum().eq(MIN_SP)
    lo_ok = lo_raw.rolling(MIN_SP, min_periods=MIN_SP).sum().eq(MIN_SP)
    # pathological both-true (never observed; degenerate fit noise): the
    # conservative side wins -- clear lo where hi is set.
    lo_ok = lo_ok & ~hi_ok

    # Fast-tier corroboration standard: a SINGLE window's raw probability above
    # threshold, same-day, no persistence (only consulted while a CUSUM alarm
    # is latched -- cross-model agreement replaces the MIN_SP wait).
    hi_any1 = (p_hi > HMM_THRESH_HIGH).any(axis=1) & usable
    lo_any1 = (p_lo > HMM_THRESH_LOW ).any(axis=1) & usable

    if verbose:
        n_hi = int(hi_ok.sum()); n_lo = int(lo_ok.sum())
        print(f"  [HMM overlay] asset={a}  windows={HMM_WINDOWS}  "
              f"thresholds hi>{HMM_THRESH_HIGH:.0%}/lo>{HMM_THRESH_LOW:.0%}  "
              f"MIN_SP={MIN_SP}  parole={'on' if HMM_PAROLE else 'off'}  "
              f"block_reentry={'on' if HMM_BLOCK_REENTRY else 'off'}")
        print(f"  [HMM overlay] consensus days: {n_hi} high-vol, {n_lo} low-vol, "
              f"{len(missing)} day(s) without HMM data")
    return hi_ok, lo_ok, hi_any1, lo_any1, missing


# ── CUSUM fast-tier loader ────────────────────────────────────────────────────
def _load_cusum_states(trading_days: pd.DatetimeIndex,
                       asset: str = None,
                       verbose: bool = True):
    """
    Pull the two-sided vol-CUSUM alarms for HMM_ASSET from cusum_regime_cache
    and convert the point events into daily LATCHED states:

      * an alarm on any of the CUSUM reference windows activates its direction
      * the latch stays active for CUSUM_ACTIVATION_WINDOW trading days
        (renewed by another same-direction alarm: clock restarts)
      * an opposite-direction alarm kills the latch immediately
      * both directions alarming the same day: UP wins (conservative)
      * days without CUSUM data: latch neither activates nor renews (abstain)

    Returns (up_latched, dn_latched): two boolean Series over trading_days.
    On any failure returns (None, None) -- the fast tier degrades to inert and
    the slow HMM consensus carries alone.
    """
    a = asset or HMM_ASSET
    try:
        q = f"""SELECT date, window_years, alarm_up, alarm_dn
                FROM {CUSUM_TBL}
                WHERE model_version = :mv AND asset = :a
                ORDER BY date, window_years"""
        raw = pd.read_sql(text(q), ENGINE,
                          params={'mv': CUSUM_MODEL_VER, 'a': a},
                          parse_dates=['date'])
    except Exception as e:
        if verbose:
            print(f"  [CUSUM tier] load failed ({e}) -- fast tier inert")
        return None, None
    if raw.empty:
        if verbose:
            print(f"  [CUSUM tier] no cached rows for asset={a} "
                  f"(model_version={CUSUM_MODEL_VER}) -- fast tier inert")
        return None, None

    # Per-window latches (each window's alarm stream runs its own latch with
    # its own opposite-direction kills); the composite requires at least
    # CUSUM_MIN_WINDOWS windows latched in the same direction. With
    # MIN_WINDOWS=1 this is any-of-3; with 2 it is the stricter 2-of-3-latched
    # variant (same-day co-alarming is NOT required -- staggered alarms across
    # windows are the norm, so the k-th window's alarm turns the key).
    windows = sorted(raw['window_years'].unique())
    n_up = pd.Series(0, index=trading_days)
    n_dn = pd.Series(0, index=trading_days)
    tot_au = tot_ad = 0
    for w in windows:
        g = raw[raw['window_years'] == w].set_index('date').sort_index()
        al_up = g['alarm_up'].reindex(trading_days).fillna(False).astype(bool)
        al_dn = g['alarm_dn'].reindex(trading_days).fillna(False).astype(bool)
        tot_au += int(al_up.sum()); tot_ad += int(al_dn.sum())
        ul = dl = 0
        for i in range(len(trading_days)):
            u, d = bool(al_up.iloc[i]), bool(al_dn.iloc[i])
            if u and d:
                d = False                 # same-day conflict: UP wins
            if u:
                ul, dl = CUSUM_ACTIVATION_WINDOW, 0
            elif d:
                dl, ul = CUSUM_ACTIVATION_WINDOW, 0
            if ul > 0:
                n_up.iloc[i] += 1
                ul -= 1
            if dl > 0:
    ...
    up_latched = n_up >= CUSUM_MIN_WINDOWS
    dn_latched = (n_dn >= CUSUM_MIN_WINDOWS) & ~up_latched   # UP wins overlaps

    if verbose:
        print(f"  [CUSUM tier] asset={a}  activation_window="
              f"{CUSUM_ACTIVATION_WINDOW}d  min_windows={CUSUM_MIN_WINDOWS}"
              f"/{len(windows)}")
        print(f"  [CUSUM tier] alarm days: {tot_au} up, {tot_ad} down; latched "
              f"days: {int(up_latched.sum())} up, {int(dn_latched.sum())} down")
    return up_latched, dn_latched


def _json_default(o):
    """Fallback encoder for json.dumps: convert any datetime-like object to an
    ISO string. Catches Timestamps nested inside dicts/lists (e.g.
    active_hedges[inst]['entry_dt']) that the field-level checks miss, and also
    numpy datetime64 / numpy scalar types."""
    import numpy as _np
...
def _state_to_json(st: dict) -> str:
    ...
    fields = ['nav','hwm','hwm_rolling','dd','gross','trough','theo_nav',
              ...
              'costs','costs_by_year','active_hedges','parole','parole_from_gross',
              'parole_blocked','parole_revoke_dt']
    snap = {}
    for f in fields:
        v = st.get(f)
    ...
    # Persist the full rebal_log so the rebalance history (and the regime map that
    # reads it) survives incremental runs -- previously only a single synthetic
    # last-rebalance event was reconstructed on restore, so the regime map showed
    # just one char per year after any incremental run. Drop the bulky per-event
    # 'w' weights Series (not needed by any consumer of the persisted log); keep
    # dt, regime, to, nav. Serialize the 'dt' Timestamp to ISO.
    snap['rebal_log'] = [
        {k: (v.isoformat() if isinstance(v, pd.Timestamp) else v)
         for k, v in ev.items() if k != 'w'}
        for ev in st.get('rebal_log', [])
    ]
    # active_hedges (and any other nested structures) may contain Timestamps
    # (e.g. 'entry_dt'); the default handler serializes them at any depth.
...
def _state_from_json(js: str) -> dict:
    ...
    # parole_revoke_dt (fast-track cooldown clock) back to Timestamp if present
    if snap.get('parole_revoke_dt') is not None:
        snap['parole_revoke_dt'] = pd.Timestamp(snap['parole_revoke_dt'])
    ...
    snap['hedge_log'] = []
    # Restore Timestamps nested in active_hedges ('entry_dt') so downstream date
    # arithmetic (e.g. holding-period days) works after a cache round-trip.
    ah = snap.get('active_hedges') or {}
    for _inst, _h in ah.items():
        if isinstance(_h, dict) and _h.get('entry_dt') is not None:
            _h['entry_dt'] = pd.Timestamp(_h['entry_dt'])
    snap['active_hedges'] = ah
    # Restore the full rebal_log if it was persisted (dt back to Timestamp). Falls
    # back to a minimal single-event reconstruction only for OLD caches written
    # before rebal_log was persisted, so the trade summary can still compute
    # _drifted_w from the last rebalance.
    _persisted_rlog = snap.get('rebal_log')
    if _persisted_rlog:
        snap['rebal_log'] = [
            {k: (pd.Timestamp(v) if k == 'dt' and v is not None else v)
             for k, v in ev.items()}
            for ev in _persisted_rlog
        ]
    elif snap.get('last_rebal') and not snap['weights'].empty:
...
def _cache_load(params_hash: str, strategy_labels: dict):
    ...
    # Load state JSON + its as-of date (Option B: state is saved as-of the
    # second-to-last processed date, so the STATE's last_date is the authoritative
    # resume anchor -- we recompute everything strictly after it, which always
    # includes the provisional last date).
    with ENGINE.connect() as conn:
        srows = conn.execute(text(f"""
            SELECT strategy, state_json, last_date FROM {MVO_STATE_CACHE_TBL}
            WHERE params_hash = :ph
        """), {'ph': params_hash}).fetchall()
    state_by_label = {lbl: _state_from_json(js) for lbl, js, _ld in srows}
    _state_last_dates = [pd.Timestamp(_ld) for _lbl, _js, _ld in srows if _ld is not None]
    # Resume anchor: the state's as-of date (min across strategies for safety).
    # Fall back to the NAV max only if no state dates are available.
    _nav_last = min(all_last) if all_last else None
    last_date = min(_state_last_dates) if _state_last_dates else _nav_last
...
def run_backtest(
    ...
    factor_pen_changed   = False,  # factor_v1 HMM/CUSUM data regenerated -> composite
    ...
    hmm                 = None,          # raw per-ETF HMM signals (from etf_regime_signals)
    cusum               = None,          # raw per-ETF CUSUM signals (from etf_regime_signals)
    etf_map_s           = None,          # stock -> sector-ETF Series (parallel to sectors_s)
    etf_penalty_scale   = ETF_PENALTY_SCALE,  # 1.0 disables the sector-stress penalty
    etf_short_win       = ETF_SHORT_WIN, # HMM/CUSUM fast window_years (must be cached)
    etf_long_win        = ETF_LONG_WIN,  # HMM/CUSUM slow window_years (must be cached)
    ss_hmm              = None,          # raw per-SUBSECTOR HMM signals (subsec_v1)
    ss_cusum            = None,          # raw per-SUBSECTOR CUSUM signals (subsec_v1)
    subsec_map_s        = None,          # stock -> sub-sector Series (parallel to etf_map_s)
    subsec_idx          = None,          # sub-sector equal-weighted price indices (for the TA leg)
    subsector_penalty_scale = SUBSECTOR_PENALTY_SCALE,  # 1.0 disables the sub-sector layer
    fac_hmm             = None,          # raw factor HMM signals (factor_v1)
    fac_cusum           = None,          # raw factor CUSUM signals (factor_v1)
    factor_px           = None,          # factor LEVEL frame (cache asset names as columns)
    factor_penalty_scales = None,        # {composite factor: scale}; None -> FACTOR_PENALTY_SCALES
    sector_cap          = SECTOR_CAP,    # max total weight per sector     (1.0 = off)
    subsector_cap       = SUBSECTOR_CAP, # max total weight per sub-sector (1.0 = off)
    sector_caps         = None,          # {sector: cap} individual caps; 0 = exclude
    subsector_caps      = None,          # {sub-sector: cap} individual caps; 0 = exclude
    ask_group_caps      = False,         # True -> prompt for individual caps at runtime
):
    """
    ...
    # ── ETF sector-stress penalty: build binary flags ONCE (PIT-safe) ─────────
    # Construction is entirely backward-looking (ewm/ffill), so a one-time build
    # + per-date <= dt lookup is both efficient and point-in-time correct.
    _etf_stress_flags = None
    if etf_penalty_scale < 1.0 and hmm is not None and cusum is not None and etf_map_s is not None:
        try:
            _etf_stress_flags = _build_etf_stress_signals(hmm, cusum, etf_px=Pxs_df,
                                                          short_win=etf_short_win,
                                                          long_win=etf_long_win)
            if _etf_stress_flags is not None and not _etf_stress_flags.empty:
                _npen = int((_etf_stress_flags == 1).sum().sum())
                print(f"  ETF sector-stress penalty ACTIVE: scale={etf_penalty_scale}, "
                      f"{_etf_stress_flags.shape[1]} ETFs, "
                      f"{_etf_stress_flags.shape[0]} dates, {_npen} flag-days")
            else:
                print("  ETF sector-stress penalty: no flags built (empty signals) — disabled")
                _etf_stress_flags = None
        except Exception as _e:
            warnings.warn(f"  ETF stress signal build failed: {_e} — penalty disabled")
            _etf_stress_flags = None
    elif etf_penalty_scale >= 1.0:
        print("  ETF sector-stress penalty: DISABLED (scale=1.0)")

    # ── SUB-SECTOR sector-stress penalty: build binary flags ONCE (PIT-safe) ──
    # Second, finer layer. Same construction as the ETF layer, but on the 30
    # floored sub-sector indices. The TA (price-trend) leg uses subsec_idx (the
    # sub-sector equal-weighted price indices), NOT Pxs_df. Independently disabled
    # at scale 1.0: when off, _ss_stress_flags stays None and the combined
    # multiplier reduces to the ETF layer alone (backward-compatible).
    _ss_stress_flags = None
    if (subsector_penalty_scale < 1.0 and ss_hmm is not None and ss_cusum is not None
            and subsec_map_s is not None):
        _ss_px = subsec_idx if subsec_idx is not None else None
        try:
            _ss_stress_flags = _build_etf_stress_signals(ss_hmm, ss_cusum, etf_px=_ss_px,
                                                         short_win=etf_short_win,
                                                         long_win=etf_long_win)
            if _ss_stress_flags is not None and not _ss_stress_flags.empty:
                _nssp = int((_ss_stress_flags == 1).sum().sum())
                print(f"  SUB-SECTOR sector-stress penalty ACTIVE: "
                      f"scale={subsector_penalty_scale}, "
                      f"{_ss_stress_flags.shape[1]} sub-sectors, "
                      f"{_ss_stress_flags.shape[0]} dates, {_nssp} flag-days")
            else:
                print("  SUB-SECTOR sector-stress penalty: no flags built — disabled")
                _ss_stress_flags = None
        except Exception as _e:
            warnings.warn(f"  SUB-SECTOR stress signal build failed: {_e} — disabled")
            _ss_stress_flags = None
    elif subsector_penalty_scale >= 1.0:
        print("  SUB-SECTOR sector-stress penalty: DISABLED (scale=1.0)")
    ...
    # ── Individual sector / sub-sector caps ──────────────────────────────────
    sector_caps    = dict(sector_caps or {})
    subsector_caps = dict(subsector_caps or {})
    if ask_group_caps:
        if input("\n  Set individual sector / sub-sector caps? (yes / anything "
                 "else = no): ").strip().lower() == 'yes':
            _picked = prompt_group_caps(sectors_s, subsec_map_s,
                                        sector_cap=sector_cap,
                                        subsector_cap=subsector_cap)
            sector_caps.update(_picked['sector_caps'])
            subsector_caps.update(_picked['subsector_caps'])
    # warn on names that match no bucket in the supplied mappings
    for _lbl, _d, _m in (('sector', sector_caps, sectors_s),
                         ('sub-sector', subsector_caps, subsec_map_s)):
        if _d and _m is not None:
            _known = set(pd.Series(_m).dropna().unique())
            _unk = [k for k in _d if k not in _known]
            if _unk:
                print(f"  WARNING: {_lbl} cap(s) match no bucket and are ignored: {_unk}")
    # a cap of exactly 0 is a UNIVERSE FILTER, not an overlay
    _excl_sectors    = sorted(k for k, v in sector_caps.items() if v <= 0.0)
    _excl_subsectors = sorted(k for k, v in subsector_caps.items() if v <= 0.0)
    sector_caps      = {k: v for k, v in sector_caps.items() if v > 0.0}
    subsector_caps   = {k: v for k, v in subsector_caps.items() if v > 0.0}
    ...
    ph           = _make_make_params_hash(ic, max_weight, min_weight, zscore_cap,
                                          ...
                                          group_caps=({'sec': sector_caps,
                                                       'sub': subsector_caps,
                                                       'xs': _excl_sectors,
                                                       'xss': _excl_subsectors}
                                                      if (sector_caps or subsector_caps
                                                          or _excl_sectors or _excl_subsectors)
                                                      else None))
    ...
    # ── Excluded sectors / sub-sectors (cap == 0): UNIVERSE FILTER ───────────
    # Applied here, before anything downstream consumes `universe`, so the whole
    # process (composite z-scores, percentile ranks, covariance, candidate pools,
    # overlay) is computed on the reduced cross-section -- which is what an
    # exclusion means, and why it cannot be a post-construction overlay.
    if _excl_sectors or _excl_subsectors:
        _n0 = len(universe)
        _xs, _xss = set(_excl_sectors), set(_excl_subsectors)
        universe = [t for t in universe
                    if (sectors_s.get(t) not in _xs)
                    and (subsec_map_s is None or subsec_map_s.get(t) not in _xss)]
        print(f"  UNIVERSE FILTER: excluded {_n0 - len(universe)} stock(s) -> "
              f"{len(universe)} remain  "
              f"(sectors={_excl_sectors or '-'}, sub-sectors={_excl_subsectors or '-'})")
        if not universe:
            raise ValueError("Every stock was excluded by the sector / sub-sector "
                             "exclusions -- nothing left to build a portfolio from.")

    # ── Feasibility check on the caps ────────────────────────────────────────
    # With the effective caps, what is the most the book can hold? If that is
    # below 100%, the shortfall necessarily ends up in the residual ticker, so
    # say it up front rather than letting it surface as a surprise SPX position.
    if sector_cap < 1.0 or subsector_cap < 1.0 or sector_caps or subsector_caps:
        _cap_msgs = []
        for _lbl, _m, _g, _u in (('sector', sectors_s, sector_cap, sector_caps),
                                 ('sub-sector', subsec_map_s, subsector_cap, subsector_caps)):
            if _m is None:
                continue
            _bk = sorted(set(pd.Series(_m).reindex(universe).dropna().unique()))
            if not _bk:
                continue
            _cap_tot = sum(min(_g, float(_u.get(b, 1.0))) for b in _bk)
            _cap_msgs.append(f"{_lbl}s: {len(_bk)} bucket(s), max total "
                             f"{_cap_tot:.0%}")
            if _cap_tot < 1.0 - 1e-9:
                print(f"  WARNING: {_lbl} caps allow at most {_cap_tot:.0%} of the "
                      f"book -> at least {1 - _cap_tot:.0%} will sit in "
                      f"{CAP_RESIDUAL_TICKER}")
        if _cap_msgs:
            print(f"  CAPS: global sec={sector_cap:.0%} sub={subsector_cap:.0%}"
                  + (f"  user={len(sector_caps)}+{len(subsector_caps)}"
                     if (sector_caps or subsector_caps) else "")
                  + f"  |  {'; '.join(_cap_msgs)}")
    ...
    if mode == 'incremental':
        _f_universe = _f_factor = _f_weights = _f_regime = _f_mr = _f_facpen = False
        ...
        _f_mr       = mr_changed
        _f_facpen   = factor_pen_changed

    # ── FACTOR-regime penalty: build per-factor activation flags ONCE ─────────
    _fac_scales = dict(FACTOR_PENALTY_SCALES if factor_penalty_scales is None
                       else factor_penalty_scales)
    _fac_flags  = pd.DataFrame()
    if any(sc < 1.0 for sc in _fac_scales.values()):
        if fac_hmm is None or fac_cusum is None or factor_px is None:
            print("  FACTOR penalty: scales < 1.0 but fac_hmm / fac_cusum / "
                  "factor_px not supplied -- DISABLED")
        else:
            _fac_flags = _build_factor_activation_flags(
                fac_hmm, fac_cusum, factor_px, _fac_scales)
            if _fac_flags.empty:
                print("  FACTOR penalty: no flags built -- DISABLED")
            else:
                _act = {f: sc for f, sc in _fac_scales.items() if sc < 1.0}
                _on  = {f: int(_fac_flags[f].sum()) for f in _fac_flags.columns}
                print(f"  FACTOR penalty ACTIVE: scales={_act}  "
                      f"legs HMM={FACTOR_HMM_MODEL}/CUSUM={FACTOR_CUSUM_MODEL} "
                      f"combine={FACTOR_LEG_COMBINE}  activation-days={_on}  "
                      f"rebal-trigger={sorted(FACTOR_REBAL_TRIGGER & set(_act))}")
    else:
        print("  FACTOR penalty: DISABLED (all scales = 1.0)")
    _fac_sig = _factor_pen_signature(_fac_scales, _fac_flags)
    # Activation EDGES (0 -> 1) for the rebalance-trigger factors. Only factors in
    # FACTOR_REBAL_TRIGGER with scale < 1.0 can force a rebalance; de-activation
    # (1 -> 0) never does -- the recovering weight flows through the normal
    # turnover gate.
    _fac_edges = {}
    if not _fac_flags.empty:
        for _f in _fac_flags.columns:
            if _f in FACTOR_REBAL_TRIGGER and _fac_scales.get(_f, 1.0) < 1.0:
                _fs = _fac_flags[_f]
                _e = _fs[(_fs == 1) & (_fs.shift(1).fillna(0) == 0)].index
                _fac_edges[_f] = list(_e)
    ...
    _force_comp_rebuild = (_f_universe or _f_factor or _f_weights
                           or _f_regime or _f_mr or _f_facpen)
    ...
    _comp_key = _make_composite_cache_key(universe, model_version,
                                          factor_pen_sig=_fac_sig)

    if _force_comp_rebuild:
        _clear_composite_cache(_comp_key)
        composite_by_date, score_dfs, _cached_set, _wt_by_date = {}, {}, set(), {}
        _reasons = [n for n, f in [('universe', _f_universe), ('factor_model', _f_factor),
                    ('weights', _f_weights), ('regime', _f_regime), ('mr', _f_mr),
                    ('factor_pen', _f_facpen)] if f]
        print(f"  Composite cache cleared (key={_comp_key}; "
              f"changed: {', '.join(_reasons)})")
    else:
        composite_by_date, score_dfs, _cached_set, _wt_by_date = _load_composite_cache(
    ...
    if len(_missing) > 0:
        ...
        _new_comp, _new_scores, _new_wts = _cb_build_composite_scores(
            universe        = universe,
            calc_dates      = _missing,
            Pxs_df          = Pxs_df,
            sectors_s       = sectors_s,
            ...
            exclude_factors = ['OU'],
            weights_by_date = weights_by_date,
            factor_flags    = _fac_flags,
            factor_scales   = _fac_scales,
        )
        if not _new_comp and _missing:
            _wd_last = sorted(weights_by_date.keys())[-1] if weights_by_date else None
            _wd_fac  = (list(weights_by_date[_wd_last].index)
                        if _wd_last is not None else [])
            raise RuntimeError(
                f"Composite build produced 0 dates for {len(_missing)} requested "
                f"date(s) -- refusing to freeze an empty panel into the cache. "
                f"Most likely cause: incomplete factor inputs in this kernel "
                f"session (weights_by_date[{_wd_last}] carries factors "
                f"{_wd_fac}; a healthy run has Value and Quality alongside the "
                f"momentum factors). Re-run the fundamental-factor and "
                f"IC-weights cells, then rerun with the composite rebuild flag.")
        # Merge newly-built into the (possibly cached) panel.
        composite_by_date.update(_new_comp)
        _wt_by_date.update(_new_wts)
    ...
    def _build_overlay(w_tier1, dt, non_core_cands, comp_scores, strat_idx):
        ...
        # PARTIAL FILL (option 2): use however many candidates clear the gates,
        # up to n_ov, rather than aborting the whole overlay when the pool is
        # short (all-or-nothing). At large AUM the ADVP capacity gate shrinks the
        # eligible pool -- overlay names are tail-liquidity by construction -- and
        # the old behaviour silently dropped the entire 10% sleeve (book fell to
        # core-only). Now each surviving name still gets the FIXED overlay weight
        # ((1-TIERONE_ALLOC)/(T-N), enforced in _apply_tier2); the unfilled
        # remainder returns to tier 1. n= in the rebalance header shows the fill.
        if not cands:
            return pd.Series(dtype=float)
        n_target = min(n_ov, len(cands))

        # Step 1: correlation filter (keep the least-correlated fraction, but at
        # least n_target so a small pool is never filtered below the fill count)
        corr_s = _compute_corr_blend(cands, w_tier1, dt)
        n_keep  = max(n_target, int(np.ceil(len(cands) * corr_filter)))
        ...
        # ETF sector-stress penalty on the overlay too: demote flagged-sector
        # names in the overlay ranking so penalised sectors don't reappear via
        # the tail-diversification sleeve. ov_score is a z-scored blend (can be
        # negative), so SUBTRACT a large offset for flagged names rather than
        # multiply (which would flip signs) -- pushes them to the bottom of
        # nlargest. Offset scales with (1-penalty): full at scale=0, none at 1.
        _ov_pen_active = ((etf_penalty_scale < 1.0 and _etf_stress_flags is not None
                           and not _etf_stress_flags.empty and etf_map_s is not None)
                          or (subsector_penalty_scale < 1.0 and _ss_stress_flags is not None
                              and not _ss_stress_flags.empty and subsec_map_s is not None))
        if _ov_pen_active and not ov_score.empty:
            _pen = _combined_penalty_multiplier(
                dt, list(ov_score.index), _etf_stress_flags, etf_map_s, etf_penalty_scale,
                _ss_stress_flags, subsec_map_s, subsector_penalty_scale)
            # penalised names have _pen < 1; demote them by a big margin
            _demote = (1.0 - _pen.reindex(ov_score.index).fillna(1.0)) * 1e6
            ov_score = ov_score - _demote
        top_ov = ov_score.nlargest(min(n_target, len(ov_score))).index.tolist()
        if not top_ov:
            return pd.Series(dtype=float)
        # values are placeholders -- _apply_tier2 assigns the fixed per-stock weight
        return pd.Series(1.0 / len(top_ov), index=top_ov)
    ...
    def _apply_tier2(w_tier1, dt, non_core_cands, comp_scores, strat_idx,
        ...
        # This ensures overlay stocks never get outsized allocation when fewer
        # than (T-N) overlay slots are FILLED (partial fill): tier1_alloc is
        # computed from the ACTUAL fill count, so each overlay name gets exactly
        # fixed_ov_weight and the unfilled remainder returns to tier 1 (rather
        # than the final renormalization smearing the shortfall pro-rata, which
        # would inflate overlay weights above the fixed level).
        ...
        total_ov_alloc   = len(ov) * fixed_ov_weight
    ...
    def _build_alpha(dt, cands, excl_set=None):
        ...
        # ETF sector-stress penalty: shrink the candidate SCORES of names whose
        # mapped sector-ETF is flagged, BEFORE ranking -- so a flagged name is
        # demoted (0.25 -> may drop out of top_n; 0.00 -> effectively removed).
        # Same driver-shrink logic as the MVO sleeve (_mb_solve_mvo), applied here
        # to the selection score instead of the alpha vector, so the penalty hits
        # the momentum sleeve too (not just the MVO sleeve).
        _c_pen_active = ((etf_penalty_scale < 1.0 and _etf_stress_flags is not None
                          and not _etf_stress_flags.empty and etf_map_s is not None)
                         or (subsector_penalty_scale < 1.0 and _ss_stress_flags is not None
                             and not _ss_stress_flags.empty and subsec_map_s is not None))
        if _c_pen_active and not c.empty:
            _pen = _combined_penalty_multiplier(
                dt, list(c.index), _etf_stress_flags, etf_map_s, etf_penalty_scale,
                _ss_stress_flags, subsec_map_s, subsector_penalty_scale)
            c = c * _pen.reindex(c.index).fillna(1.0)
    ...
    def _build_mvo(dt, cands, comp_scores):
        # ETF sector-stress penalty on CANDIDATE SELECTION: shrink the ranking
        # scores of flagged-sector names BEFORE choosing the top n_cands, so
        # flagged names are demoted out of the MVO candidate pool itself (not just
        # tilted-down inside the solver, where floors/tiers can still hold them).
        # Mirrors the alpha-sleeve fix; the in-solver alpha penalty still applies
        # to any flagged name that remains in the pool.
        _cands_ranked = cands
        _cand_pen_active = ((etf_penalty_scale < 1.0 and _etf_stress_flags is not None
                             and not _etf_stress_flags.empty and etf_map_s is not None)
                            or (subsector_penalty_scale < 1.0 and _ss_stress_flags is not None
                                and not _ss_stress_flags.empty and subsec_map_s is not None))
        if _cand_pen_active and not cands.empty:
            _pen = _combined_penalty_multiplier(
                dt, list(cands.index), _etf_stress_flags, etf_map_s, etf_penalty_scale,
                _ss_stress_flags, subsec_map_s, subsector_penalty_scale)
            _cands_ranked = (cands * _pen.reindex(cands.index).fillna(1.0)).sort_values(ascending=False)
        with _SuppressOutput():
            w, diag = _mb_solve_mvo(
                dt=dt, candidates=_cands_ranked.head(n_cands).index.tolist(),
                ...
                etf_stress_flags=_etf_stress_flags, etf_map_s=etf_map_s,
                etf_penalty_scale=etf_penalty_scale,
                ss_stress_flags=_ss_stress_flags, subsec_map_s=subsec_map_s,
                subsector_penalty_scale=subsector_penalty_scale,
            )
        if not w.empty:
    ...
    def _check_trigger(s_idx, dt, w_hyp, regime):
        ...
        # ETF sector-stress penalty override: a currently-held name has had its
        # penalty NEWLY activated (0->1) -> force an immediate off-cycle rebalance,
        # skipping the calendar/turnover gate (dynamic strategies only). Removal of
        # a penalty never forces anything (asymmetric) -- it relaxes naturally at
        # the next scheduled rebalance. Flag is set in the daily edge check below.
        if st.get('penalty_rebal_forced', False):
            st['penalty_rebal_forced'] = False  # consume the flag
            return True
    ...
    # ── Helper: ETF-penalty edge detection (force off-cycle rebalance) ─────────
    def _check_penalty_edge(s_idx, dt):
        """
        For a DYNAMIC strategy, detect whether any CURRENTLY-HELD name has had its
        sector-stress penalty NEWLY activated (0->1) since the last check, from
        EITHER active layer (ETF or sub-sector). If so, force an immediate rebalance
        this date, skipping the calendar/turnover gate.

        Each layer is gated on its own scale: a layer at scale 1.0 is IGNORED for
        the trigger (its multiplier is 1.0 by construction, so it can never push a
        name's combined multiplier below 1.0). This is what makes a scale-1.0 layer
        fully inert -- no spurious rebalances from a disabled layer -- so that
        SUBSECTOR_PENALTY_SCALE=1.0 reproduces the ETF-only behaviour exactly.

        Asymmetric by design: only activation (0->1) triggers; a penalty going 1->0
        sets nothing. Held names are the strategy's actually-deployed weights.
        """
        _etf_on = (etf_penalty_scale < 1.0 and _etf_stress_flags is not None
                   and not _etf_stress_flags.empty and etf_map_s is not None)
        _ss_on  = (subsector_penalty_scale < 1.0 and _ss_stress_flags is not None
                   and not _ss_stress_flags.empty and subsec_map_s is not None)
        if not (_etf_on or _ss_on):
            return
        if s_idx not in DYNAMIC:
            return
        st = state[s_idx]
        w = st.get('weights')
        if w is None or w.empty:
            st['_pen_held_prev'] = set()
            return
        # Combined multiplier over held names; a disabled layer contributes 1.0, so
        # only ACTIVE layers can drive a name's multiplier below 1.0 (= penalized).
        _pen = _combined_penalty_multiplier(
            dt, list(w.index), _etf_stress_flags, etf_map_s, etf_penalty_scale,
            _ss_stress_flags, subsec_map_s, subsector_penalty_scale)
        now_pen = {t for t in w.index if _pen.get(t, 1.0) < 1.0}
        prev_pen = st.get('_pen_held_prev', set())
        newly = now_pen - prev_pen          # 0->1 transitions on held names
        st['_pen_held_prev'] = now_pen      # update tracker
        if newly:
            st['penalty_rebal_forced'] = True
            # Attribute each newly-penalised name to the layer(s) that flagged it,
            # so the banner reports the true trigger (ETF, sub-sector, or both) --
            # not a hardcoded "ETF" label.
            _etf_only_mult = (_etf_penalty_multiplier(dt, sorted(newly), _etf_stress_flags,
                                                      etf_map_s, etf_penalty_scale)
                              if _etf_on else None)
            _ss_only_mult  = (_etf_penalty_multiplier(dt, sorted(newly), _ss_stress_flags,
                                                      subsec_map_s, subsector_penalty_scale)
                              if _ss_on else None)
            _attrib = {}
            for t in sorted(newly):
                _layers = []
                if _etf_only_mult is not None and _etf_only_mult.get(t, 1.0) < 1.0:
                    _layers.append('ETF')
                if _ss_only_mult is not None and _ss_only_mult.get(t, 1.0) < 1.0:
                    _layers.append('SS')
                _attrib[t] = '+'.join(_layers) if _layers else '?'
            st['_pen_newly_activated'] = sorted(newly)
            st['_pen_newly_attrib'] = _attrib


    def _apply_dd(s_idx, dt, hi_c=False, lo_c=False, lo_fast=False,
                  virtual=False):
        """Update DD zone state — runs daily after NAV update.

        virtual=True (Smart/Dynamic/Dyn+Hedge): identical zone ENTRY (rolling-dd
        breach) and EXIT (theo recovery + mirror rule) machinery driving the
        construction gate, but NO gross action and NO parole (nothing was cut).

        hi_c / lo_c: today's HMM consensus flags.
          lo_c -> parole: revocable re-gross to 100% while in-zone (real only)
          hi_c -> mirror rule: blocks the permanent (theo-recovery) zone exit
        """
        st  = state[s_idx]
        nav = st['nav']

        # Update theo_nav daily when in-zone — tracks what the full-exposure
        # long book would have returned (used only for zone-exit assessment).
        # For virtual strategies nothing is cut, so this equals their own long
        # book by construction (hedge P&L deliberately excluded for a clean,
        # ladder-consistent recovery definition).
        ...
        # ── Rolling HWM (zone-entry trigger only) ─────────────────────────────
        ...
        # dd_rolling drives zone entries; st['dd'] (lifetime) drives regime
        dd_rolling = nav / st['hwm_rolling'] - 1

        # ── Zone entry / next-level breach ────────────────────────────────────
        next_lvl = st['dd_level'] + 1
        if next_lvl < len(dd_levels):
            dd_thresh, cut_frac = dd_levels[next_lvl]
            if dd_rolling <= -dd_thresh:
                # A fresh level fire while PAROLED: risk reduction wins — revoke
                # parole first (restore the pre-parole ladder gross) so the
                # multiplicative ladder math applies to the correct base. The
                # fire also INVALIDATES the current consensus streak for parole
                # purposes: the still-true lo_c must NOT re-grant same-day (that
                # would nullify the revocation and let a sticky consensus pin
                # gross at 100% through a ladder cascade). Parole becomes
                # eligible again only after the consensus breaks and re-forms.
                if (not virtual) and st.get('parole', False):
                    st['gross']  = st.get('parole_from_gross', st['gross'])
                    st['parole'] = False
                    st['dd_log'].append({
                        'dt': dt, 'event': 'PAROLE REVOKED (DD level)',
                        'dd': dd_rolling,
                        'exp_from': 1.0, 'exp_to': st['gross'],
                    })
                if not virtual:
                    st['parole_blocked'] = True
                old_gross = st['gross']
                if not virtual:
                    st['gross'] *= (1 - cut_frac)
                st['dd_active']     = True
                ...
                st['dd_log'].append({
                    'dt': dt,
                    'event': (f'ZONE ENTRY lv{next_lvl+1} (virtual)' if virtual
                              else f'DE-GROSS lv{next_lvl+1}'),
                    'dd': dd_rolling,
                    'exp_from': old_gross, 'exp_to': st['gross'],
                })

        # ── Parole: revocable re-gross on sustained low-vol consensus ─────────
        # (real ladder strategies only; construction stays pinned 'mvo' via the
        #  zone gate — parole restores EXPOSURE, not aggression)
        if (not virtual) and HMM_PAROLE and st['dd_active']:
            if not lo_c:
                # consensus broken -> a level-fire parole block clears; the NEXT
                # consensus formation (MIN_SP fresh days) is parole-eligible
                st['parole_blocked'] = False
            if (lo_c and not st.get('parole', False)
                    and not st.get('parole_blocked', False)
                    and st['gross'] < 1.0):
                # Parole cooldown: suppress a re-grant within PAROLE_COOLDOWN_DAYS
                # trading days of the last revocation, so parole returns only on
                # genuinely new regime evidence. PAROLE_COOLDOWN_SCOPE='all' gates
                # EVERY path: a revocation can be driven by the fast HIGH tier
                # suppressing lo_eff for a single day while the slow consensus
                # stays True, so gating only the fast LOW path does not stop the
                # day-to-day flip-flopping of gross exposure.
                _cooled = False
                _cool_applies = (lo_fast if str(PAROLE_COOLDOWN_SCOPE).lower() == 'fast'
                                 else True)
                if _cool_applies and PAROLE_COOLDOWN_DAYS > 0 and st.get('parole_revoke_dt') is not None:
                    _idx = Pxs_df.index
                    _rev = pd.Timestamp(st['parole_revoke_dt'])
                    # trading days elapsed since revoke (positions in the price index)
                    try:
                        _p_now = _idx.get_indexer([pd.Timestamp(dt)])[0]
                        _p_rev = _idx.get_indexer([_rev])[0]
                        if _p_now >= 0 and _p_rev >= 0 and (_p_now - _p_rev) < PAROLE_COOLDOWN_DAYS:
                            _cooled = True
                    except Exception:
                        _cooled = False
                if not _cooled:
                    st['parole_from_gross'] = st['gross']
                    st['parole']            = True
                    st['gross']             = 1.0
                    st['dd_log'].append({
                        'dt': dt,
                        'event': ('RE-GROSS (CUSUM+HMM parole)' if lo_fast
                                  else 'RE-GROSS (HMM parole)'),
                        'dd': dd_rolling,
                        'exp_from': st['parole_from_gross'], 'exp_to': 1.0,
                    })
            elif st.get('parole', False) and not lo_c:
                st['gross']  = st.get('parole_from_gross', st['gross'])
                st['parole'] = False
                st['parole_revoke_dt'] = dt      # start the fast-track cooldown clock
                st['dd_log'].append({
                    'dt': dt, 'event': 'PAROLE REVOKED',
                    'dd': dd_rolling,
                    'exp_from': 1.0, 'exp_to': st['gross'],
                })

        # ── Permanent zone exit: theo recovery, confirmed, not vol-blocked ────
        # Gate on dd_active (NOT gross<1: a paroled book has gross==1 while
        # still in-zone, and the exit clock must keep running through parole).
        if st['dd_active']:
            ...
            if theo_recovery >= dd_reentry_pct:
                ...
                st['reentry_count']  = 0
            # Mirror rule: a sustained high-vol consensus BLOCKS the exit even
            # when the recovery is confirmed (don't re-enter on a bear-market
            # rally the vol state calls unstable). The confirm counter keeps
            # accruing; the exit fires on the first unblocked confirmed day.
            if (st['reentry_count'] >= dd_reentry_confirm and
                    not (HMM_BLOCK_REENTRY and hi_c)):
                old_gross           = st['gross']
                st['gross']         = 1.0
                st['dd_active']     = False
                st['dd_level']      = -1
                st['reentry_count'] = 0
                st['parole']        = False
                st['hwm']           = nav   # reset HWM to current NAV on exit
                st['trough']        = nav
                st['theo_trough']   = nav
                st['dd_log'].append({
                    'dt': dt,
                    'event': ('ZONE EXIT (virtual)' if virtual
                              else 'RE-ENTRY 100%'),
                    'dd': theo_recovery,
    ...
    def _update_nav(s_idx, d_from, d_to, gross=None):
        ...
        # Hedge P&L. A hedge instrument that is ALSO held long (the cap residual
        # is parked in SPX) is NETTED: the effective short is hedge - long. The
        # long leg is excluded from `tks` above, so this is the only place it is
        # counted, and no exposure is double-counted in either direction.
        for inst, h in st.get('active_hedges', {}).items():
            if inst in Pxs_df.columns:
                r = -(Pxs_df.loc[d_to, inst] / Pxs_df.loc[d_from, inst] - 1)
                _long_leg = float(w.get(inst, 0.0)) if inst in w.index else 0.0
                hedge_ret += r * (h['weight'] - _long_leg)
    ...
    # ── HMM regime overlay: load exogenous vol-state consensus (once) ─────────
    def _leadin_days(lead):
        """Return trading_days extended backward by `lead` days from the full
        price index -- needed so rolling/latched signal loaders have correct
        history at the INCREMENTAL resume boundary (truncated trading_days)."""
        _all_idx = Pxs_df.index
        if len(trading_days) == 0:
            return _all_idx
        _win_start = pd.DatetimeIndex(trading_days)[0]
        _win_end   = pd.DatetimeIndex(trading_days)[-1]
        _pos = _all_idx.get_indexer([_win_start])[0]
        _ext_pos = max(0, _pos - lead)
        return _all_idx[(_all_idx >= _all_idx[_ext_pos]) & (_all_idx <= _win_end)]

    if HMM_REGIME_OVERLAY:
        # The consensus is a rolling(MIN_SP) computation, so in INCREMENTAL mode
        # (trading_days truncated to the resume window) it MUST get MIN_SP days of
        # lead-in before the window start -- else the first MIN_SP-1 days get
        # NaN/False consensus, spuriously breaking parole at the resume boundary.
        _hmm_hi, _hmm_lo, _hmm_hi1, _hmm_lo1, _hmm_missing =             _load_hmm_consensus(pd.DatetimeIndex(_leadin_days(MIN_SP + 10)))
        _hmm_missing = set(_hmm_missing)
    else:
        _hmm_hi = _hmm_lo = _hmm_hi1 = _hmm_lo1 = None
        _hmm_missing = set()

    # ── CUSUM fast tier: latch alarms + build composite flags (once) ──────────
    # hi_eff = hi_slow OR (up_latched AND any-window p_high raw)
    # lo_eff = lo_slow OR (dn_latched AND any-window p_low  raw), hi wins ties.
    # With the CUSUM tier off/unavailable the composites ARE the slow flags.
    _fast_hi = _fast_lo = None
    if CUSUM_REGIME_OVERLAY and HMM_REGIME_OVERLAY and _hmm_hi is not None:
        # CUSUM latches persist for CUSUM_ACTIVATION_WINDOW days, so it needs an
        # even longer lead-in than the HMM rolling window at the resume boundary.
        _cu_up, _cu_dn = _load_cusum_states(
            pd.DatetimeIndex(_leadin_days(CUSUM_ACTIVATION_WINDOW + 10)))
        if _cu_up is not None:
            _fast_hi = _cu_up & _hmm_hi1
            _fast_lo = _cu_dn & _hmm_lo1
    if _hmm_hi is not None:
        _eff_hi = _hmm_hi | _fast_hi if _fast_hi is not None else _hmm_hi
        _eff_lo = _hmm_lo | _fast_lo if _fast_lo is not None else _hmm_lo
        _eff_lo = _eff_lo & ~_eff_hi          # conservative side wins conflicts
        if _fast_hi is not None:
            n_fh = int((_fast_hi & ~_hmm_hi).sum())
            n_fl = int((_fast_lo & ~_hmm_lo & ~_eff_hi).sum())
            print(f"  [CUSUM tier] fast-path-only days: {n_fh} high "
                  f"(accelerated vs MIN_SP={MIN_SP}), {n_fl} low")
    else:
        _eff_hi = _eff_lo = None
    ...
    _cap_log_by_dt = {}      # dt -> (log, residual) for the tracked strategy

    def _cap_groups(w_in, s_idx=None, dt_=None):
        """Apply the sector / sub-sector allocation caps to a FINAL weight vector.
        Construction-time only: later drift above a cap is left alone (it just
        shows up as turnover in the next calculated portfolio)."""
        if ((sector_cap >= 1.0 and subsector_cap >= 1.0
             and not sector_caps and not subsector_caps)
                or w_in is None or len(w_in) == 0):
            return w_in
        w_out, resid, log = _apply_group_caps(
            w_in, sectors_s, subsec_map_s,
            sector_cap=sector_cap, subsector_cap=subsector_cap,
            max_weight=max_weight,
            sector_caps=sector_caps, subsector_caps=subsector_caps)
        if (log or resid) and s_idx == S_EXCL and dt_ is not None:
            _cap_log_by_dt[dt_] = (log, resid)
        return w_out
    ...
    def _advp_then_tier2_loop(w_t1, s_idx, dt_, cands_, non_core_cands, comp_s):
        ...
        if tier2_active:
            ...
            ov_set = set()
        w_out = _cap_groups(w_out, s_idx, dt_)
        return w_out, aff, ov_set

    import copy as _copy
    _state_snapshot_prev = None   # per-strategy state as-of the last SETTLED date
    _snapshot_date       = None   # the date that snapshot is as-of
    _provisional_dt      = Pxs_df.index[-1]   # the most-recent date -- always provisional

    for day_idx, dt in enumerate(trading_days):
        # Option B — never trust/persist the PROVISIONAL (most-recent) date's state.
        # Prices for Pxs_df.index[-1] are provisional (intraday / revised), and
        # incremental runs happen many times per day. So we snapshot each strategy's
        # state right BEFORE processing the provisional date, and persist THAT
        # snapshot (as-of the last SETTLED date). The provisional date is always
        # recomputed fresh next run FROM this settled snapshot -- so a re-run
        # OVERRIDES it rather than layering on its provisional mutations (which
        # previously caused offsetting events, e.g. parole revoke+re-grant).
        if dt == _provisional_dt:
            _state_snapshot_prev = {s: _copy.deepcopy(state[s]) for s in range(N_STRAT)}
            # as-of date = the day before the provisional one (prev settled date)
            _snapshot_date = (trading_days[day_idx - 1] if day_idx > 0
                              else _cache_last_dt)
        ...
        # ── Today's composite flags (missing HMM day -> both False + warning) ──
        _hi_c = _lo_c = _lo_fast = False
        if HMM_REGIME_OVERLAY and _eff_hi is not None:
            if dt in _hmm_missing:
                print(f"  [HMM overlay] WARNING: no HMM data for "
                      f"{pd.Timestamp(dt).date()} -- falling back to "
                      f"zone gate / DD rules for this day")
            else:
                _hi_c = bool(_eff_hi.get(dt, False))
                _lo_c = bool(_eff_lo.get(dt, False))
                if _lo_c and _fast_lo is not None:
                    # provenance for the dd_log grant label only
                    _lo_fast = bool(_fast_lo.get(dt, False)
                                    and not _hmm_lo.get(dt, False))

        # ── Zone tracking (must run BEFORE the regime chain reads zone state) ──
        # Real ladder (gross cuts + parole + mirror rule):
        for s_idx in [S_DD, S_EXCL]:
            _apply_dd(s_idx, dt, hi_c=_hi_c, lo_c=_lo_c, lo_fast=_lo_fast)
        # Virtual zones (Smart/Dynamic/Dyn+Hedge): same entry/exit machinery
        # driving the construction gate, no gross action, no parole.
        for s_idx in [S_SMART, S_DYN, S_HEDGE]:
            _apply_dd(s_idx, dt, hi_c=_hi_c, lo_c=_lo_c, virtual=True)

        # ── Regime chain (construction only; exposure handled above) ──────────
        #   1. Zone gate: in the risk-reduction zone (real or virtual, paroled
        #      or not) construction is pinned 'mvo'. DD gates alpha.
        #   2. p_high consensus -> 'mvo' (conservative override, always honored)
        #   3. p_low  consensus -> 'alpha' (only reachable OUT of zone)
        #   4. DD drawdown thresholds (existing endogenous rule)
        for s_idx in range(N_STRAT):
            st_ = state[s_idx]
            if st_.get('dd_active', False):
                st_['regime'] = 'mvo'
            elif _hi_c:
                st_['regime'] = 'mvo'
            elif _lo_c:
        ...
        if is_calc_date:
            ...
            if comp is not None:
                ...
                # Baseline (S0) ──────────────────────────────────────────
                w_bl = _cap_groups(_build_baseline(dt), S_BASE, dt)
                ...
                def _advp_then_tier2(w_t1, s_idx, non_core_cands, comp_s):
                    ...
                    if tier2_active:
                        ...
                        ov_set = set()
                    w_out = _cap_groups(w_out, s_idx, dt)
                    return w_out, aff, ov_set
            ...
            if _any_eligible:
                ...
                for s_idx in _dyn_list:
                    ...
                    if s_idx == S_EXCL:
                        ...
                        comp_s = comp_dyn
                        w_hyp  = _wm.copy()
                        # Book is ALWAYS the MVO construction, independent of the
                        # DD-regime label. Pin the trigger regime to 'mvo' so (a)
                        # the TO trigger uses to_thresh_mvo (not to_thresh_alpha
                        # via the shallow-dd label), and (b) the regime_switch
                        # trigger can never fire spuriously on DD-label changes
                        # that don't alter this book.
                        reg = 'mvo'
                    else:
                        comp_s = comp_dyn
                        w_hyp = _regime_weights(dt, reg, {
                    ...
                    # ETF-penalty edge: newly-activated penalty on a held name forces
                    # an off-cycle rebalance (dynamic strategies only). Runs BEFORE
                    # the trigger check so the forced flag is seen this same date.
                    _check_penalty_edge(s_idx, dt)

                    # FACTOR-penalty activation edge: a trigger factor's flag turned
                    # ON since the previous trading day -> force an off-cycle
                    # rebalance (dynamic strategies). Factor-level event: it fires
                    # regardless of holdings, since the whole composite shifts.
                    if _fac_edges and s_idx in DYNAMIC:
                        _fx = [f for f, _ed in _fac_edges.items()
                               if any((prev_dt is None or prev_dt < _d) and _d <= dt
                                      for _d in _ed)]
                        if _fx:
                            st['penalty_rebal_forced'] = True
                            st['_fac_newly_activated'] = _fx

                    # Check rebalance trigger
                    _did_rebal = False
                    if st.get('penalty_rebal_forced', False):
                        _was_pen_forced = st.pop('_pen_newly_activated', None)
                        _pen_attrib     = st.pop('_pen_newly_attrib', None)
                        _fac_forced     = st.pop('_fac_newly_activated', None)
                    else:
                        _was_pen_forced = None
                        _pen_attrib     = None
                        _fac_forced     = None
                    ...
                    if s_idx == S_EXCL and _did_rebal:
                        ...
                        _forced   = _was_pen_forced
                        if _fac_forced:
                            print(f"\n  *** {dt.date()} OFF-CYCLE REBALANCE forced by "
                                  f"FACTOR penalty activation: "
                                  f"{', '.join(_fac_forced)} ***", flush=True)
                        if _forced:
                            # Report each newly-penalised name WITH the layer that
                            # flagged it (ETF, SS = sub-sector, or ETF+SS), and date
                            # the banner so it can't be misread as an adjacent event.
                            if _pen_attrib:
                                _items = [f"{t}({_pen_attrib.get(t, '?')})"
                                          for t in _forced]
                                _layers_seen = sorted({l for v in _pen_attrib.values()
                                                       for l in v.split('+') if l in ('ETF', 'SS')})
                                _lbl = '+'.join(_layers_seen) if _layers_seen else 'penalty'
                            else:
                                _items = list(_forced)
                                _lbl = 'penalty'
                            _blead = "" if _fac_forced else "\n"
                            print(f"{_blead}  *** {dt.date()} OFF-CYCLE REBALANCE forced by "
                                  f"{_lbl} activation on held: {', '.join(_items)} ***",
                                  flush=True)
                        # No blank line between the banner and its header: the banner
                        # belongs to THIS portfolio (below), not the block above. Lead
                        # with a blank line only when there was NO banner (so a normal
                        # rebalance is still separated from the previous date's output).
                        _hdr_lead = "" if (_forced or _fac_forced) else "\n"
                        print(f"{_hdr_lead}  -- {dt.date()}  [DD+Excl #{_rebal_n}]  "
                        ...
                        # ETF sector-stress penalty: show which sectors are flagged
                        # on this date and how many held names they penalise.
                        if (_etf_stress_flags is not None
                                and not _etf_stress_flags.empty
                                and etf_penalty_scale < 1.0):
                            _past = _etf_stress_flags.loc[:dt]
                            if not _past.empty:
                                _frow = _past.iloc[-1]
                                _flagged = [e for e in _frow.index
                                            if pd.notna(_frow[e]) and int(_frow[e]) == 1]
                                if _flagged and etf_map_s is not None:
                                    # count held names whose ETF is flagged
                                    _held_pen = {e: [] for e in _flagged}
                                    for _t in w_hyp.index:
                                        _e = etf_map_s.get(_t, None)
                                        if _e in _held_pen:
                                            _held_pen[_e].append(_t)
                                    _parts = []
                                    for _e in sorted(_flagged):
                                        _names = _held_pen.get(_e, [])
                                        if _names:
                                            _parts.append(f"{_e}[{','.join(_names)}]")
                                        else:
                                            _parts.append(_e)
                                    print(f"  PENALTY (x{etf_penalty_scale:.2f}) "
                                          f"flagged sectors: {'  '.join(_parts)}",
                                          flush=True)
                                elif _flagged:
                                    print(f"  PENALTY (x{etf_penalty_scale:.2f}) "
                                          f"flagged sectors: {'  '.join(sorted(_flagged))}",
                                          flush=True)
                        # SUB-SECTOR penalty: same exhibit for the finer layer,
                        # only shown when the sub-sector layer is active (<1.0).
                        if (_ss_stress_flags is not None
                                and not _ss_stress_flags.empty
                                and subsector_penalty_scale < 1.0):
                            _sspast = _ss_stress_flags.loc[:dt]
                            if not _sspast.empty:
                                _ssrow = _sspast.iloc[-1]
                                _ssflagged = [e for e in _ssrow.index
                                              if pd.notna(_ssrow[e]) and int(_ssrow[e]) == 1]
                                if _ssflagged and subsec_map_s is not None:
                                    _sshp = {e: [] for e in _ssflagged}
                                    for _t in w_hyp.index:
                                        _e = subsec_map_s.get(_t, None)
                                        if _e in _sshp:
                                            _sshp[_e].append(_t)
                                    _ssparts = []
                                    for _e in sorted(_ssflagged):
                                        _names = _sshp.get(_e, [])
                                        _ssparts.append(f"{_e}[{','.join(_names)}]"
                                                        if _names else _e)
                                    print(f"  PENALTY-SS (x{subsector_penalty_scale:.2f}) "
                                          f"flagged sub-sectors: {'  '.join(_ssparts)}",
                                          flush=True)
                        # FACTOR WEIGHTS actually used to build the composite at
                        # this date: normalised so they sum to 1 (the composite's
                        # final z-score makes only relative weight matter). When a
                        # factor penalty is active, both the pre-penalty and the
                        # post-penalty (renormalised) weights are shown.
                        _wt_rec = _wt_by_date.get(dt)
                        if _wt_rec:
                            def _wfmt(d):
                                _tot = sum(v for v in d.values() if v == v)
                                if _tot <= 0:
                                    return "  ".join(f"{k}={v:.3f}" for k, v in d.items())
                                return "  ".join(f"{k}={v/_tot:.3f}"
                                                 for k, v in d.items())
                            _wpre, _wpost = _wt_rec.get('pre', {}), _wt_rec.get('post', {})
                            _changed = any(abs(_wpre.get(k, 0) - _wpost.get(k, 0)) > 1e-12
                                           for k in set(_wpre) | set(_wpost))
                            if _changed:
                                print(f"  FACTOR WEIGHTS pre-penalty : {_wfmt(_wpre)}",
                                      flush=True)
                                print(f"  FACTOR WEIGHTS post-penalty: {_wfmt(_wpost)}",
                                      flush=True)
                            else:
                                print(f"  FACTOR WEIGHTS: {_wfmt(_wpost)}", flush=True)
                        # SECTOR / SUB-SECTOR caps that bound at this rebalance
                        _cl = _cap_log_by_dt.get(dt)
                        if _cl:
                            _clog, _cres = _cl
                            _seen, _parts = set(), []
                            for _d in _clog:
                                ...
                                _seen.add(_k)
                                _parts.append(
                                    f"{_d['name']}[{_d['level'][:3]} "
                                    f"{_d.get('cap', 0)*100:.0f}%"
                                    + (" user" if _d.get('src') == 'user' else "")
                                    + f"] {_d['pre']*100:.1f}%->{_d['post']*100:.1f}%")
                            if _parts:
                                print(f"  CAPS (sec={sector_cap:.0%}/sub={subsector_cap:.0%}) "
                                      f"binding: {'  '.join(_parts)}", flush=True)
                            if _cres > 1e-12:
                                print(f"  CAPS residual parked in {CAP_RESIDUAL_TICKER}: "
                                      f"{_cres*100:.2f}%", flush=True)
                        # FACTOR penalty: which factor weights are being scaled down
                        # at THIS rebalance (PIT: last flag on/before dt), with the
                        # scale applied to each -- the factor-level analogue of the
                        # PENALTY / PENALTY-SS lines above.
                        if not _fac_flags.empty:
                            _fpast = _fac_flags.loc[:dt]
                            if not _fpast.empty:
                                _frow2 = _fpast.iloc[-1]
                                _facon = [f"{f}(x{_fac_scales[f]:.2f})"
                                          for f in _fac_flags.columns
                                          if _fac_scales.get(f, 1.0) < 1.0
                                          and int(_frow2[f]) == 1]
                                if _facon:
                                    print(f"  PENALTY-FAC flagged factors: "
                                          f"{'  '.join(_facon)}", flush=True)
                        # MR top scores
    ...
    # Option B: persist the state snapshot taken BEFORE the provisional date, keyed
    # to the last SETTLED date -- so the provisional date is always recomputed fresh
    # next run. Works for single-day incremental runs too (snapshot = the restored
    # prior-day state). Falls back to final state only if no provisional date was
    # processed (shouldn't happen, but safe).
    if _state_snapshot_prev is not None and _snapshot_date is not None:
        _save_state_by_sidx = _state_snapshot_prev
        _save_state_date    = _snapshot_date
    else:
        _save_state_by_sidx = {s: state[s] for s in range(N_STRAT)}
        _save_state_date    = (trading_days[-1] if len(trading_days) else _cache_last_dt)
    for s_idx in range(N_STRAT):
        lbl = STRATEGY_LABELS[s_idx]
        _cache_save_nav(ph, lbl, _new_nav[s_idx])
        _cache_save_state(ph, lbl, _save_state_by_sidx[s_idx], _save_state_date)
    ...
    def _build_results(ns, st, labels, params_h):
        ...
        return {
            ...
            'factor_pen_flags':           _fac_flags,
            'factor_pen_scales':          _fac_scales,
            'factor_weights_by_date':     _wt_by_date,
            'universe':                   universe,
        }
    ...
    # ── Monthly returns — rolling last 12 months (same format as yearly) ──────
    # Month-over-month return per strategy: nav at month-end / prior month-end - 1.
    # Uses each strategy's own NAV; the AUM column uses the tracked (Dyn+Hedge) NAV.
    print(f"\n  Monthly returns  (rolling last 12 months)")
    print(hdr)
    print(_sep)
    # month-end NAV series per strategy (last obs of each calendar month)
    def _month_end_nav(nav_s):
        if nav_s is None or nav_s.empty:
            return pd.Series(dtype=float)
        return nav_s.groupby([nav_s.index.year, nav_s.index.month]).last()
    _me = {s_idx: _month_end_nav(nav_series[s_idx]) for s_idx, _ in _COLS}
    _me_aum = _month_end_nav(_aum_nav)
    # the set of (year, month) keys present, last 13 needed to get 12 returns
    _all_months = sorted(_me_aum.index) if len(_me_aum) else []
    _last13 = _all_months[-13:]                 # 13 month-ends -> 12 MoM returns
    for i in range(1, len(_last13)):
        (py, pm) = _last13[i - 1]
        (cy, cm) = _last13[i]
        _label = f"{cy}-{cm:02d}"
        row = f"  {_label:<6}"
        for s_idx, _ in _COLS:
            s = _me[s_idx]
            if (py, pm) in s.index and (cy, cm) in s.index and s[(py, pm)] != 0:
                r = (s[(cy, cm)] / s[(py, pm)] - 1) * 100
            else:
                r = np.nan
            row += f"  {_fmt_ret(r)}"
        # AUM at this month-end (tracked NAV)
        _aum_close = (aum * float(_me_aum[(cy, cm)])
                      if (cy, cm) in _me_aum.index else aum)
        row += f"  ${_aum_close/1e6:>{_COL-2}.1f}M"
        print(row)
    ...
    return {
        ...
        'score_dfs':                  score_dfs,
        'ic_weights_by_date':         weights_by_date,
        # ── FACTOR-regime penalty (for independent auditing) ──────────────────
        #   factor_pen_flags : DataFrame dates x factor, 1 = penalty ACTIVE
        #   factor_weights_by_date : {dt: {'pre': {...}, 'post': {...}}}
        'factor_pen_flags':           _fac_flags,
        'factor_pen_scales':          _fac_scales,
        'factor_weights_by_date':     _wt_by_date,
        'universe':                   universe,
