...
# cache). Consensus across ALL THREE windows (1y/3y/5y), held for MIN_SP
# consecutive days, OVERRULES the DD-based regime for portfolio construction:
#   * all windows p_high > HMM_THRESH_HIGH  ->  regime 'mvo'   (max conservative)
#   * all windows p_low  > HMM_THRESH_LOW   ->  regime 'alpha'
#   * otherwise                             ->  fall through to the chain below
# Precedence: HMM consensus > de-grossed-pin (gross<100% -> 'mvo') > DD levels.
# Rationale: the DD regime is ENDOGENOUS (measured on the strategy's own,
# protected NAV) so successful de-grossing suppresses it (2022: de-grossed for
# 18 months yet regime read 'hybrid'). The HMM measures the MARKET's vol state
# and cannot be suppressed by the book's own protection. HMM_ASSET defaults to
# the high-vol quintile index -- the riskiest subset of the market, the best
# fit to this book's risk profile.
...
HMM_THRESH_HIGH     = 0.30              # p_high consensus threshold -> 'mvo'
HMM_THRESH_LOW      = 0.30              # p_low  consensus threshold -> 'alpha'
...
def _load_hmm_consensus(trading_days: pd.DatetimeIndex,
    ...
    Returns (override, missing):
      override : pd.Series over trading_days with values 'mvo' / 'alpha' / None
                 'mvo'   when ALL HMM_WINDOWS have p_high > HMM_THRESH_HIGH,
                         sustained for >= MIN_SP consecutive days (PIT: the
                         probabilities on each date are filtered as-of that
                         date's close; the persistence window looks BACKWARD)
                 'alpha' when ALL windows have p_low > HMM_THRESH_LOW, same
                         persistence rule
                 None    otherwise (fall through to de-gross pin / DD levels)
      missing  : pd.DatetimeIndex of trading_days with no usable HMM data
                 (never treated as consensus; caller warns)

    On any failure (table absent, no rows) returns (all-None, all-days-missing)
    so the backtest degrades gracefully to the DD-based regime.
    """
    a = asset or HMM_ASSET
    none_override = pd.Series([None] * len(trading_days), index=trading_days,
                              dtype=object)
    try:
        ...
        return none_override, pd.DatetimeIndex(trading_days)
    ...
    # today (backward-looking rolling window -> strictly PIT)
    hi_ok = hi_raw.rolling(MIN_SP, min_periods=MIN_SP).sum().eq(MIN_SP)
    lo_ok = lo_raw.rolling(MIN_SP, min_periods=MIN_SP).sum().eq(MIN_SP)

    override = none_override.copy()
    override[hi_ok] = 'mvo'      # p_high consensus wins if (pathologically) both
    override[lo_ok & ~hi_ok] = 'alpha'
    ...
    if verbose:
        ...
        print(f"  [HMM overlay] asset={a}  windows={HMM_WINDOWS}  "
              ...
              f"MIN_SP={MIN_SP}")
        print(f"  [HMM overlay] override days: {n_hi} -> MVO, {n_lo} -> alpha, "
...
def run_backtest(
    ...
    if HMM_REGIME_OVERLAY:
        _hmm_override, _hmm_missing = _load_hmm_consensus(
            pd.DatetimeIndex(trading_days))
        _hmm_missing = set(_hmm_missing)
    else:
        _hmm_override, _hmm_missing = None, set()
    ...
    for day_idx, dt in enumerate(trading_days):
        ...
        # Update regime for all strategies.
        # Precedence chain (per design):
        #   1. HMM consensus override ('mvo'/'alpha') -- exogenous market vol
        #      state, unsuppressible by the book's own protection; overrules
        #      everything for CONSTRUCTION choice (exposure/gross untouched --
        #      the DD ladder still governs de-gross/re-entry independently).
        #   2. De-grossed pin: while gross < 100%, construction is pinned to
        #      max-conservative 'mvo' (a de-grossed book should never run a
        #      concentrated alpha construction).
        #   3. DD drawdown thresholds (existing endogenous rule).
        _hmm_reg = None
        if HMM_REGIME_OVERLAY and _hmm_override is not None:
            if dt in _hmm_missing:
                print(f"  [HMM overlay] WARNING: no HMM data for "
                      f"{pd.Timestamp(dt).date()} -- falling back to "
                      f"de-gross pin / DD rules for this day")
            else:
                _hmm_reg = _hmm_override.get(dt)      # 'mvo' | 'alpha' | None
        for s_idx in range(N_STRAT):
            if _hmm_reg is not None:
                state[s_idx]['regime'] = _hmm_reg
            elif state[s_idx].get('gross', 1.0) < 1.0:
                state[s_idx]['regime'] = 'mvo'        # de-grossed -> pinned
            else:
        ...
        for s_idx in [S_DD, S_EXCL]:
            _apply_dd(s_idx, dt)
            # _apply_dd sets regime to the DD-level regime on a firing day; the
            # HMM consensus outranks it for CONSTRUCTION choice (the gross cut
            # and the min-hold-overriding forced rebalance still stand -- only
            # the construction mode stays with the exogenous signal).
            if _hmm_reg is not None:
                state[s_idx]['regime'] = _hmm_reg
