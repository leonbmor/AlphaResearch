...
HMM_THRESH_HIGH     = 0.30              # p_high consensus threshold
HMM_THRESH_LOW      = 0.30              # p_low  consensus threshold
MIN_SP              = 3                 # consensus must hold N consecutive days
...
def _load_hmm_consensus(trading_days: pd.DatetimeIndex,
    ...
    try:
        ...
        return None, None, pd.DatetimeIndex(trading_days)
...
def run_backtest(
    ...
    def _apply_dd(s_idx, dt, hi_c=False, lo_c=False, virtual=False):
        ...
        if (not virtual) and HMM_PAROLE and st['dd_active']:
            ...
            if (lo_c and not st.get('parole', False)
                ...
                st['gross']             = 1.0
                st['dd_log'].append({
                    'dt': dt, 'event': 'RE-GROSS (HMM parole)',
                    'dd': dd_rolling,
    ...
    if HMM_REGIME_OVERLAY:
        _hmm_hi, _hmm_lo, _hmm_missing = _load_hmm_consensus(
            pd.DatetimeIndex(trading_days))
        _hmm_missing = set(_hmm_missing)
    else:
        _hmm_hi, _hmm_lo, _hmm_missing = None, None, set()
    ...
    for day_idx, dt in enumerate(trading_days):
        ...
        # ── Today's HMM consensus flags (missing day -> both False + warning) ──
        _hi_c = _lo_c = False
        if HMM_REGIME_OVERLAY and _hmm_hi is not None:
            if dt in _hmm_missing:
                ...
                _hi_c = bool(_hmm_hi.get(dt, False))
                _lo_c = bool(_hmm_lo.get(dt, False))
        ...
        for s_idx in [S_DD, S_EXCL]:
            _apply_dd(s_idx, dt, hi_c=_hi_c, lo_c=_lo_c)
