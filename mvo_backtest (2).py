...
def _mb_build_x_snapshots(rebal_dates, Pxs_df, sectors_s,
    ...
    Reuses _rd_build_X and _rd_month_end_dates from portfolio_risk_decomp.
    """
...
def _mb_solve_mvo(dt, candidates, composite_scores, Pxs_df, sectors_s,
    ...
    try:
        ...
        warnings.warn(f"  Cov matrix build failed {dt.date()}: {e}")
        return pd.Series(dtype=float)
    ...
    # Eligibility filter: stocks selected by >= min_matrix_count matrices
    top_n_by = {}
    for mname, Sigma in matrices.items():
        w_m = _mvo_max_alpha_portfolio(
            Sigma, alpha, valid,
            risk_aversion=risk_aversion,
            max_weight=max_weight,
            min_weight=0.0   # no floor for eligibility check
        )
        top_n_by[mname] = set(w_m[w_m > 1e-4].index.tolist())

    eligible = [t for t in valid
                if sum(1 for m in top_n_by if t in top_n_by[m])
                >= min_matrix_count]

    if len(eligible) < 3:
        eligible = valid   # fallback: use all valid

    # Final MVO on eligible universe with ensemble covariance
    ...
    # Step 1: solve unconstrained (long-only, sum=1)
    try:
        import cvxpy as cp
        N  = len(eligible)
        w  = cp.Variable(N)
        ...
        Sc = S_ens_el
        prob = cp.Problem(
            cp.Maximize(a @ w - (risk_aversion / 2) * cp.quad_form(w, Sc)),
            [cp.sum(w) == 1, w >= 0]
        )
        prob.solve(solver=cp.OSQP, verbose=False)
        if prob.status in ['optimal', 'optimal_inaccurate'] and w.value is not None:
            w_raw = pd.Series(np.maximum(w.value, 0.0), index=eligible)
            if w_raw.sum() > 0:
                w_raw = w_raw / w_raw.sum()
        else:
            w_raw = pd.Series(1.0 / N, index=eligible)
    except Exception:
        w_raw = pd.Series(1.0 / len(eligible), index=eligible)

    # Step 2: apply floor then cap
    w_ens = _mvo_floor_then_cap(w_raw, min_weight=min_weight,
                                  max_weight=max_weight)

    return w_ens.reindex(candidates).fillna(0.0)
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    force_rebuild_cache: bool  — rebuild X snapshots from scratch (default True)
    """
    print("=" * 72)
    ...
    for i, dt in enumerate(calc_dates):
        ...
        if not w_mvo.empty and w_mvo.sum() > 0:
            mvo_weights_by_date[dt] = w_mvo[w_mvo > 1e-6]
