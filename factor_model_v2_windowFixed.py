...
_STALE_RUN_LIMIT  = 3
MIN_HIST          = 126    # minimum trading days of valid price history for universe inclusion
                           # and as min_periods for all rolling calculations
QUALITY_ANCHOR_TBL = 'valuation_metrics_anchors'
...
def get_universe(Pxs_df: pd.DataFrame, sectors_s: pd.Series,
    ...
    """
    Build the broad candidate universe: all sector-mapped, DB-present stocks
    that have ANY valid price in Pxs_df. This is a superset — per-date
    eligibility (MIN_HIST) is enforced inside the cross-sectional loop via
    get_eligible_at_date(). No pre-start history requirement here, allowing
    recent IPOs and re-listings to enter the model as soon as they accumulate
    sufficient history.
    """
    try:
...
def get_eligible_at_date(universe: list, Pxs_df: pd.DataFrame,
                         dt: pd.Timestamp,
                         min_hist: int = MIN_HIST) -> list:
    """
    Per-date eligible set: stocks in universe with at least min_hist non-NaN
    prices up to and including dt. Stocks with shorter history (new IPOs,
    re-listings) are excluded until they clear the threshold; delisted stocks
    drop out naturally as prices go NaN.

    Parameters
    ----------
    universe  : broad candidate list from get_universe()
    Pxs_df    : full price panel
    dt        : current calculation date
    min_hist  : minimum trading days of valid price history (default MIN_HIST=126)
    """
    px_to_dt = Pxs_df.loc[Pxs_df.index <= dt, universe]
    valid_counts = px_to_dt.notna().sum()
    eligible = valid_counts[valid_counts >= min_hist].index.tolist()
    return eligible
    ...
    for val_date, grp in df_all.groupby('date'):
        ...
        for m in VALUE_METRICS:
            ...
            for sec, sg in snap.groupby('_sector'):
                ...
                normed = (1.0 - (ranks - 1) / (len(ranks) - 1))                          if len(ranks) > 1 else pd.Series(0.5, index=ranks.index)
