...
def run_daily_cache_build(
    ...
    # Penalised composite is now built inline per date by modifying comp_scores
    # No separate full recomputation needed — eliminates floating point divergence
    if MR_K > 0:
        print(f"  Penalised composite: built inline from standard (MR_K={MR_K})")
    ...
    for idx, dt in enumerate(to_compute):
        ...
        comp_scores_pen = comp_scores.copy()
        if MR_K > 0:
            _mr_dt = _load_momentum_exclusions(dt)
            for _tkr, _mr_s in _mr_dt.items():
                if _tkr in comp_scores_pen and _mr_s > 0:
                    comp_scores_pen[_tkr] = comp_scores_pen[_tkr] / np.exp(MR_K * _mr_s)
        ...
        def _advp_filter_replace(w: pd.Series, label: str) -> pd.Series:
            if volumeRaw_df is None or w.empty:
                return w
            _aum = float(aum_by_date[aum_by_date.index <= dt].iloc[-1]) \
                   if (aum_by_date is not None and not aum_by_date.empty
        ...
        # Excl portfolios — derived from standard portfolios with MR penalty applied
        try:
            _mr_n  = int(MR_CAP * len(cands)) if MR_CAP > 0 else 0
            excl_d = _load_momentum_exclusions(dt, top_n=_mr_n) if _mr_n > 0 else {}

            if not excl_d and MR_K == 0:
                # No penalty, no exclusions — excl = standard (guaranteed convergence)
                w_ae    = w_alpha.copy()
                w_me_nz = w_mvo_nz.copy()
                w_he    = w_hybrid.copy()
            else:
                # Apply MR penalty: start from standard alpha portfolio.
                # Only remove stocks with positive MR scores if penalised enough
                # to drop below the next-best zero-MR candidate.
                # Stocks with zero MR score ALWAYS stay if they're in standard portfolio.

                # Step 1: remove hard-excluded stocks from standard portfolio
                alpha_tickers = list(w_alpha.index)
                if excl_d:
                    alpha_tickers = [t for t in alpha_tickers if t not in excl_d]

                # Step 2: for remaining standard stocks, check if any penalised-MR stock
                # ranks below a better zero-MR candidate from the full 125-pool
                cands_pen = comp_scores_pen.dropna()
                cands_pen = cands_pen.loc[[t for t in cands_pen.index if t in pxs_cols]]
                if excl_d:
                    cands_pen = cands_pen[~cands_pen.index.isin(excl_d.keys())]
                re_ = cands_pen.sort_values(ascending=False)

                # Final selection: top_n from penalised ranking
                # Zero-MR stocks in standard portfolio keep their original score
                # so they can only be displaced by genuinely higher-scoring names
                ...
                if conc_factor == 1.0 or n_ < 2:
                    ...
                    ta_ = conc_factor / (conc_factor + 1.0)
                    ba_ = 1.0 / (conc_factor + 1.0)
                    w_ae = pd.Series({t: (ta_ / nt_ if j < nt_ else ba_ / nb_)
                                      for j, t in enumerate(ap_)})
                try:
                    with _SuppressOutput():
                        w_me, _ = _mb_solve_mvo(
                            dt=dt, candidates=re_.head(n_cands).index.tolist(),
                    ...
                    w_he = (w_ae.reindex(at_).fillna(0) +
                            w_me_nz.reindex(at_).fillna(0)) / 2.0
                    w_he = w_he[w_he > 0]
                    if w_he.sum() > 0:
                        w_he = w_he / w_he.sum()
                except Exception:
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
                ...
                if 'alpha' in port_cache and 'mvo' in port_cache:
                    ...
                    if volumeRaw_df is not None and not _candidates_cache.empty:
                        _w_alpha_c = _advp_filter_and_replace(
                            ...
                            Pxs_df, volumeRaw_df, _current_aum_c,
    ...
    if past_rebals:
        ...
        # Compute current drawdown for Smart Hybrid
        _sh_dd_str = ''
        if nav_smart is not None and not nav_smart.empty:
            _sh_nav_to_date = nav_smart[nav_smart.index <= today_ts]
            if not _sh_nav_to_date.empty:
                _sh_peak = _sh_nav_to_date.cummax().iloc[-1]
                _sh_curr = _sh_nav_to_date.iloc[-1]
                _sh_dd   = (_sh_curr / _sh_peak - 1) * 100
                _sh_dd_str = f"  |  DD={_sh_dd:+.1f}%"
        print(f"\n  Rebalance: {live_dt.date()}  |  As of: {today_ts.date()}  "
              f"|  {len(w_live)} positions{_sh_dd_str}")
    ...
    _s9_rebal_only = {dt: w for dt, w in _s9_wbd.items() if dt in _s9_costs} if _s9_wbd else {}
    if _s9_wbd:
        _first_s9 = sorted(_s9_wbd.keys())[0]
        _s9_rebal_only[_first_s9] = _s9_wbd[_first_s9]
    if _s9_rebal_only:
        print("\n  " + "=" * 72)
        print(f"  LIVE DYN+EXCL PORTFOLIO (Strategy 9, MR_CAP={MR_CAP:.1%}) — P&L SINCE LAST REBALANCE")
        print("  " + "=" * 72)
        _live_pnl_hybrid(_s9_rebal_only, Pxs_df)

        print("\n  " + "=" * 72)
        print("  CURRENT LIVE DYN+EXCL PORTFOLIO (as of last rebalance)")
        print("  " + "=" * 72)
        _past_s9 = sorted([d for d in _s9_rebal_only if d <= today_ts])
        if _past_s9:
            _ldt9 = _past_s9[-1]
            _wl9  = _s9_wbd[_ldt9]; _wl9 = _wl9[_wl9 > 1e-6].sort_values(ascending=False)
            _tk9  = [t for t in _wl9.index if t in Pxs_df.columns]
            if _ldt9 == today_ts:
                _wd9 = _wl9.reindex(_tk9) if _tk9 else _wl9
            elif _tk9 and _ldt9 in Pxs_df.index:
                _px9r = Pxs_df.loc[_ldt9, _tk9]; _px9t = Pxs_df.loc[today_ts, _tk9]
                _wd9_ = _wl9.reindex(_tk9) * (_px9t/_px9r).fillna(1)
                _wd9  = (_wd9_/_wd9_.sum()) if _wd9_.sum() > 0 else _wl9.reindex(_tk9)
            else:
                _wd9 = _wl9.reindex(_tk9) if _tk9 else _wl9
            # Trigger info from strategy 8 log (same schedule)
            _lrec9 = next((r for r in reversed(_dyn_rebal_log) if r['dt'] == _ldt9), None)
            _trig9 = f"  trigger={_lrec9['trigger']}  [{_lrec9.get('trigger_type','').upper()}]" if _lrec9 else ""
            # Compute current drawdown for Strategy 9
            _s9_dd_str = ''
            if nav_dd_excl is not None and not nav_dd_excl.empty:
                _s9_nav_to_date = nav_dd_excl[nav_dd_excl.index <= today_ts]
                if not _s9_nav_to_date.empty:
                    _s9_peak = _s9_nav_to_date.cummax().iloc[-1]
                    _s9_curr = _s9_nav_to_date.iloc[-1]
                    _s9_dd   = (_s9_curr / _s9_peak - 1) * 100
                    _s9_dd_str = f"  DD={_s9_dd:+.1f}%"
            print(f"\n  Rebalance: {_ldt9.date()}  |  As of: {today_ts.date()}  "
                  f"|  {len(_wl9)} positions{_trig9}"
                  + (f"  |{_s9_dd_str}" if _s9_dd_str else ""))
            print(f"  {'Ticker':<8}  {'Rebal Wt%':>10}  {'Curr Wt%':>10}  {'Drift':>8}  Sector")
            print("  " + "-" * 60)
            for tkr in _wl9.index:
                rw = _wl9.get(tkr,0.0)*100; cw = _wd9.get(tkr,rw/100)*100 if tkr in _tk9 else rw
                print(f"  {tkr:<8}  {rw:>9.2f}%  {cw:>9.2f}%  {cw-rw:>+7.2f}%  {sectors_s.get(tkr,'')}")
