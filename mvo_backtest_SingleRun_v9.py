...
def _advp_filter_and_replace(weights: pd.Series,
    ...
    def _alloc_tier(tickers, budget):
        """Allocate budget across tickers respecting ADVP caps and min/max."""
        ...
        for _ in range(20):
            ...
            for tkr in list(w.index):
                cap = _advp_caps.get(tkr, max_weight)
                effective_cap = min(cap, max_weight)
                if w[tkr] > effective_cap + 1e-9:
                    w[tkr] = effective_cap
                    changed = True
            # Apply min_weight floor — drop stocks below floor
            to_drop = [t for t in w.index if w[t] < min_weight - 1e-9]
            if to_drop:
                w = w.drop(to_drop)
                changed = True
            # Renorm to budget
            ...
            if not changed:
                break
        return w, budget - w.sum()  # return weights + any unallocatable excess
