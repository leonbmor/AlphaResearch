...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    for i, dt in enumerate(calc_dates):
        ...
        if not w_mvo.empty and w_mvo.sum() > 0:
            ...
            # ── Per-date portfolio display (mirrors mvo_diagnostics output) ───
            eff_n     = 1.0 / (w_mvo_nz**2).sum() if len(w_mvo_nz) > 0 else 0
            ...
            print(f"\n  ── {dt.date()}  [{i+1}/{n}]  "
                  f"n={len(w_mvo_nz)}  eff_N={eff_n:.1f}  "
                  f"min={w_mvo_nz.min():.1%}  max={w_mvo_nz.max():.1%} ──")
            print(f"  {'Ticker':<8}  {'Weight%':>7}  {'AnnAlpha%':>10}  "
                  f"{'AnnVol%':>8}  {'Z-score':>8}  {'Sector':<28}  In matrices")
            print(f"  {'-'*95}")
            for tkr, wt in w_mvo_nz[w_mvo_nz > 1e-6].sort_values(ascending=False).items():
                sec  = sectors_s.get(tkr, '')
                a    = alpha_ann.get(tkr, 0.0) * 100
                v    = vol_ann.get(tkr,   0.0) * 100
                z    = z_s.get(tkr,       0.0)
                # Show which individual matrices selected this stock + E for ensemble
                in_m = '/'.join(m for m in ['E','L','F','P']
                                if tkr in top_n_by.get(m, []))
                if in_m:
                    in_m += '/E'
                print(f"  {tkr:<8}  {wt*100:>6.2f}%  {a:>+9.1f}%  "
                      f"{v:>7.1f}%  {z:>+8.3f}  {sec:<28}  {in_m}")

    # ── Hybrid portfolio: MVO sleeve + Alpha complement ─────────────────────
