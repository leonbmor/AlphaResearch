...
def _ics_save_ic_cache(ic_results_new, model_version):
    """Save new IC values to cache table."""
    ...
    with ENGINE.begin() as conn:
        conn.execute(text(f"""
            DELETE FROM {ICS_IC_CACHE_TBL}
            WHERE date = ANY(:dates) AND model_version = :mv
        """), {'dates': dates, 'mv': model_version})
    df.to_sql(ICS_IC_CACHE_TBL, ENGINE, if_exists='append', index=False)
...
if __name__ == "__main__":
    print("Usage:")
    print("  ic_results, ic_annual = run_ic_study_alpha(Pxs_df, sectors_s, model_version='v2')")
    print("  weights_by_year, regime_s, weights_by_date = compute_rolling_regime_weights(")
    print("      ic_results, ic_annual, Pxs_df, horizons=['21d', '63d'])")
