...
def run_backtest(
    ...
    _strat_lbls_yr = ['Baseline', 'Pure Alpha', 'MVO', 'Hybrid', 'Smart',
                      'Dynamic', 'Dyn+Hdg', 'DD Pol', 'Excl']
    print(f"\n  Yearly returns  (starting AUM: ${aum/1e6:.1f}M)")
    hdr = (f"  {'Year':<6} {'Baseline':>10} {'Pure Alpha':>12} {'MVO':>9} "
           f"{'Hybrid':>9} {'Smart':>9} {'Dynamic':>9} {'Dyn+Hdg':>9} "
           f"{'DD Pol':>8} {'Excl':>8} {'AUM($M)':>9}")
    print(hdr)
    # DD row — current drawdown per strategy, aligned under each column
    def _fmt_dd(s_idx):
        if s_idx == S_BASE: return f"  {'':>9}"   # baseline has no DD tracking
    ...
    print(hdr.replace('AUM($M)', ''))
    print("  " + "-" * (len(hdr) - 2))
    for yr in all_years:
        def fmt_c(s_idx):
            c = state[s_idx]['costs_by_year'].get(yr, 0.0)
            return f"  {c*100:>+8.2f}%" if c > 0 else f"  {'':>9}"
        print(f"  {yr:<6}"
              + fmt_c(S_BASE) + fmt_c(S_ALPHA) + fmt_c(S_MVO)
              + fmt_c(S_HYB) + fmt_c(S_SMART) + fmt_c(S_DYN)
              + fmt_c(S_HEDGE) + fmt_c(S_DD) + fmt_c(S_EXCL))
    # Total row
    print("  " + "-" * (len(hdr) - 2))
    def fmt_ct(s_idx):
        c = state[s_idx]['costs']
        return f"  {c*100:>+8.2f}%" if c > 0 else f"  {'':>9}"
    print(f"  {'Total':<6}"
          + fmt_ct(S_BASE) + fmt_ct(S_ALPHA) + fmt_ct(S_MVO)
          + fmt_ct(S_HYB) + fmt_ct(S_SMART) + fmt_ct(S_DYN)
          + fmt_ct(S_HEDGE) + fmt_ct(S_DD) + fmt_ct(S_EXCL))
