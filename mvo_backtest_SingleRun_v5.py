...
def run_backtest(
    ...
    def _get_regime(dd, exog_regime):
        """
        Determine portfolio regime based on drawdown thresholds.
        Simple threshold comparison — no hysteresis, no persistence.
        Matches old run_mvo_backtest Smart Hybrid logic exactly.
        """
        if dd <= -sh_dd_hybrid:
            return 'mvo'
        elif dd <= -sh_dd_alpha:
            ...
            if ta in _hedge_results:
                ...
                if not sig_df.empty and dt in sig_df.index:
                    ...
                    if len(prev_sig_dates) > 0:
                        ...
                        _hedge_diag.append(f"{ta}:sig={prev_sig}")
                        if prev_sig == 1:
                            trigger_on = True
                            break
                else:
                    _hedge_diag.append(f"{ta}:not_in_sig_df")
            else:
                _hedge_diag.append(f"{ta}:not_in_hedge_results")

        # Print diagnostic once (first 3 dates when S_HEDGE called)
        if s_idx == S_HEDGE and not hasattr(_update_hedge, '_diag_count'):
            _update_hedge._diag_count = 0
        if s_idx == S_HEDGE and _update_hedge._diag_count < 3:
            _update_hedge._diag_count += 1
            print(f"  [HEDGE DIAG] {dt.date()}  trigger_on={trigger_on}  "
                  f"details={_hedge_diag}  "
                  f"hedge_multi_keys={list(hedge_multi.keys()) if hedge_multi else None}",
                  flush=True)
    ...
    for day_idx, dt in enumerate(trading_days):
        ...
        if last_calc_dt is not None and last_calc_dt in composite_by_date:
            ...
            for s_idx in [S_DYN, S_HEDGE, S_DD, S_EXCL]:
                ...
                w_hyp = _apply_advp(w_hyp, cands_s, dt, s_idx)
                ...
                if _check_trigger(s_idx, dt, w_hyp, reg):
                    ...
                    if s_idx == S_DD:
                        ...
                        # ADVP flagged stocks
                        _advp_flagged = set()
                        if volumeRaw_df is not None:
                            for _tkr in w_hyp.index:
                                if _tkr not in volumeRaw_df.columns: continue
                                _past = [d for d in Pxs_df.index if d <= dt][-ADV_WINDOW:]
                                _pxs  = Pxs_df.loc[_past, _tkr].dropna()
                                _vols = volumeRaw_df.loc[_past, _tkr].dropna()
                                _com  = _pxs.index.intersection(_vols.index)
                                if len(_com) < 3: continue
                                _dv   = (_pxs.reindex(_com) *
                                         _vols.reindex(_com).replace({0: np.nan})).median()
                                if (_dv * advp_cap) / _cur_aum < w_hyp[_tkr] - 1e-4:
                                    _advp_flagged.add(_tkr)
                        if _advp_flagged:
                            print(f"  *** ADVP cap active "
                                  f"(AUM=${_cur_aum:,.0f}, cap={advp_cap:.1%} ADV): "
                                  f"{', '.join(sorted(_advp_flagged))}", flush=True)
                        for _tkr in w_hyp.sort_values(ascending=False).index:
                            _flag = '***' if _tkr in _advp_flagged else '   '
        ...
        for s_idx in range(N_STRAT):
            st = state[s_idx]
            st['regime'] = _get_regime(st['dd'], exog)
