...
def run_backtest(
    ...
    def _check_penalty_edge(s_idx, dt):
        ...
        if newly:
            ...
            st['_pen_newly_activated'] = sorted(newly)  # for logging
    ...
    if HMM_REGIME_OVERLAY:
        _hmm_hi, _hmm_lo, _hmm_hi1, _hmm_lo1, _hmm_missing =             _load_hmm_consensus(pd.DatetimeIndex(trading_days))
    ...
    if CUSUM_REGIME_OVERLAY and HMM_REGIME_OVERLAY and _hmm_hi is not None:
        _cu_up, _cu_dn = _load_cusum_states(pd.DatetimeIndex(trading_days))
        ...
        if _dyn_today is not None and not _dyn_today.empty:
            ...
            if _any_eligible:
                ...
                for s_idx in _dyn_list:
                    ...
                    _did_rebal = False
                    _was_pen_forced = st.pop('_pen_newly_activated', None)                         if st.get('penalty_rebal_forced', False) else None
                    ...
                    if s_idx == S_EXCL and _did_rebal:
                        ...
                        if _forced:
                            print(f"\n  *** OFF-CYCLE REBALANCE forced by ETF penalty "
                                  f"activation on held: {', '.join(_forced)} ***",
                                  flush=True)
...
results = run_backtest(
    ...
    etf_penalty_scale=0.01    
)

# New keys available when hedge_multi is provided:
# results['nav_dyn_hedged']   # 7th strategy NAV series
# results['hedge_results']    # full hedge engine output dict:
                              #   ['hedge_account_by_date']
                              #   ['hedge_sweep_by_date']
                              #   ['hedge_log']
                              #   ['port_ret_s']
                              #   ['active_hedges']

scores_df, ic_weights, prefilt, alpha_pool, mvo_pool = get_last_rebal_diagnostics(
    results, Pxs_df, top_n=25, prefilt_pct=0.5, mom_filter=2,
    universe_mult=5, min_cov_matrices=2
)
