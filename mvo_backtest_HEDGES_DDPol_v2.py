...
DD_HALF_THRESHOLD  = 0.175   # combined NAV DD → halve exposure (aligned with SH_DD_HYBRID)
DD_FULL_THRESHOLD  = 0.25    # combined NAV DD → fully flat
DD_REENTRY_PCT     = 0.075   # single recovery threshold from trough → full re-entry
DD_REENTRY_CONFIRM = 10      # days recovery must persist before re-entry
...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    nav_dd = None
    _dd_log = []   # list of DD events for display
    if nav_dyn_hedged is not None and not nav_dyn_hedged.empty:
        print(f"\n  {'='*72}")
        print(f"  DRAWDOWN POLICY  (half@{DD_HALF_THRESHOLD:.0%}, flat@{DD_FULL_THRESHOLD:.0%})")
        print(f"  Re-entry: +{DD_REENTRY_PCT_1:.0%}/+{DD_REENTRY_PCT_2:.0%}/"
              f"+{DD_REENTRY_PCT_3:.0%}/+{DD_REENTRY_PCT_4:.0%} from trough "
              f"(confirm {DD_REENTRY_CONFIRM}d, MVO mode)")
        print(f"  {'='*72}")

        # Daily returns of fully loaded hedged strategy
        base_ret    = nav_dyn_hedged.pct_change().fillna(0)
        # Theoretical fully-loaded MVO portfolio returns (for re-entry tracking)
        ...
        hwm         = nav_dyn_hedged.iloc[0]
        exposure    = 1.0        # current exposure level (0, 0.25, 0.5, 0.75, 1.0)
        trough      = nav_dyn_hedged.iloc[0]  # lowest combined NAV since DD activated
        theo_nav    = nav_dyn_hedged.iloc[0]  # theoretical fully-loaded NAV from trough
        dd_active   = False      # whether DD policy is currently active
        confirm_cnt = 0          # consecutive days above re-entry threshold
        ...
        for i, dt in enumerate(nav_dyn_hedged.index[1:], 1):
            ...
            actual_r = raw_r * exposure

            curr_nav = prev_nav * (1 + actual_r)
            ...
            # ── DD POLICY: exit / de-gross ─────────────────────────────────
            if not dd_active and dd_current <= -DD_FULL_THRESHOLD:
                # Fully flat
                old_exp = exposure
                exposure  = 0.0
                dd_active = True
                trough    = curr_nav
                theo_nav  = curr_nav   # reset theoretical from here
                confirm_cnt = 0
                # Compute hedge cost for unwinding (current active hedges)
                hedge_unwind_cost = 0.0
                if hedge_results:
                    for h in hedge_results.get('active_hedges', {}).values():
                        hedge_unwind_cost += 2 * TRADING_COST_BPS / 10000 * h['weight']
                # Portfolio trading cost for full unwind
                port_cost = old_exp * 0.5 * TRADING_COST_BPS / 10000  # approx
                curr_nav  = curr_nav * (1 - port_cost - hedge_unwind_cost)
                dd_nav.iloc[i] = curr_nav
                _dd_log.append({
                    'date': dt, 'event': 'FULLY FLAT',
                    'dd': dd_current, 'exposure_from': old_exp,
                    'exposure_to': 0.0, 'trough': trough,
                })
                print(f"\n  {dt.date()}  [DD POLICY: FULLY FLAT]  "
                      f"combined NAV dd={dd_current:.1%}  "
                      f"exposure {old_exp:.0%}→0%  costs≈{(port_cost+hedge_unwind_cost)*100:.2f}bps")

            elif not dd_active and dd_current <= -DD_HALF_THRESHOLD:
                # Halve exposure
                old_exp  = exposure
                exposure = 0.5
                dd_active = True
                ...
                theo_nav  = curr_nav
                confirm_cnt = 0
                port_cost = old_exp * 0.25 * TRADING_COST_BPS / 10000
                curr_nav  = curr_nav * (1 - port_cost)
                dd_nav.iloc[i] = curr_nav
                _dd_log.append({
                    'date': dt, 'event': 'HALF EXPOSURE',
                    'dd': dd_current, 'exposure_from': old_exp,
                    'exposure_to': 0.5, 'trough': trough,
                })
                print(f"\n  {dt.date()}  [DD POLICY: HALF EXPOSURE]  "
                      f"combined NAV dd={dd_current:.1%}  "
                      f"exposure {old_exp:.0%}→50%  costs≈{port_cost*100:.2f}bps")

            # ── Escalate to fully flat if already halved and drops further ──
            elif dd_active and exposure == 0.5 and dd_current <= -DD_FULL_THRESHOLD:
                old_exp  = exposure
                exposure = 0.0
                trough   = min(trough, curr_nav)
                theo_nav = curr_nav
                confirm_cnt = 0
                port_cost = old_exp * 0.5 * TRADING_COST_BPS / 10000
                curr_nav  = curr_nav * (1 - port_cost)
                dd_nav.iloc[i] = curr_nav
                _dd_log.append({
                    'date': dt, 'event': 'FULLY FLAT (escalation)',
                    'dd': dd_current, 'exposure_from': old_exp,
                    'exposure_to': 0.0, 'trough': trough,
                })
                print(f"\n  {dt.date()}  [DD POLICY: ESCALATE TO FLAT]  "
                      f"dd={dd_current:.1%}  exposure 50%→0%")

            # ── Re-entry logic ──────────────────────────────────────────────
            elif dd_active and exposure < 1.0:
                # Update trough
                trough = min(trough, curr_nav)
                ...
                if confirm_cnt >= DD_REENTRY_CONFIRM:
                    ...
                    print(f"\n  {dt.date()}  [DD POLICY: FULL RE-ENTRY]  "
                          f"theoretical recovery={theo_recovery:.1%}  "
                          f"confirmed {DD_REENTRY_CONFIRM}d  MVO mode  "
                          f"costs≈{port_cost*100:.2f}bps  [FULLY REINVESTED]")
    ...
    for nav_s, lbl in [
        ...
        (nav_dd,           f'Dyn+Hedge+DD ({DD_HALF_THRESHOLD:.0%}/{DD_FULL_THRESHOLD:.0%})'),
    ]:
    ...
    if hedge_results is not None:
        for e in hedge_results.get('hedge_log', []):
            if e['event'] == 'CLOSE':
                yr = e['date'].year
                # Total round-trip cost = sum of 2*10bps*weight per instrument
    ...
    print()
    sep_width = 78 + (16 if show_hedge else 0)
    print(f"  {'-'*sep_width}")
