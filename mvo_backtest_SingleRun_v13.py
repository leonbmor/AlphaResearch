...
def run_backtest(
    ...
    if _last_cd:
        ...
        _exog_td  = _exog_regime(today_ts)
        ...
        for lbl, s_idx in _all_strats_summary:
            st  = state[s_idx]
            reg = _get_regime(st['dd'], _exog_td)
            if s_idx == S_BASE:
                _fresh_w[lbl] = _build_baseline(today_ts)
            elif s_idx == S_ALPHA:
    ...
    # Drifted current weights = what you currently hold (last rebalance drifted to today)
    # For static strategies on a calc_date: use the rebalance before last
    # so the delta shows what actually changed vs previous holdings
    _drifted_w = {}
    for lbl, s_idx in _all_strats_summary:
        st  = state[s_idx]
        log = st['rebal_log']
        if not log:
            ...
            continue
        if len(log) >= 2:
            # Use the second-to-last rebalance drifted to today
            prev_r = log[-2]
            _drifted_w[lbl] = _drift(prev_r['w'], prev_r['dt'], today_ts)
        else:
            # Only one rebalance on record — no previous to compare against
            _drifted_w[lbl] = pd.Series(dtype=float)
