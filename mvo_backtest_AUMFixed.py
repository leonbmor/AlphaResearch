...
def run_mvo_backtest(Pxs_df, sectors_s, weights_by_year, regime_s,
    ...
    for yr in all_years:
        ...
        # Use actual NAV at year-end for correct AUM display
        if _aum_nav is not None and not _aum_nav.empty:
            nav_to_yr = _aum_nav[_aum_nav.index.year <= yr]
            if not nav_to_yr.empty:
                closing_aum = AUM * nav_to_yr.iloc[-1]
            else:
                closing_aum = AUM
        else:
            closing_aum = AUM
    ...
    try:
        ...
        if comp_today is not None:
            # Use running NAV for liquidity filter — same as backtest loop
            _today_aum = AUM * float(_aum_nav.iloc[-1]) if (
                _aum_nav is not None and not _aum_nav.empty) else AUM
            cands_t = comp_today.dropna()
            cands_t = cands_t.loc[[t for t in cands_t.index if t in pxs_cols]]
            # Apply ADVP liquidity filter with correct running AUM
            if volumeRaw_df is not None and _today_aum > 0:
                _past_liq = [d for d in Pxs_df.index if d <= today_ts][-63:]
                def _snap_liquid(tkr):
                    if tkr not in volumeRaw_df.columns: return True
                    px_s  = Pxs_df.loc[_past_liq, tkr].dropna()
                    vol_s = volumeRaw_df.loc[_past_liq, tkr].dropna()
                    com   = px_s.index.intersection(vol_s.index)
                    if len(com) < 3: return True
                    dv = (px_s.reindex(com) * vol_s.reindex(com).replace({0: np.nan})).median()
                    return (dv * advp_cap) / _today_aum >= min_weight
                cands_t = cands_t[[t for t in cands_t.index if _snap_liquid(t)]]
    ...
    try:
        ...
        # If the last rebalance is today, use the previous one as the
        # "old" portfolio base — today's portfolio IS the new target
        ...
        if _s9_ldt and _s9_wbd:
            ...
            if not _s9_lw.empty:
                ...
                if _s9_tk and _s9_ldt in Pxs_df.index and today_ts in Pxs_df.index:
                    _s9_pxr = Pxs_df.loc[_s9_ldt, _s9_tk]
                    _s9_pxt = Pxs_df.loc[today_ts, _s9_tk]
                    _s9_d   = _s9_lw.reindex(_s9_tk) * (_s9_pxt / _s9_pxr).fillna(1)
                    _w_s9_old = (_s9_d / _s9_d.sum()) if _s9_d.sum() > 0 else _s9_lw.reindex(_s9_tk)
                else:
                    _w_s9_old = _s9_lw

        # Compute allocation changes per strategy
        all_tickers = set(w_dyn_old.index) | set(w_dyn_new.index) | set(_w_s9_old.index)
        ...
        for tkr in sorted(all_tickers):
            ...
            # Excl (S9) column: target = w_smart_t (excl-filtered); old = strategy 9 drifted
            new_s9 = (w_smart_t.get(tkr, 0.0)
                      if w_smart_t is not None and not w_smart_t.empty else new_dyn)
