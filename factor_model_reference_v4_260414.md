...
**Trigger:** QQQ signal = 1 OR SPX signal = 1 (1-day lag)
**Unwind:** QQQ signal = 0 AND SPX signal = 0
...
Baseline (quality)         27.3%  28.0%    0.98  -42.0%    0.65x
Pure Alpha (conc=2.0x)     83.5%  42.6%    1.96  -45.2%    1.85x
MVO (IC=0.03, max=8%)      57.2%  33.7%    1.70  -38.9%    1.47x
Hybrid (Alpha+MVO avg)     70.3%  37.8%    1.86  -41.1%    1.71x
Smart Hybrid               79.0%  40.1%    1.97  -41.4%    1.91x
Dynamic                    76.5%  39.3%    1.95  -39.0%    1.96x
Dynamic + Hedge            80.0%  39.5%    2.03  -37.8%    2.12x  ← best all-round
```
...
### Next Session
1. **Drawdown policy** — hard HWM-based boundary applied after all overlays
2. **Smart hybrid threshold robustness** — grid search over SH parameters
3. **Capacity / liquidity sensitivity** — grid of `mktcap_floor` and volume filters
...
- [DONE] Macro hedge indicators (MOVE proxy + QBD) with point-in-time signal fitting
- [DONE] Multi-instrument hedge analysis with PostgreSQL caching (`macro_indicators.py`)
- [DONE] Hedge engine integrated into `mvo_backtest.py` (self-contained)
- [DONE] Dynamic + Hedge as 7th strategy: CAGR 80.0%, Sharpe 2.03, MDD -37.8%, CAGR/DD 2.12x
- [DONE] 2022 protection: +3.65% vs -6.00% for Dynamic standalone
- [DONE] Asymmetric smart hybrid exit thresholds + regime persistence
- [DONE] Regime-specific TO thresholds for dynamic rebalancing
- [DONE] Drift-adjusted implied turnover in dynamic triggers
...
- **SPX vs SPY:** portfolio uses `SPX` column name (not `SPY`) in `Pxs_df`. Always use `SPX` in `hedges_l` and `trigger_assets`.
