    ...
    def _load_weights_cache(mv):
        try:
            ...
            if not rows:
                return {}
            result = {}
            for date_, factor, weight in rows:
                dt = pd.Timestamp(date_)
                if dt not in result:
                    result[dt] = {}
                result[dt][factor] = float(weight)
            return {dt: pd.Series(w) for dt, w in result.items()}
        except Exception as e:
            print(f"  WARNING: _load_weights_cache failed: {e}")
            return {}
