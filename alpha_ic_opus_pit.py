...
# Factors that receive ALPHA WEIGHT downstream. OU is still MEASURED in the IC
# study (computed, summarised, plotted) but is NOT given weight — so it is
# excluded here, before the bounded-normalize, and the four below are normalized
# directly on their own simplex (sum to 1, bounds respected in one pass).
ICS_ALPHA_FACTORS = ['Mom_12M1', 'Value', 'Quality', 'Idio_Mom']

# (IC cache tables removed — no SQL caching, all computation in pandas)
...
def compute_rolling_regime_weights(ic_results, ic_annual, Pxs_df,
    ...
    # Restrict the WEIGHTED factor set to the alpha factors (OU excluded).
    # Preserve ICS_ALPHA_FACTORS order; intersect with what's actually present
    # in ic_results so a missing factor degrades gracefully.
    factors   = [f for f in ICS_ALPHA_FACTORS if f in ic_results]
    regimes   = [0.0, 0.5, 1.0]
    ...
    # Each entry: (score_dt, realize_dt, factor, ic_value, regime_at_score_dt)
    #
    # PIT / train-validation split: an IC observation is only KNOWN once its
    # forward-return window has fully closed. For a score date s and horizon h
    # (in TRADING days), the window closes on the h-th trading session after s
    # ('realize_dt'). At a rebalance date c_dt we may only use observations whose
    # realize_dt <= c_dt (the "validation set" — forward returns already realized).
    #
    # Horizons are kept SEPARATE (not pre-averaged) so each is gated on its own
    # realization: the 21d IC at s becomes usable 21 trading days later, the 63d
    # IC at the same s only after 63. Merging first would force the 21d horizon to
    # wait for the 63d window to close — an unnecessary extra lag with no PIT basis.
    def _get_regime(dt):
        past = regime_s[regime_s.index <= dt]
        return float(past.iloc[-1]) if not past.empty else 0.5

    # Trading calendar for mapping score_dt -> realize_dt (h-th session after s)
    _cal = Pxs_df.index

    def _realize_dt(score_dt, h_trd):
        """h_trd-th trading session strictly after score_dt; NaT if beyond data."""
        future = _cal[_cal > score_dt]
        if len(future) < h_trd:
            return pd.NaT
        return future[h_trd - 1]

    # Horizon label ('21d') -> integer trading days (21)
    def _h_to_int(h):
        return int(str(h).replace('d', ''))
    ...
    for fname in factors:
        ...
        for h in hz_cols:
            ...
            for dt, ic_val in s.items():
                score_dt = pd.Timestamp(dt)
                rdt = _realize_dt(score_dt, h_trd)
                if pd.isna(rdt):
                    continue                       # window not closed within data
                ic_obs.append((score_dt, rdt, fname, float(ic_val),
                               _get_regime(score_dt)))

    ic_obs.sort(key=lambda x: x[0])

    # ── Helper: compute weights at a given date for a given target regime ─────
    _decay_lambda = np.log(2) / half_life_yrs   # exponential decay constant

    def _weights_at(cutoff_dt, r_target):
        """
        Similarity × recency weighted IC over the VALIDATION SET at cutoff_dt:
        only observations whose forward window has closed (realize_dt <= cutoff_dt).
        Recency decay is measured from the SCORE date (age of the evidence), not
        the realization date.
        """
        prior = [(s_dt, fn, ic, r_obs)
                 for s_dt, r_dt, fn, ic, r_obs in ic_obs
                 if r_dt <= cutoff_dt]
        if not prior:
            return equal_w.copy()
        ...
        for fname in factors:
            num = den = 0.0
            for s_dt, fn, ic_val, r_obs in prior:
                if fn != fname:
                    continue
                sim = max(0.0, 1.0 - abs(r_target - r_obs))
                if sim == 0.0:
                    continue
                age_yrs = (cutoff_dt - s_dt).days / 365.25
                decay   = np.exp(-_decay_lambda * age_yrs)
                w       = sim * decay
                num    += w * ic_val
                den    += w
    ...
    # Emitted weight vector contains exactly the four alpha factors (OU excluded).
    ALL_FACTORS = ICS_ALPHA_FACTORS

    # weights_by_date: computed fresh each run — deterministic because
    # calc_dates are frozen, guaranteeing ic_obs is always identical
    weights_by_date = {}
    for dt in rebal_dates:
        r_t = _get_regime(dt)
        w   = _weights_at(dt, r_t)
        w   = w.reindex(ALL_FACTORS).fillna(0.0)
    ...
    for yr in range(min_yr, max_yr + 1):
        ...
        # Count the validation set actually usable at cutoff (forward window closed),
        # consistent with what _weights_at consumes.
        prior    = [x for x in ic_obs if x[1] <= cutoff]   # x[1] = realize_dt
        n_obs    = len(prior)
...
def _ics_load_quality(universe, calc_dates, Pxs_df, sectors_s):
    """Load quality scores directly from DB cache.
    Bypasses load_quality_scores kernel function to avoid any Pxs_df dependency.
    Z-scoring is deferred to per-date composite building over active_u."""
    print("  Loading quality scores from DB cache...")
    try:
        with ENGINE.connect() as conn:
            df = pd.read_sql(
        ...
        if df.empty:
            raise ValueError("empty quality cache")
        df['date']   = pd.to_datetime(df['date'])
        df['ticker'] = df['ticker'].apply(clean_ticker)
        df['score']  = df['score'].astype(float)
        qual = df.pivot_table(index='date', columns='ticker',
                              values='score', aggfunc='last')
        qual = qual.reindex(columns=universe)
        # Forward-fill to calc_dates
        all_dates = calc_dates.union(qual.index).sort_values()
        qual_ff   = qual.reindex(all_dates).ffill().reindex(calc_dates)
        print(f"  Quality scores: {qual_ff.notna().any(axis=1).sum()} dates with data")
        return qual_ff
    except Exception as e:
        print(f"  WARNING: DB quality cache load failed ({e}) — returning empty")
...
def _ics_load_value(universe, calc_dates, sectors_s, model_version):
    """Load value scores from DB cache, forward-filled to calc_dates."""
    ...
    try:
        ...
        print(f"  WARNING: could not load {tbl}: {e} — Value factor will be skipped")
        return pd.DataFrame(index=calc_dates, columns=universe, dtype=float)
...
def run_ic_study_alpha(Pxs_df, sectors_s,
    ...
    All computation is done in pandas — no SQL caching logic.
    SQL is used only for loading raw inputs (quality, value, OU, residuals).

    IC values are recomputed fresh every run from frozen calc_dates.
    This guarantees stability: adding a new row to Pxs_df cannot change
    historical IC values because all computations are strictly PIT.
    ...
    rebal_freq    : int — rebalance frequency in trading days (default 21)
    force_recompute : bool — kept for API compatibility, always recomputes
    ...
    # ── Calc dates: pure pandas, no DB ────────────────────────────────────────
    # Stride from index 0 — stable as Pxs_df grows (new dates only added at end,
    # and only when a full rebal_freq of new trading days has accumulated).
    ...
    # ── Forward returns — computed once for all calc_dates ────────────────────
    print(f"\n[1/6] Computing forward returns for {len(calc_dates)} dates...")
    fwd_rets = {}
    for h in ICS_HORIZONS:
        fwd_rets[h] = _ics_forward_returns(universe, calc_dates, Pxs_df, h)
        print(f"  {h}d forward returns: "
              f"{fwd_rets[h].notna().any(axis=1).sum()} dates")
    ...
    # ── Compute IC series for all calc_dates ──────────────────────────────────
    factor_map = {
    ...
    for fname, scores_df in factor_map.items():
        if scores_df is None or scores_df.empty:
            warnings.warn(f"  {fname}: no scores — skipping")
            continue
...
# force rebuild:
# ic_results, ic_annual = run_ic_study_alpha(Pxs_df, sectors_s, force_recompute=True, model_version='v2')
